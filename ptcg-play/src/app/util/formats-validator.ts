...
export class FormatValidator {
  ...
  static getValidFormatsForCardList(
    cards: Card[],
    allCards?: Card[],
  ): Format[] {
      ...
      .filter(
        (c) =>
          ...
          (c.superType !== SuperType.ENERGY ||
            (<any>c).energyType === EnergyType.SPECIAL),
      )
      .forEach((card) => {
    ...
    if (hasUnownTag) {
      const unownCount = cards.filter((card) =>
        card.name.includes("Unown"),
      ).length;
      if (unownCount > 4) {
        return formatList.filter(
          (f) =>
    ...
    const hasArceusRule = cards.some((card) =>
      card.tags.includes(CardTag.ARCEUS),
    );
    if (hasArceusRule) {
      const arceusRuleCount = cards.filter((card) =>
        card.tags.includes(CardTag.ARCEUS),
      ).length;
    ...
    if (formatList.includes(Format.GLC)) {
      ...
      const nonBasicEnergyCards = cards.filter(
        (c) =>
          c.superType !== SuperType.ENERGY &&
          (<any>c).energyType !== EnergyType.BASIC,
      );
    ...
    if (formatList.includes(Format.ETERNAL)) {
      ...
      const CrushingHammerPresent = cards.filter(
        (c) => c.name === "Crushing Hammer",
      );
      const EnergyRemoval2Present = cards.filter(
        (c) => c.name === "Energy Removal 2",
      );
      if (
        CrushingHammerPresent.length > 0 &&
        EnergyRemoval2Present.length > 0
      ) {
    ...
    if (set.has("Fairy Energy") || set.has("Wonder Energy")) {
      return formatList.filter(
        (f) => f !== Format.STANDARD && f !== Format.RETRO,
      );
    }

    if (set.has("Metal Energy") || set.has("Darkness Energy")) {
      return formatList.filter((f) => f !== Format.RETRO);
    }
  ...
  static getValidFormats(card: Card, allCards?: Card[]): Format[] {
    ...
    [
      ...
      this.isValid(card, format, ANY_PRINTING_ALLOWED, allCards)
        ? formats.push(format)
        : null;
    });
  ...
  static isValid(
    ...
    if (
      ...
      (<any>card).energyType === EnergyType.BASIC
    ) {
      return true;
    }
    if (anyPrintingAllowed && anyPrintingAllowed.includes(card.name)) {
      switch (format) {
        case Format.UNLIMITED:
          return true;
        case Format.ETERNAL:
          return !BanLists[format].includes(
            `${card.name} ${card.set} ${card.setNumber}`,
          );
        case Format.STANDARD: {
          if (allCards) {
            const allPrintings = allCards.filter(
              (c) => c && c.name === card.name,
            );
        ...
        case Format.STANDARD_NIGHTLY: {
          if (allCards) {
            ...
            return allPrintings.some((c) =>
              this.isPrintingLegalInStandardNightly(c),
            );
          }
          return this.isPrintingLegalInStandardNightly(card);
        }
...
export const BanLists: { [key: number]: string[] } = {
  ...
  [Format.ETERNAL]: [
    ...
    "Giratina LOT 97",
    "Giratina SMP 151",
    "Mewtwo V-UNION SWSH 159",
    "Mewtwo V-UNION SWSH 160",
    "Mewtwo V-UNION SWSH 161",
    "Mewtwo V-UNION SWSH 162",
    "Celebi ex pop2 17",
...
export const PromoReleaseDates: PromoReleaseRule[] = Object.entries(
  promoRangesBySet,
...
export const SetReleaseDates: { [key: string]: Date } = {
  ...
  M6: new Date("2026-07-31"), // Storm Emerald, no equivalent English release date yet
