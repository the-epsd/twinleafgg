...
import { ChooseCardsPrompt, GameError, GameMessage, PokemonCard, ShowCardsPrompt, ShuffleDeckPrompt, StateUtils, Player } from '../../../game';
...
function* playCard(next: Function, store: StoreLike, state: State, effect: TrainerEffect): IterableIterator<State> {
  ...
  player.deck.cards.forEach((card, index) => {
    if (!(card instanceof PokemonCard) || !card.tags.includes(CardTag.POKEMON_ex) || !card.tags.includes(CardTag.POKEMON_SV_MEGA)) {
      blocked.push(index);
    }
  });
