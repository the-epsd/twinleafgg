...
export class MagnetMetalEnergy extends EnergyCard {
  ...
  public text: string =
    ...
    'As long as this card is attached to a [M] Pokémon, that Pokémon has no Retreat Cost.';
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    ...
    if (effect instanceof CheckRetreatCostEffect) {
      ...
      const hasThisEnergy =
        player.active.cards.includes(this) || player.active.energies.cards.includes(this);
      if (!hasThisEnergy) {
        return state;
      }
