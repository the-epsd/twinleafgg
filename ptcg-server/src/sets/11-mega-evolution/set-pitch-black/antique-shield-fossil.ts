...
export class AntiqueShieldFossil extends TrainerCard {
  ...
  public name: string = 'Antique Shield Fossil';
  public fullName: string = 'Antique Shield Fossil M5';

  private armorProtectionActive(store: StoreLike, state: State, defenderOwner: ReturnType<typeof StateUtils.findOwner>): boolean {
    const activeCard = defenderOwner.active.getPokemonCard();
    if (activeCard !== this) {
      return false;
    }
    try {
      store.reduceEffect(state, new PowerEffect(defenderOwner, this.powers[0], this as unknown as PokemonCard));
    } catch {
      return false;
    }
    return true;
