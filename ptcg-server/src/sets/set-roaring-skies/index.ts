import { Card } from '../../game/store/card/card';
import { Absol } from './absol';
import { Altaria } from './altaria';
import { Altaria2 } from './altaria-2';
import { Articuno } from './articuno';
import { Articuno2 } from './articuno-2';
import { Bagon } from './bagon';
import { Bagon2 } from './bagon-2';
import { Banette } from './banette';
import { Banette2 } from './banette-2';
import { Beautifly } from './beautifly';
import { Binacle } from './binacle';
import { Carbink } from './carbink';
import { Cascoon } from './cascoon';
import { Deoxys } from './deoxys';
import { Dragonair } from './dragonair';
import { Dragonite } from './dragonite';
import { Dragonite2 } from './dragonite-2';
import { Dratini } from './dratini';
import { Dunsparce } from './dunsparce';
import { Dustox } from './dustox';
import { Dustox2 } from './dustox-2';
import { Electrike } from './electrike';
import { Electrode } from './electrode';
import { Exeggcute } from './exeggcute';
import { Exeggutor } from './exeggutor';
import { Fearow } from './fearow';
import { Fletchinder } from './fletchinder';
import { Fletchling } from './fletchling';
import { GalladeEx } from './gallade-ex';
import { Gligar } from './gligar';
import { Gliscor } from './gliscor';
import { Hawlucha } from './hawlucha';
import { HydreigonEx } from './hydreigon-ex';
import { Inkay } from './inkay';
import { Jirachi } from './jirachi';
import { Klefki } from './klefki';
import { LatiosEx } from './latios-ex';
import { MGalladeEx } from './m-gallade-ex';
import { MLatiosEx } from './m-latios-ex';
import { MRayquazaEX } from './m-rayquaza-ex';
import { Manectric } from './manectric';
import { Meowth } from './meowth';
import { Natu } from './natu';
import { Natu2 } from './natu-2';
import { Nincada } from './nincada';
import { Ninjask } from './ninjask';
import { Pelipper } from './pelipper';
import { Pidove } from './pidove';
import { Pikachu } from './pikachu';
import { RayquazaEx } from './rayquaza-ex';
import { RayquazaEx2 } from './rayquaza-ex-2';
import { Reshiram } from './reshiram';
import { Salamence } from './salamence';
import { ShayminEx } from './shaymin-ex';
import { Shedinja } from './shedinja';
import { Shelgon } from './shelgon';
import { Shuppet } from './shuppet';
import { Silcoon } from './silcoon';
import { Skarmory } from './skarmory';
import { Spearow } from './spearow';
import { Swablu } from './swablu';
import { Swellow } from './swellow';
import { Swellow2 } from './swellow-2';
import { Taillow } from './taillow';
import { Talonflame } from './talonflame';
import { ThundurusEx } from './thundurus-ex';
import { Togekiss } from './togekiss';
import { Togekiss2 } from './togekiss-2';
import { Togepi } from './togepi';
import { Togetic } from './togetic';
import { Tranquill } from './tranquill';
import { Tropius } from './tropius';
import { Unfezant } from './unfezant';
import { Unfezant2 } from './unfezant-2';
import { Victini } from './victini';
import { Voltorb } from './voltorb';
import { Wingull } from './wingull';
import { Wurmple } from './wurmple';
import { Xatu } from './xatu';
import { Zapdos } from './zapdos';
import { Zekrom } from './zekrom';
import { GalladeSpiritLink } from './gallade-spirit-link';
import { HealingScarf } from './healing-scarf';
import { LatiosSpiritLink } from './latios-spirit-link';
import { MegaTurbo } from './mega-turbo';
import { RayquazaSpiritLink } from './rayquaza-spirit-link';
import { SkyField } from './sky-field';
import { Steven } from './steven';
import { TrainersMail } from './trainers-mail';
import { Wally } from './wally';
import { WideLens } from './wide-lens';
import { Winona } from './winona';
import { DoubleDragonEnergy } from './double-dragon-energy';

import {
  TrainersMailROS,
  ReviveROS,
  SwitchROS,
  UltraBallROS,
  RayquazaEx3ROS,
  MRayquazaEx3ROS,
  ShayminEx3ROS,
  Wally2ROS,
  Winona2ROS,
  EnergySwitchPKROS,
  VsSeekerSVROS,
  ShayminEx2ROS,
  TrainersMail2,
  ThundurusEx2,
  GalladeEx2,
  MGalladeEx2,
  LatiosEx2,
  MLatiosEx2,
  HydreigonEx2,
} from './other-prints';
import { MRayquazaEX2 } from './m-rayquaza-ex-2';

export const setRoaringSkies: Card[] = [
  // Pokemon
  new Absol(),
  new Altaria(),
  new Altaria2(),
  new Articuno(),
  new Articuno2(),
  new Bagon(),
  new Bagon2(),
  new Banette(),
  new Banette2(),
  new Beautifly(),
  new Binacle(),
  new Carbink(),
  new Cascoon(),
  new Deoxys(),
  new Dragonair(),
  new Dragonite(),
  new Dragonite2(),
  new Dratini(),
  new Dunsparce(),
  new Dustox(),
  new Dustox2(),
  new Electrike(),
  new Electrode(),
  new Exeggcute(),
  new Exeggutor(),
  new Fearow(),
  new Fletchinder(),
  new Fletchling(),
  new GalladeEx(),
  new Gligar(),
  new Gliscor(),
  new Hawlucha(),
  new HydreigonEx(),
  new Inkay(),
  new Jirachi(),
  new Klefki(),
  new LatiosEx(),
  new MGalladeEx(),
  new MLatiosEx(),
  new MRayquazaEX(),
  new MRayquazaEX2(),
  new Manectric(),
  new Meowth(),
  new Natu(),
  new Natu2(),
  new Nincada(),
  new Ninjask(),
  new Pelipper(),
  new Pidove(),
  new Pikachu(),
  new RayquazaEx(),
  new RayquazaEx2(),
  new Reshiram(),
  new Salamence(),
  new ShayminEx(),
  new Shedinja(),
  new Shelgon(),
  new Shuppet(),
  new Silcoon(),
  new Skarmory(),
  new Spearow(),
  new Swablu(),
  new Swellow(),
  new Swellow2(),
  new Taillow(),
  new Talonflame(),
  new ThundurusEx(),
  new Togekiss(),
  new Togekiss2(),
  new Togepi(),
  new Togetic(),
  new Tranquill(),
  new Tropius(),
  new Unfezant(),
  new Unfezant2(),
  new Victini(),
  new Voltorb(),
  new Wingull(),
  new Wurmple(),
  new Xatu(),
  new Zapdos(),
  new Zekrom(),

  // Trainers
  new GalladeSpiritLink(),
  new HealingScarf(),
  new LatiosSpiritLink(),
  new MegaTurbo(),
  new RayquazaSpiritLink(),
  new SkyField(),
  new Steven(),
  new TrainersMail(),
  new Wally(),
  new WideLens(),
  new Winona(),

  // Energy
  new DoubleDragonEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new TrainersMailROS(),
  new ReviveROS(),
  new SwitchROS(),
  new UltraBallROS(),
  new RayquazaEx3ROS(),
  new MRayquazaEx3ROS(),
  new ShayminEx3ROS(),
  new Wally2ROS(),
  new Winona2ROS(),
  new EnergySwitchPKROS(),
  new VsSeekerSVROS(),
  new ShayminEx2ROS(),
  new TrainersMail2(),
  new ThundurusEx2(),
  new GalladeEx2(),
  new MGalladeEx2(),
  new LatiosEx2(),
  new MLatiosEx2(),
  new HydreigonEx2(),
];
