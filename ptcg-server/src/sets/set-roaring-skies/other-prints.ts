import { Revive } from '../set-base-set/revive';
import { Switch } from '../set-base-set/switch';
import { UltraBall } from '../set-scarlet-and-violet/ultra-ball';
import { RayquazaEx as RayquazaExROS75 } from '../set-roaring-skies/rayquaza-ex';
import { MRayquazaEx as MRayquazaExROS76 } from '../set-roaring-skies/mega-rayquaza-ex';
import { ShayminEx as ShayminExROS77 } from '../set-roaring-skies/shaymin-ex';
import { Wally as WallyROS94 } from '../set-roaring-skies/wally';
import { Winona as WinonaROS96 } from '../set-roaring-skies/winona';
import { EnergySwitchPK } from '../set-ex-power-keepers/other-prints';
import { VsSeekerSV } from '../set-supreme-victors/other-prints';
import { TrainersMail } from './trainers-mail';
import { ThundurusEx } from './thundurus-ex';
import { GalladeEx } from './gallade-ex';
import { MGalladeEx } from './m-gallade-ex';
import { LatiosEx } from './latios-ex';
import { MLatiosEx } from './m-latios-ex';
import { HydreigonEx } from './hydreigon-ex';

export class TrainersMailROS extends TrainersMail {
  public set: string = 'ROS';
  public setNumber: string = '92a';
  public fullName: string = 'Trainers\' Mail (ROS 92a)';
  public legacyFullName = 'Trainers Mail ROS 92a';
}
export class ReviveROS extends Revive {
  public setNumber = '88';
  public fullName: string = 'Revive (ROS 88)';
  public legacyFullName = 'Revive ROS';
  public set = 'ROS';
}

export class SwitchROS extends Switch {
  public setNumber = '91';
  public fullName: string = 'Switch (ROS 91)';
  public legacyFullName = 'Switch ROS';
  public set = 'ROS';
}

export class UltraBallROS extends UltraBall {
  public setNumber = '93';
  public fullName: string = 'Ultra Ball (ROS 93)';
  public legacyFullName = 'Ultra Ball ROS';
  public set = 'ROS';
}

export class RayquazaEx3ROS extends RayquazaExROS75 {
  public setNumber = '104';
  public fullName: string = 'Rayquaza-EX (ROS 104)';
  public legacyFullName = 'Rayquaza EX3 ROS';
  public set = 'ROS';
}

export class MRayquazaEx3ROS extends MRayquazaExROS76 {
  public setNumber = '105';
  public fullName: string = 'M Rayquaza-EX (ROS 105)';
  public legacyFullName = 'M Rayquaza-EX3 ROS';
  public set = 'ROS';
}

export class ShayminEx3ROS extends ShayminExROS77 {
  public setNumber = '106';
  public fullName: string = 'Shaymin-EX (ROS 106)';
  public legacyFullName = 'Shaymin EX3 ROS';
  public set = 'ROS';
}

export class Wally2ROS extends WallyROS94 {
  public setNumber = '107';
  public fullName: string = 'Wally (ROS 107)';
  public legacyFullName = 'Wally2 ROS';
  public set = 'ROS';
}

export class Winona2ROS extends WinonaROS96 {
  public setNumber = '108';
  public fullName: string = 'Winona (ROS 108)';
  public legacyFullName = 'Winona2 ROS';
  public set = 'ROS';
}

export class EnergySwitchPKROS extends EnergySwitchPK {
  public setNumber = '109';
  public fullName: string = 'Energy Switch (ROS 109)';
  public legacyFullName = 'Energy Switch ROS';
  public set = 'ROS';
}

export class VsSeekerSVROS extends VsSeekerSV {
  public setNumber = '110';
  public fullName: string = 'VS Seeker (ROS 110)';
  public legacyFullName = 'VS Seeker ROS';
  public set = 'ROS';
}

export class ShayminEx2ROS extends ShayminExROS77 {
  public setNumber = '77a';
  public fullName: string = 'Shaymin-EX (ROS 77a)';
  public legacyFullName = 'Shaymin EX2 ROS';
  public set = 'ROS';
}

export class TrainersMail2 extends TrainersMail {
  public set: string = 'ROS';
  public setNumber: string = '92a';
  public fullName: string = 'Trainers\' Mail (ROS 92a)';
  public legacyFullName = 'Trainers\' Mail ROS 92a';
}

export class ThundurusEx2 extends ThundurusEx {
  public set: string = 'ROS';
  public setNumber: string = '98';
  public fullName: string = 'Thundurus-EX (ROS 98)';
  public legacyFullName = 'Thundurus-EX ROS 98';
}

export class GalladeEx2 extends GalladeEx {
  public set: string = 'ROS';
  public setNumber: string = '99';
  public fullName: string = 'Gallade-EX (ROS 99)';
  public legacyFullName = 'Gallade-EX ROS 99';
}

export class MGalladeEx2 extends MGalladeEx {
  public set: string = 'ROS';
  public setNumber: string = '100';
  public fullName: string = 'M Gallade-EX (ROS 100)';
  public legacyFullName = 'M Gallade-EX ROS 100';
}

export class LatiosEx2 extends LatiosEx {
  public set: string = 'ROS';
  public setNumber: string = '101';
  public fullName: string = 'Latios-EX (ROS 101)';
  public legacyFullName = 'Latios-EX ROS 101';
}

export class MLatiosEx2 extends MLatiosEx {
  public set: string = 'ROS';
  public setNumber: string = '102';
  public fullName: string = 'M Latios-EX (ROS 102)';
  public legacyFullName = 'M Latios-EX ROS 102';
}

export class HydreigonEx2 extends HydreigonEx {
  public set: string = 'ROS';
  public setNumber: string = '103';
  public fullName: string = 'Hydreigon-EX (ROS 103)';
  public legacyFullName = 'Hydreigon-EX ROS 103';
}
