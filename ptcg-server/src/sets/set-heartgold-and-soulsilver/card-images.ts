import { Cleffa } from './cleffa';
import { Cyndaquil } from './cyndaquil';
import { Drowzee } from './drowzee';
import { Hypno } from './hypno';
import { Pichu } from './pichu';
import { PokemonCollector } from './pokemon-collector';
import { ProfessorOaksNewTheory } from './professor-oaks-new-theory';
import { Quilava } from './quilava';
import { Typhlosion } from './typhlosion';
import { Tyrogue } from './tyrogue';
import { Unown } from './unown';

export class CleffaArt extends Cleffa {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_017_R_EN.png';
}

export class CyndaquilArt extends Cyndaquil {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_061_R_EN.png';
}

export class DrowzeeArt extends Drowzee {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_062_R_EN.png';
}

export class HypnoArt extends Hypno {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_023_R_EN.png';
}

export class PichuArt extends Pichu {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_028_R_EN.png';
}

export class PokemonCollectorArt extends PokemonCollector {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_097_R_EN.png';
}

export class ProfessorOaksNewTheoryArt extends ProfessorOaksNewTheory {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_101_R_EN.png';
}

export class QuilavaArt extends Quilava {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_049_R_EN.png';
}

export class TyphlosionArt extends Typhlosion {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_110_R_EN.png';
}

export class TyrogueArt extends Tyrogue {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_033_R_EN.png';
}

export class UnownArt extends Unown {
  public cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/HS/HS_054_R_EN.png';
}