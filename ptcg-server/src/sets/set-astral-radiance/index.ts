import { Card } from '../../game/store/card/card';
import { Absol } from './absol';
import { Azelf } from './azelf';
import { Bastiodon } from './bastiodon';
import { BeedrillV } from './beedrill-v';
import { Bergmite } from './bergmite';
import { Bisharp } from './bisharp';
import { Bronzong } from './bronzong';
import { Bronzor } from './bronzor';
import { Chatot } from './chatot';
import { Combee } from './combee';
import { Cranidos } from './cranidos';
import { Cyndaquil } from './cyndaquil';
import { DarkraiV } from './darkrai-v';
import { DarkraiVSTAR } from './darkrai-vstar';
import { Dartrix } from './dartrix';
import { Diancie } from './diancie';
import { Drifblim } from './drifblim';
import { Drifloon } from './drifloon';
import { Eevee } from './eevee';
import { GalarianMrRimeV } from './galarian-mr-rime-v';
import { Gallade } from './gallade';
import { GarchompV } from './garchomp-v';
import { Glaceon } from './glaceon';
import { Glameow } from './glameow';
import { Golduck } from './golduck';
import { HeatranV } from './heatran-v';
import { HeatranVmax } from './heatran-vmax';
import { Heracross } from './heracross';
import { Hippopotas } from './hippopotas';
import { Hippowdon } from './hippowdon';
import { HisuianArcanine } from './hisuian-arcanine';
import { HisuianAvalugg } from './hisuian-avalugg';
import { HisuianBasculegion } from './hisuian-basculegion';
import { HisuianBasculin } from './hisuian-basculin';
import { HisuianBraviary } from './hisuian-braviary';
import { HisuianDecidueye } from './hisuian-decidueye';
import { HisuianDecidueyeV } from './hisuian-decidueye-v';
import { HisuianDecidueyeVSTAR } from './hisuian-decidueye-vstar';
import { HisuianElectrode } from './hisuian-electrode';
import { HisuianGrowlithe } from './hisuian-growlithe';
import { HisuianLilligant } from './hisuian-lilligant';
import { HisuianLilligantV } from './hisuian-lilligant-v';
import { HisuianLilligantVstar } from './hisuian-lilligant-vstar';
import { HisuianOverqwil } from './hisuian-overqwil';
import { HisuianOverqwil2 } from './hisuian-overqwil-2';
import { HisuianQwilfish } from './hisuian-qwilfish';
import { HisuianQwilfish2 } from './hisuian-qwilfish-2';
import { HisuianSamurott } from './hisuian-samurott';
import { HisuianSamurottV } from './hisuian-samurott-v';
import { HisuianSamurottVSTAR } from './hisuian-samurott-vstar';
import { HisuianSneasel } from './hisuian-sneasel';
import { HisuianSneasler } from './hisuian-sneasler';
import { HisuianSneaslerV } from './hisuian-sneasler-v';
import { HisuianTyphlosion } from './hisuian-typhlosion';
import { HisuianTyphlosionV } from './hisuian-typhlosion-v';
import { HisuianTyphlosionVstar } from './hisuian-typhlosion-vstar';
import { HisuianVoltorb } from './hisuian-voltorb';
import { Hoothoot } from './hoothoot';
import { JirachiV } from './jirachi-v';
import { Keldeo } from './keldeo';
import { Kirlia } from './kirlia';
import { Kleavor } from './kleavor';
import { Kleavor2 } from './kleavor-2';
import { KleavorV } from './kleavor-v';
import { KleavorVstar } from './kleavor-vstar';
import { Kricketot } from './kricketot';
import { Kricketune } from './kricketune';
import { Leafeon } from './leafeon';
import { LucarioV } from './lucario-v';
import { LuxrayV } from './luxray-v';
import { MachampV } from './machamp-v';
import { MachampVmax } from './machamp-vmax';
import { Magnemite } from './magnemite';
import { Magneton } from './magneton';
import { Magnezone } from './magnezone';
import { Mamoswine } from './mamoswine';
import { Mantine } from './mantine';
import { Mesprit } from './mesprit';
import { Mightyena } from './mightyena';
import { Miltank } from './miltank';
import { Misdreavus } from './misdreavus';
import { Mismagius } from './mismagius';
import { Nickit } from './nickit';
import { Noctowl } from './noctowl';
import { OranguruV } from './oranguru-v';
import { OriginFormeDialgaV } from './origin-forme-dialga-v';
import { OriginFormeDialgaVSTAR } from './origin-forme-dialga-vstar';
import { OriginFormePalkiaV } from './origin-forme-palkia-v';
import { OriginFormePalkiaVSTAR } from './origin-forme-palkia-vstar';
import { Oshawott } from './oshawott';
import { Pawniard } from './pawniard';
import { Petilil } from './petilil';
import { Piloswine } from './piloswine';
import { Ponyta } from './ponyta';
import { Poochyena } from './poochyena';
import { Psyduck } from './psyduck';
import { Purugly } from './purugly';
import { Quilava } from './quilava';
import { RadiantGreninja } from './radiant-greninja';
import { RadiantHawlucha } from './radiant-hawlucha';
import { RadiantHeatran } from './radiant-heatran';
import { Ralts } from './ralts';
import { Rampardos } from './rampardos';
import { Rapidash } from './rapidash';
import { Regice } from './regice';
import { Regidrago } from './regidrago';
import { Regieleki } from './regieleki';
import { Regigigas } from './regigigas';
import { Regirock } from './regirock';
import { Registeel } from './registeel';
import { Rowlet } from './rowlet';
import { Rufflet } from './rufflet';
import { Scyther } from './scyther';
import { Scyther2 } from './scyther-2';
import { Shaymin } from './shaymin';
import { Shieldon } from './shieldon';
import { Stantler } from './stantler';
import { StarmieV } from './starmie-v';
import { Sudowoodo } from './sudowoodo';
import { Swinub } from './swinub';
import { Teddiursa } from './teddiursa';
import { Thievul } from './thievul';
import { Togekiss } from './togekiss';
import { Togepi } from './togepi';
import { Togetic } from './togetic';
import { Ursaluna } from './ursaluna';
import { Ursaring } from './ursaring';
import { Uxie } from './uxie';
import { Vespiquen } from './vespiquen';
import { VirizionV } from './virizion-v';
import { Whiscash } from './whiscash';
import { Wyrdeer } from './wyrdeer';
import { WyrdeerV } from './wyrdeer-v';
import { Yanma } from './yanma';
import { Yanmega } from './yanmega';
import { Adaman } from './adaman';
import { CancelingCologne } from './canceling-cologne';
import { Choy } from './choy';
import { Cyllene } from './cyllene';
import { DarkPatch } from './dark-patch';
import { EnergyLoto } from './energy-loto';
import { FeatherBall } from './feather-ball';
import { GapejawBog } from './gapejaw-bog';
import { GardeniasVigor } from './gardenias-vigor';
import { Grant } from './grant';
import { GutsyPickaxe } from './gutsy-pickaxe';
import { HisuianHeavyBall } from './hisuian-heavy-ball';
import { Irida } from './irida';
import { JubilifeVillage } from './jubilife-village';
import { Kamado } from './kamado';
import { Roxanne } from './roxanne';
import { SpicySeasonedCurry } from './spicy-seasoned-curry';
import { SupereffectiveGlasses } from './supereffective-glasses';
import { SweetHoney } from './sweet-honey';
import { SwitchCart } from './switch-cart';
import { TempleofSinnoh } from './temple-of-sinnoh';
import { TrekkingShoes } from './trekking-shoes';
import { WaitAndSeeTurbo } from './wait-and-see-turbo';
import { Zisu } from './zisu';

import {
  BronzongASR,
  UnidentifiedFossilASR,
  StarmieV2ASR,
  OriginFormePalkiaV2ASR,
  LuxrayV2ASR,
  HisuianDecidueyeV2ASR,
  HisuianSamurottV2ASR,
  OriginFormeDialgaV2ASR,
  GarchompV2ASR,
  OranguruV2ASR,
  WyrdeerV2ASR,
  Adaman2ASR,
  Cyllene2ASR,
  GardeniasVigor2ASR,
  Grant2ASR,
  Irida2ASR,
  Roxanne2ASR,
  OriginFormePalkiaVSTAR2ASR,
  HisuianDecidueyeVSTAR2ASR,
  HisuianSamurottVSTAR2ASR,
  OriginFormeDialgaVSTAR2ASR,
  Adaman3ASR,
  Cyllene3ASR,
  GardeniasVigor3ASR,
  Grant3ASR,
  Irida3ASR,
  Roxanne3ASR,
  OriginFormePalkiaVSTAR3ASR,
  HisuianSamurottVSTAR3ASR,
  OriginFormeDialgaVSTAR3ASR,
  ChoiceBeltASR,
  JubilifeVillage2ASR,
  PathToThePeakASR,
  TempleofSinnoh2ASR,
  TrekkingShoes2ASR,
  DoubleTurboEnergyASR,
  FlappleASR,
  FrosmothASR,
  GardevoirASR,
  FalinksASR,
  Mightyena2ASR,
  StarmieV3ASR,
  IceRiderCalyrexVASR,
  IceRiderCalyrexVMAXASR,
  GalarianArticunoVASR,
  ShadowRiderCalyrexVASR,
  ShadowRiderCalyrexVMAXASR,
  GalarianZapdosVASR,
  GalarianMoltresVASR,
  ZacianVASR,
  GarchompV3ASR,
  MelonyASR,
  PiersASR,
  IceRiderCalyrexVMAX2ASR,
  ShadowRiderCalyrexVMAX2ASR,
  BarboachASR,
  DewottASR,
  BeedrillV2,
  BeedrillV3,
  HisuianLilligantV2,
  HisuianLilligantV3,
  HeatranV2,
  HisuianTyphlosionV2,
  MachampV2,
  MachampV3,
  HisuianSneaslerV2,
  HisuianSneaslerV3,
  Choy2,
  Kamado2,
  Zisu2,
  HisuianLilligantVstar2,
  HeatranVmax2,
  HisuianTyphlosionVstar2,
  MachampVmax2,
  Choy3,
  Kamado3,
  Zisu3,
} from './other-prints';

export const setAstralRadiance: Card[] = [
  // Pokemon
  new Absol(),
  new Azelf(),
  new Bastiodon(),
  new BeedrillV(),
  new Bergmite(),
  new Bisharp(),
  new Bronzong(),
  new Bronzor(),
  new Chatot(),
  new Combee(),
  new Cranidos(),
  new Cyndaquil(),
  new DarkraiV(),
  new DarkraiVSTAR(),
  new Dartrix(),
  new Diancie(),
  new Drifblim(),
  new Drifloon(),
  new Eevee(),
  new GalarianMrRimeV(),
  new Gallade(),
  new GarchompV(),
  new Glaceon(),
  new Glameow(),
  new Golduck(),
  new HeatranV(),
  new HeatranVmax(),
  new Heracross(),
  new Hippopotas(),
  new Hippowdon(),
  new HisuianArcanine(),
  new HisuianAvalugg(),
  new HisuianBasculegion(),
  new HisuianBasculin(),
  new HisuianBraviary(),
  new HisuianDecidueye(),
  new HisuianDecidueyeV(),
  new HisuianDecidueyeVSTAR(),
  new HisuianElectrode(),
  new HisuianGrowlithe(),
  new HisuianLilligant(),
  new HisuianLilligantV(),
  new HisuianLilligantVstar(),
  new HisuianOverqwil(),
  new HisuianOverqwil2(),
  new HisuianQwilfish(),
  new HisuianQwilfish2(),
  new HisuianSamurott(),
  new HisuianSamurottV(),
  new HisuianSamurottVSTAR(),
  new HisuianSneasel(),
  new HisuianSneasler(),
  new HisuianSneaslerV(),
  new HisuianTyphlosion(),
  new HisuianTyphlosionV(),
  new HisuianTyphlosionVstar(),
  new HisuianVoltorb(),
  new Hoothoot(),
  new JirachiV(),
  new Keldeo(),
  new Kirlia(),
  new Kleavor(),
  new Kleavor2(),
  new KleavorV(),
  new KleavorVstar(),
  new Kricketot(),
  new Kricketune(),
  new Leafeon(),
  new LucarioV(),
  new LuxrayV(),
  new MachampV(),
  new MachampVmax(),
  new Magnemite(),
  new Magneton(),
  new Magnezone(),
  new Mamoswine(),
  new Mantine(),
  new Mesprit(),
  new Mightyena(),
  new Miltank(),
  new Misdreavus(),
  new Mismagius(),
  new Nickit(),
  new Noctowl(),
  new OranguruV(),
  new OriginFormeDialgaV(),
  new OriginFormeDialgaVSTAR(),
  new OriginFormePalkiaV(),
  new OriginFormePalkiaVSTAR(),
  new Oshawott(),
  new Pawniard(),
  new Petilil(),
  new Piloswine(),
  new Ponyta(),
  new Poochyena(),
  new Psyduck(),
  new Purugly(),
  new Quilava(),
  new RadiantGreninja(),
  new RadiantHawlucha(),
  new RadiantHeatran(),
  new Ralts(),
  new Rampardos(),
  new Rapidash(),
  new Regice(),
  new Regidrago(),
  new Regieleki(),
  new Regigigas(),
  new Regirock(),
  new Registeel(),
  new Rowlet(),
  new Rufflet(),
  new Scyther(),
  new Scyther2(),
  new Shaymin(),
  new Shieldon(),
  new Stantler(),
  new StarmieV(),
  new Sudowoodo(),
  new Swinub(),
  new Teddiursa(),
  new Thievul(),
  new Togekiss(),
  new Togepi(),
  new Togetic(),
  new Ursaluna(),
  new Ursaring(),
  new Uxie(),
  new Vespiquen(),
  new VirizionV(),
  new Whiscash(),
  new Wyrdeer(),
  new WyrdeerV(),
  new Yanma(),
  new Yanmega(),

  // Trainers
  new Adaman(),
  new CancelingCologne(),
  new Choy(),
  new Cyllene(),
  new DarkPatch(),
  new EnergyLoto(),
  new FeatherBall(),
  new GapejawBog(),
  new GardeniasVigor(),
  new Grant(),
  new GutsyPickaxe(),
  new HisuianHeavyBall(),
  new Irida(),
  new JubilifeVillage(),
  new Kamado(),
  new Roxanne(),
  new SpicySeasonedCurry(),
  new SupereffectiveGlasses(),
  new SweetHoney(),
  new SwitchCart(),
  new TempleofSinnoh(),
  new TrekkingShoes(),
  new WaitAndSeeTurbo(),
  new Zisu(),

  // Other Prints (Reprints & Alt Arts)
  new BronzongASR(),
  new UnidentifiedFossilASR(),
  new StarmieV2ASR(),
  new OriginFormePalkiaV2ASR(),
  new LuxrayV2ASR(),
  new HisuianDecidueyeV2ASR(),
  new HisuianSamurottV2ASR(),
  new OriginFormeDialgaV2ASR(),
  new GarchompV2ASR(),
  new OranguruV2ASR(),
  new WyrdeerV2ASR(),
  new Adaman2ASR(),
  new Cyllene2ASR(),
  new GardeniasVigor2ASR(),
  new Grant2ASR(),
  new Irida2ASR(),
  new Roxanne2ASR(),
  new OriginFormePalkiaVSTAR2ASR(),
  new HisuianDecidueyeVSTAR2ASR(),
  new HisuianSamurottVSTAR2ASR(),
  new OriginFormeDialgaVSTAR2ASR(),
  new Adaman3ASR(),
  new Cyllene3ASR(),
  new GardeniasVigor3ASR(),
  new Grant3ASR(),
  new Irida3ASR(),
  new Roxanne3ASR(),
  new OriginFormePalkiaVSTAR3ASR(),
  new HisuianSamurottVSTAR3ASR(),
  new OriginFormeDialgaVSTAR3ASR(),
  new ChoiceBeltASR(),
  new JubilifeVillage2ASR(),
  new PathToThePeakASR(),
  new TempleofSinnoh2ASR(),
  new TrekkingShoes2ASR(),
  new DoubleTurboEnergyASR(),
  new FlappleASR(),
  new FrosmothASR(),
  new GardevoirASR(),
  new FalinksASR(),
  new Mightyena2ASR(),
  new StarmieV3ASR(),
  new IceRiderCalyrexVASR(),
  new IceRiderCalyrexVMAXASR(),
  new GalarianArticunoVASR(),
  new ShadowRiderCalyrexVASR(),
  new ShadowRiderCalyrexVMAXASR(),
  new GalarianZapdosVASR(),
  new GalarianMoltresVASR(),
  new ZacianVASR(),
  new GarchompV3ASR(),
  new MelonyASR(),
  new PiersASR(),
  new IceRiderCalyrexVMAX2ASR(),
  new ShadowRiderCalyrexVMAX2ASR(),
  new BarboachASR(),
  new DewottASR(),
  new BeedrillV2(),
  new BeedrillV3(),
  new HisuianLilligantV2(),
  new HisuianLilligantV3(),
  new HeatranV2(),
  new HisuianTyphlosionV2(),
  new MachampV2(),
  new MachampV3(),
  new HisuianSneaslerV2(),
  new HisuianSneaslerV3(),
  new Choy2(),
  new Kamado2(),
  new Zisu2(),
  new HisuianLilligantVstar2(),
  new HeatranVmax2(),
  new HisuianTyphlosionVstar2(),
  new MachampVmax2(),
  new Choy3(),
  new Kamado3(),
  new Zisu3(),
];
