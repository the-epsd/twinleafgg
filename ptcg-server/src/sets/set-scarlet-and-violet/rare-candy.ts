...
import { TrainerType, Stage, SuperType, Format } from '../../game/store/card/card-types';
...
function* playCardModern(next: Function, store: StoreLike, state: State, effect: TrainerEffect): IterableIterator<State> {
...
// This is different enough that it'd probably be a mess to separate within the function itself
...
export class RareCandy extends TrainerCard {
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof TrainerEffect && effect.trainerCard === this) {
      ...
      let generator = playCardModern(() => generator.next(), store, state, effect);
      if (isRspk) {
        generator = playCardRspk(() => generator.next(), store, state, effect);
      }
