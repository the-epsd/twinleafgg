import { GothoritaEPO, GothitelleEPO, GreatBallEPO, Tornadus2EPO } from './other-prints';
import { Card } from '../../../game/store/card/card';
import { Bianca } from './bianca';
import { CrushingHammerEPO, RecycleEPO } from './other-prints';
import { Cheren } from './cheren';
import { MaxPotion } from './max-potion';
import { Tornadus } from './tornadus';
import { PokemonCatcher } from './pokemon-catcher';
import { Pansage } from './pansage';
import { Sewaddle } from './sewaddle';
import { Sewaddle2 } from './sewaddle-2';
import { Darumaka } from './darumaka';
import { Sandile } from './sandile';
import { Pidove } from './pidove';
import { Rufflet } from './rufflet';
import { Rufflet2 } from './rufflet-2';
import { Joltik } from './joltik';
import { Cubchoo2 } from './cubchoo-2';
import { Ferroseed2 } from './ferroseed-2';
import { Drilbur2 } from './drilbur-2';
import { Pansear } from './pansear';
import { Panpour } from './panpour';
import { Petilil } from './petilil';
import { Emolga } from './emolga';
import { Woobat } from './woobat';
import { Galvantula } from './galvantula';
import { Venipede } from './venipede';
import { Zorua } from './zorua';
import { Deerling } from './deerling';
import { Cottonee } from './cottonee';
import { Gothita } from './gothita';
import { Gothita2 } from './gothita-2';
import { Klink } from './klink';
import { Patrat } from './patrat';
import { Minccino } from './minccino';
import { Tranquill } from './tranquill';
import { Braviary } from './braviary';
import { Roggenrola } from './roggenrola';
import { Sawsbuck } from './sawsbuck';
import { Swadloon } from './swadloon';
import { Swadloon2 } from './swadloon-2';
import { Krokorok } from './krokorok';
import { Zoroark } from './zoroark';
import { Simisage } from './simisage';
import { Simisear } from './simisear';
import { Simipour } from './simipour';
import { Darmanitan } from './darmanitan';
import { Ferroseed } from './ferroseed';
import { Klang } from './klang';
import { Beartic } from './beartic';
import { Beartic2 } from './beartic-2';
import { Virizion } from './virizion';
import { Terrakion } from './terrakion';
import { Cobalion } from './cobalion';
import { Thundurus } from './thundurus';
import { Thundurus2 } from './thundurus-2';
import { Swanna } from './swanna';
import { Ducklett } from './ducklett';
import { Cubchoo } from './cubchoo';
import { Swoobat } from './swoobat';
import { Lilligant } from './lilligant';
import { Whirlipede } from './whirlipede';
import { Scolipede } from './scolipede';
import { Basculin } from './basculin';
import { Basculin2 } from './basculin-2';
import { Sigilyph } from './sigilyph';
import { Sigilyph2 } from './sigilyph-2';
import { Roggenrola2 } from './roggenrola-2';
import { Boldore } from './boldore';
import { Boldore2 } from './boldore-2';
import { Drilbur } from './drilbur';
import { Excadrill } from './excadrill';
import { Excadrill2 } from './excadrill-2';
import { Gigalith } from './gigalith';
import { Throh } from './throh';
import { Sawk } from './sawk';
import { Purrloin } from './purrloin';
import { Liepard } from './liepard';
import { Vullaby } from './vullaby';
import { Mandibuzz } from './mandibuzz';
import { Ferrothorn } from './ferrothorn';
import { Ferrothorn2 } from './ferrothorn-2';
import { Klinklang } from './klinklang';
import { Cottonee2 } from './cottonee-2';
import { Whimsicott } from './whimsicott';
import { Whimsicott2 } from './whimsicott-2';
import { Leavanny } from './leavanny';
import { Leavanny2 } from './leavanny-2';
import { Gothorita2 } from './gothorita-2';
import { Gothitelle2 } from './gothitelle-2';
import { Watchog } from './watchog';
import { Unfezant } from './unfezant';
import { Audino } from './audino';
import { Cinccino } from './cinccino';
import { Krookodile } from './krookodile';

export const setEmergingPowers: Card[] = [
  new Bianca(),
  new Cheren(),
  new CrushingHammerEPO(),
  new MaxPotion(),
  new RecycleEPO(),
  new PokemonCatcher(),
  new Tornadus(),
  new GothoritaEPO(),
  new GothitelleEPO(),
  new GreatBallEPO(),
  new Tornadus2EPO(),
  new Pansage(),
  new Sewaddle(),
  new Sewaddle2(),
  new Darumaka(),
  new Sandile(),
  new Pidove(),
  new Rufflet(),
  new Rufflet2(),
  new Joltik(),
  new Cubchoo2(),
  new Ferroseed2(),
  new Drilbur2(),
  new Pansear(),
  new Panpour(),
  new Petilil(),
  new Emolga(),
  new Woobat(),
  new Galvantula(),
  new Venipede(),
  new Zorua(),
  new Deerling(),
  new Cottonee(),
  new Gothita(),
  new Gothita2(),
  new Klink(),
  new Patrat(),
  new Minccino(),
  new Tranquill(),
  new Braviary(),
  new Roggenrola(),
  new Sawsbuck(),
  new Swadloon(),
  new Swadloon2(),
  new Krokorok(),
  new Zoroark(),
  new Simisage(),
  new Simisear(),
  new Simipour(),
  new Darmanitan(),
  new Ferroseed(),
  new Klang(),
  new Beartic(),
  new Beartic2(),
  new Virizion(),
  new Terrakion(),
  new Cobalion(),
  new Thundurus(),
  new Thundurus2(),
  new Swanna(),
  new Ducklett(),
  new Cubchoo(),
  new Swoobat(),
  new Lilligant(),
  new Whirlipede(),
  new Scolipede(),
  new Basculin(),
  new Basculin2(),
  new Sigilyph(),
  new Sigilyph2(),
  new Roggenrola2(),
  new Boldore(),
  new Boldore2(),
  new Drilbur(),
  new Excadrill(),
  new Excadrill2(),
  new Gigalith(),
  new Throh(),
  new Sawk(),
  new Purrloin(),
  new Liepard(),
  new Vullaby(),
  new Mandibuzz(),
  new Ferrothorn(),
  new Ferrothorn2(),
  new Klinklang(),
  new Cottonee2(),
  new Whimsicott(),
  new Whimsicott2(),
  new Leavanny(),
  new Leavanny2(),
  new Gothorita2(),
  new Gothitelle2(),
  new Watchog(),
  new Unfezant(),
  new Audino(),
  new Cinccino(),
  new Krookodile(),
];
