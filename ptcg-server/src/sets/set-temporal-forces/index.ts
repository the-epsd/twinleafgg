import { Card } from '../../game/store/card/card';
import { AncientBoosterEnergyCapsuleArt, BeldumArt, BiancasDevotionArt, BrambleghastArt, BuddyBuddyPoffinArt, CharjabugArt, ChatotArt, CinccinoArt, CroconawArt, CryptomaniacsDecipheringArt, DeliveryBoxArt, DrilburArt, DrumsOfAwakeningArt, DudunsparceArt, DunsparceArt, DuosionArt, EmergencyBoardArt, EriArt, ExplorersGuidanceArt, FeraligatrArt, FlutterManeArt, FullMetalLabArt, FutureBoosterEnergyCapsuleArt, GastlyArt, GengarexArt, GougingFireexArt, GreatTuskArt, GrotleArt, GrubbinArt, HandTrimmerArt, HaunterArt, HeavyBatonArt, HerosCapeArt, IronCrownexArt, IronHandsArt, IronJugulisArt, IronLeavesexArt, IronThornsArt, KoraidonArt, KoraidonexArt, MasterBallArt, MaximumBeltArt, MetagrossArt, MetangArt, MinccinoArt, MiraidonArt, MiraidonexArt, MistEnergyArt, MortysConvictionArt, MrMimeArt, NeoUpperEnergyArt, PalafinArt, PerilousJungleArt, PikachuArt, PrimeCatcherArt, RabscaArt, RagingBoltexArt, RaichuArt, RebootPodArt, RellorArt, ReuniclusArt, RoaringMoonArt, SableyeArt, SalvatoreArt, SandyShocksArt, SkittyArt, SolosisArt, TorterraexArt, TotodileArt, TurtwigArt, VictiniArt, VikavoltArt, WalkingWakeexArt } from './card-images';
import { GrotleIR, CharjabugIR, ReuniclusIR, GastlyIR, MetagrossIR, ChatotIR, MinccinoIR, CinccinoIR, TorterraexFA, IronLeavesexFA, GougingFireexFA, WalkingWakeexFA, IronCrownexFA, GengarexFA, RagingBoltexFA, BiancasDevotionFA, EriFA, ExplorersGuidanceFA, MortysConvictionFA, SalvatoreFA, IronLeavesexSIR, GougingFireexSIR, WalkingWakeexSIR, IronCrownexSIR, RagingBoltexSIR, BiancasDevotionSIR, EriSIR, MortysConvictionSIR, SalvatoreSIR, IronLeavesexHR, GougingFireexHR, WalkingWakeexHR, IronCrownexHR, RagingBoltexHR, CryptomaniacsDecipheringFA } from './full-art';

export const setTemporalForces: Card[] = [

  new AncientBoosterEnergyCapsuleArt(),
  new BeldumArt(),
  new BiancasDevotionArt(),
  new BrambleghastArt(),
  new BuddyBuddyPoffinArt(),
  new CharjabugArt(),
  new ChatotArt(),
  new CinccinoArt(),
  new CroconawArt(),
  new CryptomaniacsDecipheringArt(),
  new DeliveryBoxArt(),
  new DrilburArt(),
  new DrumsOfAwakeningArt(),
  new DudunsparceArt(),
  new DunsparceArt(),
  new DuosionArt(),
  new EmergencyBoardArt(),
  new EriArt(),
  new ExplorersGuidanceArt(),
  new FeraligatrArt(),
  new FlutterManeArt(),
  new FullMetalLabArt(),
  new FutureBoosterEnergyCapsuleArt(),
  new GastlyArt(),
  new GengarexArt(),
  new GougingFireexArt(),
  new GreatTuskArt(),
  new GrotleArt(),
  new GrubbinArt(),
  new HandTrimmerArt(),
  new HaunterArt(),
  new HeavyBatonArt(),
  new HerosCapeArt(),
  new IronCrownexArt(),
  new IronHandsArt(),
  new IronJugulisArt(),
  new IronLeavesexArt(),
  new IronThornsArt(),
  new KoraidonexArt(),
  new KoraidonArt(),
  new MasterBallArt(),
  new MaximumBeltArt(),
  new MetagrossArt(),
  new MetangArt(),
  new MinccinoArt(),
  new MiraidonexArt(),
  new MiraidonArt(),
  new MistEnergyArt(),
  new MortysConvictionArt(),
  new MrMimeArt(),
  new NeoUpperEnergyArt(),
  new PalafinArt(),
  new PerilousJungleArt(),
  new PikachuArt(),
  new PrimeCatcherArt(),
  new RabscaArt(),
  new RagingBoltexArt(),
  new RaichuArt(),
  new RebootPodArt(),
  new RellorArt(),
  new ReuniclusArt(),
  new RoaringMoonArt(),
  new SableyeArt(),
  new SalvatoreArt(),
  new SandyShocksArt(),
  new SkittyArt(),
  new SolosisArt(),
  new TorterraexArt(),
  new TotodileArt(),
  new TurtwigArt(),
  new VictiniArt(),
  new VikavoltArt(),
  new WalkingWakeexArt(),

  //FA
  new GrotleIR(),
  new CharjabugIR(),
  new ReuniclusIR(),
  new GastlyIR(),
  new MetagrossIR(),
  new ChatotIR(),
  new MinccinoIR(),
  new CinccinoIR(),
  new TorterraexFA(),
  new IronLeavesexFA(),
  new GougingFireexFA(),
  new WalkingWakeexFA(),
  new IronCrownexFA(),
  new GengarexFA(),
  new RagingBoltexFA(),
  new BiancasDevotionFA(),
  new CryptomaniacsDecipheringFA(),
  new EriFA(),
  new ExplorersGuidanceFA(),
  new MortysConvictionFA(),
  new SalvatoreFA(),
  new IronLeavesexSIR(),
  new GougingFireexSIR(),
  new WalkingWakeexSIR(),
  new IronCrownexSIR(),
  new RagingBoltexSIR(),
  new BiancasDevotionSIR(),
  new EriSIR(),
  new MortysConvictionSIR(),
  new SalvatoreSIR(),
  new IronLeavesexHR(),
  new GougingFireexHR(),
  new WalkingWakeexHR(),
  new IronCrownexHR(),
  new RagingBoltexHR(),

];