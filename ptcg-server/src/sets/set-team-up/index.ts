import { Card } from '../../game/store/card/card';
import { Absol } from './absol';
import { Aegislash } from './aegislash';
import { Aerodactyl } from './aerodactyl';
import { AlolanExeggutor2 } from './alolan-exeggutor-2';
import { AlolanGeodude } from './alolan-geodude';
import { AlolanGeodude2 } from './alolan-geodude-2';
import { AlolanGolem } from './alolan-golem';
import { AlolanGraveler } from './alolan-graveler';
import { AlolanGrimer } from './alolan-grimer';
import { AlolanMuk } from './alolan-muk';
import { AlolanNinetales } from './alolan-ninetales';
import { AmpharosGx } from './ampharos-gx';
import { Articuno } from './articuno';
import { Beedrill } from './beedrill';
import { Bisharp } from './bisharp';
import { Blastoise } from './blastoise';
import { Blitzle } from './blitzle';
import { Bronzong } from './bronzong';
import { Bronzor } from './bronzor';
import { CelebiVenusaurGX } from './celebi-and-venusaur-gx';
import { Charizard } from './charizard';
import { Charmander } from './charmander';
import { Charmander2 } from './charmander-2';
import { Charmeleon } from './charmeleon';
import { CobalionGX } from './cobalion-gx';
import { Cosmoem } from './cosmoem';
import { Cosmog } from './cosmog';
import { Doublade } from './doublade';
import { Dragonair } from './dragonair';
import { Dragonite } from './dragonite';
import { Dratini } from './dratini';
import { Dratini2 } from './dratini-2';
import { EeveeSnorlaxGX } from './eevee-and-snorlax-gx';
import { Electrode } from './electrode';
import { Emolga } from './emolga';
import { Exeggcute } from './exeggcute';
import { Farfetchd } from './farfetchd';
import { Ferroseed } from './ferroseed';
import { Ferrothorn } from './ferrothorn';
import { Flaaffy } from './flaaffy';
import { Galvantula } from './galvantula';
import { GengarMimikyuGX } from './gengar-and-mimikyu-gx';
import { Golduck } from './golduck';
import { Grimer } from './grimer';
import { Gyarados } from './gyarados';
import { Heliolisk } from './heliolisk';
import { Helioptile } from './helioptile';
import { Hitmonchan } from './hitmonchan';
import { Hitmonlee } from './hitmonlee';
import { Honedge } from './honedge';
import { HoopaGx } from './hoopa-gx';
import { IncineroarGx } from './incineroar-gx';
import { Jirachi } from './jirachi';
import { Joltik } from './joltik';
import { Jynx } from './jynx';
import { Kabuto } from './kabuto';
import { Kabutops } from './kabutops';
import { Kakuna } from './kakuna';
import { Kangaskhan } from './kangaskhan';
import { Klefki } from './klefki';
import { Lapras } from './lapras';
import { Larvitar } from './larvitar';
import { LatiasAndLatiosGx } from './latias-and-latios-gx';
import { Litten } from './litten';
import { Lugia } from './lugia';
import { LycanrocGx } from './lycanroc-gx';
import { Magikarp } from './magikarp';
import { MagikarpWailordGX } from './magikarp-and-wailord-gx';
import { Mandibuzz } from './mandibuzz';
import { Mankey } from './mankey';
import { Mareep } from './mareep';
import { Meowth } from './meowth';
import { Mightyena } from './mightyena';
import { Mimikyu } from './mimikyu';
import { Moltres } from './moltres';
import { MrMime } from './mr-mime';
import { MrMimeGX } from './mr-mime-gx';
import { Muk } from './muk';
import { Nidoking } from './nidoking';
import { Nidoqueen } from './nidoqueen';
import { NidoranF } from './nidoran-f';
import { NidoranFemale } from './nidoran-female';
import { NidoranM } from './nidoran-m';
import { Nidorina } from './nidorina';
import { Nidorino } from './nidorino';
import { Ninetales } from './ninetales';
import { Omanyte } from './omanyte';
import { Omastar } from './omastar';
import { Pancham } from './pancham';
import { Pangoro } from './pangoro';
import { Paras } from './paras';
import { Parasect } from './parasect';
import { Pawniard } from './pawniard';
import { Persian } from './persian';
import { Pidgeot } from './pidgeot';
import { Pidgeotto } from './pidgeotto';
import { Pidgey } from './pidgey';
import { Pidgey2 } from './pidgey-2';
import { Pinsir } from './pinsir';
import { Ponyta } from './ponyta';
import { Poochyena } from './poochyena';
import { Primeape } from './primeape';
import { Psyduck } from './psyduck';
import { Pupitar } from './pupitar';
import { Rapidash } from './rapidash';
import { Shaymin } from './shaymin';
import { ShayminPrismStar } from './shaymin-prism-star';
import { Skarmory } from './skarmory';
import { Spiritomb } from './spiritomb';
import { Squirtle } from './squirtle';
import { Squirtle2 } from './squirtle-2';
import { Starmie } from './starmie';
import { Staryu } from './staryu';
import { TapuKoko } from './tapu-koko';
import { TapuKokoPrismStar } from './tapu-koko-prism-star';
import { Tauros } from './tauros';
import { Tentacool } from './tentacool';
import { Tentacruel } from './tentacruel';
import { Torracat } from './torracat';
import { Tyranitar } from './tyranitar';
import { Voltorb } from './voltorb';
import { Vullaby } from './vullaby';
import { Vulpix } from './vulpix';
import { Wartortle } from './wartortle';
import { Weedle } from './weedle';
import { Weedle2 } from './weedle-2';
import { Yveltal } from './yveltal';
import { Zangoose } from './zangoose';
import { Zapdos } from './zapdos';
import { Zebstrika } from './zebstrika';
import { Zeraora } from './zeraora';
import { Zoroark } from './zoroark';
import { Zorua } from './zorua';
import { BillsAnalysis } from './bills-analysis';
import { BlackMarket } from './black-market';
import { BlackMarketPrismStar } from './black-market-prism-star';
import { BuffPadding } from './buff-padding';
import { Dana } from './dana';
import { DangerousDrill } from './dangerous-drill';
import { Electrocharger } from './electrocharger';
import { Evelyn } from './evelyn';
import { FairyCharmUb } from './fairy-charm-ub';
import { GrassMemory } from './grass-memory';
import { IngoAndEmmet } from './ingo-and-emmet';
import { Jasmine } from './jasmine';
import { JudgeWhistle } from './judge-whistle';
import { LavenderTown } from './lavender-town';
import { MetalGoggles } from './metal-goggles';
import { Morgan } from './morgan';
import { Nanu } from './nanu';
import { Nita } from './nita';
import { PokemonCommunication } from './pokemon-communication';
import { ReturnLabel } from './return-label';
import { SabrinasSuggestion } from './sabrinas-suggestion';
import { ViridianForest } from './viridian-forest';
import { WaterMemory } from './water-memory';
import { WondrousLabyrinth } from './wondrous-labyrinth';
import { WondrousLabyrinthPrismStar } from './wondrous-labyrinth-prism-star';

import {
  BrocksGritTEU,
  PikachuZekromGXTEU,
  ErikasHospitalityTEU,
  UnidentifiedFossilTEU,
  CelebiVenusaurGX2TEU,
  MagikarpWailordGX2TEU,
  PikachuZekromGX2TEU,
  GengarMimikyuGX2TEU,
  GengarMimikyuGX3TEU,
  CobalionGX2TEU,
  EeveeSnorlaxGX2TEU,
  BrocksGrit2TEU,
  ErikasHospitality2TEU,
  IngoAndEmmet2TEU,
  Jasmine2TEU,
  CelebiVenusaurGX3TEU,
  MagikarpWailordGX3TEU,
  PikachuZekromGX3TEU,
  GengarMimikyuGX4TEU,
  CobalionGX3TEU,
  EeveeSnorlaxGX3TEU,
  DangerousDrill2TEU,
  JudgeWhistle2TEU,
  MetalGoggles2TEU,
  PokemonCommunicationHS4TEU,
  PokemonCommunicationHS2TEU,
  PokemonCommunicationHS3TEU,
  GrassEnergyTEU,
  FireEnergyTEU,
  WaterEnergyTEU,
  LightningEnergyTEU,
  PsychicEnergyTEU,
  FightingEnergyTEU,
  DarknessEnergyTEU,
  MetalEnergyTEU,
  FairyEnergyTEU,
  AlolanMarowakTEU,
  AlolanExeggutorTEU,
  AmpharosGx2,
  HoopaGx2,
  IncineroarGx2,
  LatiasAndLatiosGx2,
  LatiasAndLatiosGx3,
  Dana2,
  Evelyn2,
  Morgan2,
  Nanu2,
  Nita2,
  SabrinasSuggestion2,
  AmpharosGx3,
  HoopaGx3,
  IncineroarGx3,
  LatiasAndLatiosGx4,
  Electrocharger2,
} from './other-prints';

export const setTeamUp: Card[] = [
  // Pokemon
  new Absol(),
  new Aegislash(),
  new Aerodactyl(),
  new AlolanExeggutor2(),
  new AlolanGeodude(),
  new AlolanGeodude2(),
  new AlolanGolem(),
  new AlolanGraveler(),
  new AlolanGrimer(),
  new AlolanMuk(),
  new AlolanNinetales(),
  new AmpharosGx(),
  new Articuno(),
  new Beedrill(),
  new Bisharp(),
  new Blastoise(),
  new Blitzle(),
  new Bronzong(),
  new Bronzor(),
  new CelebiVenusaurGX(),
  new Charizard(),
  new Charmander(),
  new Charmander2(),
  new Charmeleon(),
  new CobalionGX(),
  new Cosmoem(),
  new Cosmog(),
  new Doublade(),
  new Dragonair(),
  new Dragonite(),
  new Dratini(),
  new Dratini2(),
  new EeveeSnorlaxGX(),
  new Electrode(),
  new Emolga(),
  new Exeggcute(),
  new Farfetchd(),
  new Ferroseed(),
  new Ferrothorn(),
  new Flaaffy(),
  new Galvantula(),
  new GengarMimikyuGX(),
  new Galvantula(),
  new Golduck(),
  new Grimer(),
  new Gyarados(),
  new Heliolisk(),
  new Helioptile(),
  new Hitmonchan(),
  new Hitmonlee(),
  new Honedge(),
  new HoopaGx(),
  new IncineroarGx(),
  new Jirachi(),
  new Joltik(),
  new Jynx(),
  new Kabuto(),
  new Kabutops(),
  new Kakuna(),
  new Kangaskhan(),
  new Klefki(),
  new Lapras(),
  new Larvitar(),
  new LatiasAndLatiosGx(),
  new Litten(),
  new Lugia(),
  new LycanrocGx(),
  new Magikarp(),
  new MagikarpWailordGX(),
  new Mandibuzz(),
  new Mankey(),
  new Mareep(),
  new Meowth(),
  new Mightyena(),
  new Mimikyu(),
  new Moltres(),
  new MrMime(),
  new MrMimeGX(),
  new Muk(),
  new Nidoking(),
  new Nidoqueen(),
  new NidoranF(),
  new NidoranFemale(),
  new NidoranM(),
  new Nidorina(),
  new Nidorino(),
  new Ninetales(),
  new Omanyte(),
  new Omastar(),
  new Pancham(),
  new Pangoro(),
  new Paras(),
  new Parasect(),
  new Pawniard(),
  new Persian(),
  new Pidgeot(),
  new Pidgeotto(),
  new Pidgey(),
  new Pidgey2(),
  new Pinsir(),
  new Ponyta(),
  new Poochyena(),
  new Primeape(),
  new Psyduck(),
  new Pupitar(),
  new Rapidash(),
  new Shaymin(),
  new ShayminPrismStar(),
  new Skarmory(),
  new Spiritomb(),
  new Squirtle(),
  new Squirtle2(),
  new Starmie(),
  new Staryu(),
  new TapuKoko(),
  new TapuKokoPrismStar(),
  new Tauros(),
  new Tentacool(),
  new Tentacruel(),
  new Torracat(),
  new Tyranitar(),
  new Voltorb(),
  new Vullaby(),
  new Vulpix(),
  new Wartortle(),
  new Weedle(),
  new Weedle2(),
  new Yveltal(),
  new Zangoose(),
  new Zapdos(),
  new Zebstrika(),
  new Zeraora(),
  new Zoroark(),
  new Zorua(),

  // Trainers
  new BillsAnalysis(),
  new BlackMarket(),
  new BlackMarketPrismStar(),
  new BuffPadding(),
  new Dana(),
  new DangerousDrill(),
  new Electrocharger(),
  new Evelyn(),
  new FairyCharmUb(),
  new GrassMemory(),
  new IngoAndEmmet(),
  new Jasmine(),
  new JudgeWhistle(),
  new LavenderTown(),
  new MetalGoggles(),
  new Morgan(),
  new Nanu(),
  new Nita(),
  new PokemonCommunication(),
  new ReturnLabel(),
  new SabrinasSuggestion(),
  new ViridianForest(),
  new WaterMemory(),
  new WondrousLabyrinth(),
  new WondrousLabyrinthPrismStar(),

  // Other Prints (Reprints & Alt Arts)
  new BrocksGritTEU(),
  new PikachuZekromGXTEU(),
  new ErikasHospitalityTEU(),
  new UnidentifiedFossilTEU(),
  new CelebiVenusaurGX2TEU(),
  new MagikarpWailordGX2TEU(),
  new PikachuZekromGX2TEU(),
  new GengarMimikyuGX2TEU(),
  new GengarMimikyuGX3TEU(),
  new CobalionGX2TEU(),
  new EeveeSnorlaxGX2TEU(),
  new BrocksGrit2TEU(),
  new ErikasHospitality2TEU(),
  new IngoAndEmmet2TEU(),
  new Jasmine2TEU(),
  new CelebiVenusaurGX3TEU(),
  new MagikarpWailordGX3TEU(),
  new PikachuZekromGX3TEU(),
  new GengarMimikyuGX4TEU(),
  new CobalionGX3TEU(),
  new EeveeSnorlaxGX3TEU(),
  new DangerousDrill2TEU(),
  new JudgeWhistle2TEU(),
  new MetalGoggles2TEU(),
  new PokemonCommunicationHS4TEU(),
  new PokemonCommunicationHS2TEU(),
  new PokemonCommunicationHS3TEU(),
  new GrassEnergyTEU(),
  new FireEnergyTEU(),
  new WaterEnergyTEU(),
  new LightningEnergyTEU(),
  new PsychicEnergyTEU(),
  new FightingEnergyTEU(),
  new DarknessEnergyTEU(),
  new MetalEnergyTEU(),
  new FairyEnergyTEU(),
  new AlolanMarowakTEU(),
  new AlolanExeggutorTEU(),
  new AmpharosGx2(),
  new HoopaGx2(),
  new IncineroarGx2(),
  new LatiasAndLatiosGx2(),
  new LatiasAndLatiosGx3(),
  new Dana2(),
  new Evelyn2(),
  new Morgan2(),
  new Nanu2(),
  new Nita2(),
  new SabrinasSuggestion2(),
  new AmpharosGx3(),
  new HoopaGx3(),
  new IncineroarGx3(),
  new LatiasAndLatiosGx4(),
  new Electrocharger2(),
];
