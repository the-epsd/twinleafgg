import { EnergyRetrieval } from '../set-base-set/energy-retrieval';
import { EscapeRopeBUS } from '../set-burning-shadows/other-prints';
import { ExpShare } from '../set-scarlet-and-violet/exp-share';
import { RareCandy } from '../set-ex-holon-phantoms/rare-candy';
import { WailordEx as WailordExPRC38 } from '../set-primal-clash/wailord-ex';
import { GroudonEx as GroudonExPRC85 } from '../set-primal-clash/groudon-ex';
import { PrimalGroudonEx as PrimalGroudonExPRC86 } from '../set-primal-clash/primal-groudon-ex';
import { GardevoirEx as GardevoirExPRC105 } from '../set-primal-clash/gardevoir-ex';
import { MGardevoirEx as MGardevoirExPRC106 } from '../set-primal-clash/mega-gardevoir-ex';
import { ArchiesAceInTheHole as ArchiesAceInTheHolePRC124 } from '../set-primal-clash/archies-ace-in-the hole';
import { MaxiesHiddenBallTrick as MaxiesHiddenBallTrickPRC133 } from '../set-primal-clash/maxies-hidden-ball-trick';
import { Teammates as TeammatesPRC141 } from '../set-primal-clash/teammates';
import { DiveBall as DiveBallPRC125 } from '../set-primal-clash/dive-ball';
import { EnhancedHammer } from '../set-twilight-masquerade/enhanced-hammer';
import { Switch } from '../set-base-set/switch';
import { WeaknessPolicy as WeaknessPolicyPRC142 } from '../set-primal-clash/weakness-policy';
import { TrevenantEx } from './trevenant-ex';
import { CameruptEx } from './camerupt-ex';
import { KyogreEx } from './kyogre-ex';
import { PrimalKyogreEx } from './primal-kyogre-ex';
import { SharpedoEx } from './sharpedo-ex';
import { AggronEx } from './aggron-ex';
import { MAggronEx } from './m-aggron-ex';
import { ProfessorBirchsObservations } from './professor-birchs-observations';
export class EnergyRetrievalPRC extends EnergyRetrieval {
  public setNumber = '126';
  public fullName: string = 'Energy Retrieval (PRC 126)';
  public legacyFullName = 'Energy Retrieval PRC';
  public set = 'PRC';
}

export class EscapeRopeBUSPRC extends EscapeRopeBUS {
  public setNumber = '127';
  public fullName: string = 'Escape Rope (PRC 127)';
  public legacyFullName = 'Escape Rope PRC';
  public set = 'PRC';
}

export class ExpSharePRC extends ExpShare {
  public setNumber = '128';
  public fullName: string = 'Exp. Share (PRC 128)';
  public legacyFullName = 'Exp. Share PRC';
  public set = 'PRC';
}

export class RareCandyPRC extends RareCandy {
  public setNumber = '135';
  public fullName: string = 'Rare Candy (PRC 135)';
  public legacyFullName = 'Rare Candy PRC';
  public set = 'PRC';
}

export class WailordEx2PRC extends WailordExPRC38 {
  public setNumber = '147';
  public fullName: string = 'Wailord-EX (PRC 147)';
  public legacyFullName = 'Wailord EX2 PRC';
  public set = 'PRC';
}

export class GroudonEx2PRC extends GroudonExPRC85 {
  public setNumber = '150';
  public fullName: string = 'Groudon-EX (PRC 150)';
  public legacyFullName = 'Groudon EX2 PRC';
  public set = 'PRC';
}

export class PrimalGroudonEx2PRC extends PrimalGroudonExPRC86 {
  public setNumber = '151';
  public fullName: string = 'Primal Groudon-EX (PRC 151)';
  public legacyFullName = 'Primal Groudon-EX2 PRC';
  public set = 'PRC';
}

export class GardevoirEx2PRC extends GardevoirExPRC105 {
  public setNumber = '155';
  public fullName: string = 'Gardevoir-EX (PRC 155)';
  public legacyFullName = 'Gardevoir EX2 PRC';
  public set = 'PRC';
}

export class MGardevoirEx2PRC extends MGardevoirExPRC106 {
  public setNumber = '156';
  public fullName: string = 'M Gardevoir-EX (PRC 156)';
  public legacyFullName = 'M Gardevoir-EX2 PRC';
  public set = 'PRC';
}

export class ArchiesAceInTheHole2PRC extends ArchiesAceInTheHolePRC124 {
  public setNumber = '157';
  public fullName: string = 'Archie\'s Ace in the Hole (PRC 157)';
  public legacyFullName = 'Archie\'s Ace in the Hole2 PRC';
  public set = 'PRC';
}

export class MaxiesHiddenBallTrick2PRC extends MaxiesHiddenBallTrickPRC133 {
  public setNumber = '158';
  public fullName: string = 'Maxie\'s Hidden Ball Trick (PRC 158)';
  public legacyFullName = 'Maxie\'s Hidden Ball Trick2 PRC';
  public set = 'PRC';
}

export class Teammates2PRC extends TeammatesPRC141 {
  public setNumber = '160';
  public fullName: string = 'Teammates (PRC 160)';
  public legacyFullName = 'Teammates2 PRC';
  public set = 'PRC';
}

export class DiveBall2PRC extends DiveBallPRC125 {
  public setNumber = '161';
  public fullName: string = 'Dive Ball (PRC 161)';
  public legacyFullName = 'Dive Ball2 PRC';
  public set = 'PRC';
}

export class EnhancedHammerPRC extends EnhancedHammer {
  public setNumber = '162';
  public fullName: string = 'Enhanced Hammer (PRC 162)';
  public legacyFullName = 'Enhanced Hammer PRC';
  public set = 'PRC';
}

export class SwitchPRC extends Switch {
  public setNumber = '163';
  public fullName: string = 'Switch (PRC 163)';
  public legacyFullName = 'Switch PRC';
  public set = 'PRC';
}

export class WeaknessPolicy2PRC extends WeaknessPolicyPRC142 {
  public setNumber = '164';
  public fullName: string = 'Weakness Policy (PRC 164)';
  public legacyFullName = 'Weakness Policy2 PRC';
  public set = 'PRC';
}

export class TrevenantEx2 extends TrevenantEx {
  public set: string = 'PRC';
  public setNumber: string = '145';
  public fullName: string = 'Trevenant-EX (PRC 145)';
  public legacyFullName = 'Trevenant-EX PRC 145';
}

export class CameruptEx2 extends CameruptEx {
  public set: string = 'PRC';
  public setNumber: string = '146';
  public fullName: string = 'Camerupt-EX (PRC 146)';
  public legacyFullName = 'Camerupt-EX PRC 146';
}

export class KyogreEx2 extends KyogreEx {
  public set: string = 'PRC';
  public setNumber: string = '148';
  public fullName: string = 'Kyogre-EX (PRC 148)';
  public legacyFullName = 'Kyogre-EX PRC 148';
}

export class PrimalKyogreEx2 extends PrimalKyogreEx {
  public set: string = 'PRC';
  public setNumber: string = '149';
  public fullName: string = 'Primal Kyogre-EX (PRC 149)';
  public legacyFullName = 'Primal Kyogre-EX PRC 149';
}

export class SharpedoEx2 extends SharpedoEx {
  public set: string = 'PRC';
  public setNumber: string = '152';
  public fullName: string = 'Sharpedo-EX (PRC 152)';
  public legacyFullName = 'Sharpedo-EX PRC 152';
}

export class AggronEx2 extends AggronEx {
  public set: string = 'PRC';
  public setNumber: string = '153';
  public fullName: string = 'Aggron-EX (PRC 153)';
  public legacyFullName = 'Aggron-EX PRC 153';
}

export class MAggronEx2 extends MAggronEx {
  public set: string = 'PRC';
  public setNumber: string = '154';
  public fullName: string = 'M Aggron-EX (PRC 154)';
  public legacyFullName = 'M Aggron-EX PRC 154';
}

export class ProfessorBirchsObservations2 extends ProfessorBirchsObservations {
  public set: string = 'PRC';
  public setNumber: string = '159';
  public fullName: string = 'Professor Birch\'s Observations (PRC 159)';
  public legacyFullName = 'Professor Birch\'s Observations PRC 159';
}
