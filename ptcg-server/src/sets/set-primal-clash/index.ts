import { Card } from '../../game/store/card/card';
import { Aegislash } from './aegislash';
import { AggronEx } from './aggron-ex';
import { Azumarill } from './azumarill';
import { Azumarill2 } from './azumarill-2';
import { Barboach } from './barboach';
import { Beedrill } from './beedrill';
import { Bibarel } from './bibarel';
import { Bidoof } from './bidoof';
import { Bidoof2 } from './bidoof-2';
import { Blaziken } from './blaziken';
import { Bouffalant } from './bouffalant';
import { Breloom } from './breloom';
import { Bunnelby } from './bunnelby';
import { Bunnelby2 } from './bunnelby-2';
import { CameruptEx } from './camerupt-ex';
import { Chinchou } from './chinchou';
import { Clamperl } from './clamperl';
import { Combusken } from './combusken';
import { Corphish } from './corphish';
import { Crawdaunt } from './crawdaunt';
import { Delcatty } from './delcatty';
import { Diggersby } from './diggersby';
import { Doublade } from './doublade';
import { Drilbur } from './drilbur';
import { Eelektrik } from './eelektrik';
import { Eelektrik2 } from './eelektrik-2';
import { Eelektross } from './eelektross';
import { Electrike } from './electrike';
import { Electrike2 } from './electrike-2';
import { Excadrill } from './excadrill';
import { Excadrill2 } from './excadrill-2';
import { Feebas } from './feebas';
import { Flygon } from './flygon';
import { GardevoirEx } from './gardevoir-ex';
import { Gorebyss } from './gorebyss';
import { Gorebyss2 } from './gorebyss-2';
import { Groudon } from './groudon';
import { GroudonEx } from './groudon-ex';
import { Grovyle } from './grovyle';
import { Hippopotas } from './hippopotas';
import { Hippowdon } from './hippowdon';
import { Honedge } from './honedge';
import { Horsea } from './horsea';
import { Huntail } from './huntail';
import { Illumise } from './illumise';
import { Kakuna } from './kakuna';
import { Kingdra } from './kingdra';
import { Kingdra2 } from './kingdra-2';
import { Kyogre } from './kyogre';
import { KyogreEx } from './kyogre-ex';
import { Lanturn } from './lanturn';
import { Linoone } from './linoone';
import { Lombre } from './lombre';
import { Lotad } from './lotad';
import { Ludicolo } from './ludicolo';
import { Ludicolo2 } from './ludicolo-2';
import { MAggronEx } from './m-aggron-ex';
import { MGardevoirEx } from './m-gardevoir-ex';
import { Magcargo } from './magcargo';
import { Magcargo2 } from './magcargo-2';
import { Manaphy } from './manaphy';
import { Manectric } from './manectric';
import { Marill } from './marill';
import { Marshtomp } from './marshtomp';
import { Masquerain } from './masquerain';
import { Medicham } from './medicham';
import { Medicham2 } from './medicham-2';
import { Meditite } from './meditite';
import { Milotic } from './milotic';
import { MrMime } from './mr-mime';
import { Mudkip } from './mudkip';
import { Nidoqueen } from './nidoqueen';
import { Nidoqueen2 } from './nidoqueen-2';
import { NidoranF } from './nidoran-f';
import { Nidorina } from './nidorina';
import { Ninetales } from './ninetales';
import { Nosepass } from './nosepass';
import { PrimalGroudonEx } from './primal-groudon-ex';
import { PrimalKyogreEx } from './primal-kyogre-ex';
import { Probopass } from './probopass';
import { Rhydon } from './rhydon';
import { Rhyhorn } from './rhyhorn';
import { Rhyperior } from './rhyperior';
import { Rhyperior2 } from './rhyperior-2';
import { Sceptile } from './sceptile';
import { Sceptile2 } from './sceptile-2';
import { Seadra } from './seadra';
import { Sealeo } from './sealeo';
import { SharpedoEx } from './sharpedo-ex';
import { Shroomish } from './shroomish';
import { Skitty } from './skitty';
import { Slugma } from './slugma';
import { Solrock } from './solrock';
import { Spheal } from './spheal';
import { Spheal2 } from './spheal-2';
import { Spinda } from './spinda';
import { Starmie } from './starmie';
import { Staryu } from './staryu';
import { Surskit } from './surskit';
import { Swampert } from './swampert';
import { Swampert2 } from './swampert-2';
import { Tangela } from './tangela';
import { Tangrowth } from './tangrowth';
import { Tentacool } from './tentacool';
import { Tentacool2 } from './tentacool-2';
import { Tentacruel } from './tentacruel';
import { Torchic } from './torchic';
import { Torchic2 } from './torchic-2';
import { Trapinch } from './trapinch';
import { Treecko } from './treecko';
import { TrevenantEx } from './trevenant-ex';
import { Tynamo } from './tynamo';
import { Vibrava } from './vibrava';
import { Volbeat } from './volbeat';
import { Vulpix } from './vulpix';
import { WailordEx } from './wailord-ex';
import { Walrein } from './walrein';
import { Weedle } from './weedle';
import { Whiscash } from './whiscash';
import { Whiscash2 } from './whiscash-2';
import { Zigzagoon } from './zigzagoon';
import { AcroBike } from './acro-bike';
import { AggronSpiritLink } from './aggron-spirit-link';
import { ArchiesAceInTheHole } from './archies-ace-in-the-hole';
import { DiveBall } from './dive-ball';
import { FreshWaterSet } from './fresh-water-set';
import { GardevoirSpiritLink } from './gardevoir-spirit-link';
import { GroudonSpiritLink } from './groudon-spirit-link';
import { KyogreSpiritLink } from './kyogre-spirit-link';
import { MaxiesHiddenBallTrick } from './maxies-hidden-ball-trick';
import { ProfessorBirchsObservations } from './professor-birchs-observations';
import { RepeatBall } from './repeat-ball';
import { RoughSeas } from './rough-seas';
import { ScorchedEarth } from './scorched-earth';
import { ShrineOfMemories } from './shrine-of-memories';
import { SilentLab } from './silent-lab';
import { Teammates } from './teammates';
import { WeaknessPolicy } from './weakness-policy';
import { ShieldEnergy } from './shield-energy';
import { WonderEnergy } from './wonder-energy';

import {
  EnergyRetrievalPRC,
  EscapeRopeBUSPRC,
  ExpSharePRC,
  RareCandyPRC,
  WailordEx2PRC,
  GroudonEx2PRC,
  PrimalGroudonEx2PRC,
  GardevoirEx2PRC,
  MGardevoirEx2PRC,
  ArchiesAceInTheHole2PRC,
  MaxiesHiddenBallTrick2PRC,
  Teammates2PRC,
  DiveBall2PRC,
  EnhancedHammerPRC,
  SwitchPRC,
  WeaknessPolicy2PRC,
  TrevenantEx2,
  CameruptEx2,
  KyogreEx2,
  PrimalKyogreEx2,
  SharpedoEx2,
  AggronEx2,
  MAggronEx2,
  ProfessorBirchsObservations2,
} from './other-prints';

export const setPrimalClash: Card[] = [
  // Pokemon
  new Aegislash(),
  new AggronEx(),
  new Azumarill(),
  new Azumarill2(),
  new Barboach(),
  new Beedrill(),
  new Bibarel(),
  new Bidoof(),
  new Bidoof2(),
  new Blaziken(),
  new Bouffalant(),
  new Breloom(),
  new Bunnelby(),
  new Bunnelby2(),
  new CameruptEx(),
  new Chinchou(),
  new Clamperl(),
  new Combusken(),
  new Corphish(),
  new Crawdaunt(),
  new Delcatty(),
  new Diggersby(),
  new Doublade(),
  new Drilbur(),
  new Eelektrik(),
  new Eelektrik2(),
  new Eelektross(),
  new Electrike(),
  new Electrike2(),
  new Excadrill(),
  new Excadrill2(),
  new Feebas(),
  new Flygon(),
  new GardevoirEx(),
  new Gorebyss(),
  new Gorebyss2(),
  new Groudon(),
  new GroudonEx(),
  new Grovyle(),
  new Hippopotas(),
  new Hippowdon(),
  new Honedge(),
  new Horsea(),
  new Huntail(),
  new Illumise(),
  new Kakuna(),
  new Kingdra(),
  new Kingdra2(),
  new Kyogre(),
  new KyogreEx(),
  new Lanturn(),
  new Linoone(),
  new Lombre(),
  new Lotad(),
  new Ludicolo(),
  new Ludicolo2(),
  new MAggronEx(),
  new MGardevoirEx(),
  new Magcargo(),
  new Magcargo2(),
  new Manaphy(),
  new Manectric(),
  new Marill(),
  new Marshtomp(),
  new Masquerain(),
  new Medicham(),
  new Medicham2(),
  new Meditite(),
  new Milotic(),
  new MrMime(),
  new Mudkip(),
  new Nidoqueen(),
  new Nidoqueen2(),
  new NidoranF(),
  new Nidorina(),
  new Ninetales(),
  new Nosepass(),
  new PrimalGroudonEx(),
  new PrimalKyogreEx(),
  new Probopass(),
  new Rhydon(),
  new Rhyhorn(),
  new Rhyperior(),
  new Rhyperior2(),
  new Sceptile(),
  new Sceptile2(),
  new Seadra(),
  new Sealeo(),
  new SharpedoEx(),
  new Shroomish(),
  new Skitty(),
  new Slugma(),
  new Solrock(),
  new Spheal(),
  new Spheal2(),
  new Spinda(),
  new Starmie(),
  new Staryu(),
  new Surskit(),
  new Swampert(),
  new Swampert2(),
  new Tangela(),
  new Tangrowth(),
  new Tentacool(),
  new Tentacool2(),
  new Tentacruel(),
  new Torchic(),
  new Torchic2(),
  new Trapinch(),
  new Treecko(),
  new TrevenantEx(),
  new Tynamo(),
  new Vibrava(),
  new Volbeat(),
  new Vulpix(),
  new WailordEx(),
  new Walrein(),
  new Weedle(),
  new Whiscash(),
  new Whiscash2(),
  new Zigzagoon(),

  // Trainers
  new AcroBike(),
  new AggronSpiritLink(),
  new ArchiesAceInTheHole(),
  new DiveBall(),
  new FreshWaterSet(),
  new GardevoirSpiritLink(),
  new GroudonSpiritLink(),
  new KyogreSpiritLink(),
  new MaxiesHiddenBallTrick(),
  new ProfessorBirchsObservations(),
  new RepeatBall(),
  new RoughSeas(),
  new ScorchedEarth(),
  new ShrineOfMemories(),
  new SilentLab(),
  new Teammates(),
  new WeaknessPolicy(),

  // Energy
  new ShieldEnergy(),
  new WonderEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new EnergyRetrievalPRC(),
  new EscapeRopeBUSPRC(),
  new ExpSharePRC(),
  new RareCandyPRC(),
  new WailordEx2PRC(),
  new GroudonEx2PRC(),
  new PrimalGroudonEx2PRC(),
  new GardevoirEx2PRC(),
  new MGardevoirEx2PRC(),
  new ArchiesAceInTheHole2PRC(),
  new MaxiesHiddenBallTrick2PRC(),
  new Teammates2PRC(),
  new DiveBall2PRC(),
  new EnhancedHammerPRC(),
  new SwitchPRC(),
  new WeaknessPolicy2PRC(),
  new TrevenantEx2(),
  new CameruptEx2(),
  new KyogreEx2(),
  new PrimalKyogreEx2(),
  new SharpedoEx2(),
  new AggronEx2(),
  new MAggronEx2(),
  new ProfessorBirchsObservations2(),
];
