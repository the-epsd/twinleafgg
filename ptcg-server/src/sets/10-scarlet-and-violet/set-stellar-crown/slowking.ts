...
function* useSeekInspiration(
  ...
  yield store.prompt(
    state,
    new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_COPY, pokemonInQuestion, {
      allowCancel: true,
    }),
    (result) => {
      selected = result;
...
export class Slowking extends PokemonCard {
  ...
  public attacks = [
    ...
    { name: 'Super Psy Bolt', cost: [P, P, C], damage: 120, text: '' },
  ];
