import { Cheren } from '../set-emerging-powers/cheren';
import { N } from '../set-fates-collide/n';
import { UltraBall } from '../set-scarlet-and-violet/ultra-ball';
import { RaikouEx as RaikouExDEX38 } from '../set-dark-explorers/raikou-ex';
import { TornadusEx as TornadusExDEX90 } from '../set-dark-explorers/tornadus-ex';
import { Archeops } from '../set-noble-victories/archeops';
import { PokemonCatcher } from '../set-emerging-powers/pokemon-catcher';
import { DarkPatch } from '../set-astral-radiance/dark-patch';
import { DarkraiEx } from './darkrai-ex';
import { EnhancedHammer } from '../set-twilight-masquerade/enhanced-hammer';
import { ProfessorJuniper } from '../set-black-and-white/professor-juniper';
import { RareCandy } from '../set-scarlet-and-violet/rare-candy';
import { EnteiEx } from './entei-ex';
import { KyogreEx } from './kyogre-ex';
import { GroudonEx } from './groudon-ex';
import { Gardevoir } from './gardevoir';

export class DarkPatchDEX extends DarkPatch {
  public setNumber = '93';
  public fullName: string = 'Dark Patch (DEX 93)';
  public legacyFullName = 'Dark Patch DEX';
  public set = 'DEX';
}

export class DarkraiExDEX extends DarkraiEx {
  public setNumber = '107';
  public fullName: string = 'Darkrai-EX (DEX 107)';
  public legacyFullName = 'Darkrai EX DEX 107';
  public set = 'DEX';
}

export class EnhancedHammerDEX extends EnhancedHammer {
  public setNumber = '94';
  public fullName: string = 'Enhanced Hammer (DEX 94)';
  public legacyFullName = 'Enhanced Hammer DEX';
  public set = 'DEX';
}

export class ProfessorJuniperDEX extends ProfessorJuniper {
  public setNumber = '98';
  public fullName: string = 'Professor Juniper (DEX 98)';
  public legacyFullName = 'Professor Juniper DEX';
  public set = 'DEX';
}

export class RareCandyDEX extends RareCandy {
  public setNumber = '100';
  public fullName: string = 'Rare Candy (DEX 100)';
  public legacyFullName = 'Rare Candy DEX';
  public set = 'DEX';
  public text = 'Choose 1 of your Basic Pokémon in play. If you have a Stage 2 card in your hand that evolves from that Pokémon, put that card on the Basic Pokémon. (This counts as evolving that Pokémon.) You can\'t use this card during your first turn or on a Basic Pokémon that was put into play this turn.';
  public regulationMark: string = '';
}
export class CherenDEX extends Cheren {
  public setNumber = '91';
  public fullName: string = 'Cheren (DEX 91)';
  public legacyFullName = 'Cheren DEX';
  public set = 'DEX';
}

export class NDEX extends N {
  public setNumber = '96';
  public fullName: string = 'N (DEX 96)';
  public legacyFullName = 'N DEX';
  public set = 'DEX';
}

export class UltraBallDEX extends UltraBall {
  public setNumber = '102';
  public fullName: string = 'Ultra Ball (DEX 102)';
  public legacyFullName = 'Ultra Ball DEX';
  public set = 'DEX';
}

export class RaikouEx2DEX extends RaikouExDEX38 {
  public setNumber = '105';
  public fullName: string = 'Raikou-EX (DEX 105)';
  public legacyFullName = 'Raikou EX2 DEX';
  public set = 'DEX';
}

export class TornadusEx2DEX extends TornadusExDEX90 {
  public setNumber = '108';
  public fullName: string = 'Tornadus-EX (DEX 108)';
  public legacyFullName = 'Tornadus EX2 DEX';
  public set = 'DEX';
}

export class ArcheopsDEX extends Archeops {
  public setNumber = '110';
  public fullName: string = 'Archeops (DEX 110)';
  public legacyFullName = 'Archeops DEX';
  public set = 'DEX';
}

export class PokemonCatcherDEX extends PokemonCatcher {
  public setNumber = '111';
  public fullName: string = 'Pokémon Catcher (DEX 111)';
  public legacyFullName = 'Pokemon Catcher DEX';
  public set = 'DEX';
}

export class EnteiEx2 extends EnteiEx {
  public setNumber = '103';
  public fullName: string = 'Entei-EX (DEX 103)';
  public legacyFullName = 'Entei-EX DEX 103';
}

export class KyogreEx2 extends KyogreEx {
  public setNumber = '104';
  public fullName: string = 'Kyogre-EX (DEX 104)';
  public legacyFullName = 'Kyogre-EX DEX 104';
}

export class GroudonEx2 extends GroudonEx {
  public setNumber = '106';
  public fullName: string = 'Groudon-EX (DEX 106)';
  public legacyFullName = 'Groudon-EX DEX 106';
}

export class GardevoirDEX extends Gardevoir {
  public setNumber = '109';
  public fullName: string = 'Gardevoir (DEX 109)';
  public legacyFullName = 'Gardevoir DEX 109';
}
