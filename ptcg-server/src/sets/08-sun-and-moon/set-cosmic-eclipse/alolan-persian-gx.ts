...
export class AlolanPersianGX extends PokemonCard {
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof AbstractAttackEffect && effect.target.getPokemonCard() === this) {
      ...
      const isTagTeam = sourceCard?.hasTag(CardTag.TAG_TEAM);
      const isUltraBeast = sourceCard?.hasTag(CardTag.ULTRA_BEAST);
      const hasSpecialEnergy = effect.source.energies.cards.some(
        (energy) => energy.energyType === EnergyType.SPECIAL,
      );
