import { Card } from '../../../game/store/card/card';
import { Aipom } from './aipom';
import { Ambipom } from './ambipom';
import { Amoonguss } from './amoonguss';
import { Ampharos } from './ampharos';
import { Anorith } from './anorith';
import { Armaldo } from './armaldo';
import { Avalugg } from './avalugg';
import { Azumarill } from './azumarill';
import { Bastiodon } from './bastiodon';
import { Bergmite } from './bergmite';
import { Bisharp } from './bisharp';
import { Braviary } from './braviary';
import { Chandelure } from './chandelure';
import { Chimchar } from './chimchar';
import { Clauncher } from './clauncher';
import { Clawitzer } from './clawitzer';
import { ClawitzerBreak } from './clawitzer-break';
import { Cobalion } from './cobalion';
import { Croagunk } from './croagunk';
import { Deino } from './deino';
import { Dewott } from './dewott';
import { Drifblim } from './drifblim';
import { Drifloon } from './drifloon';
import { Druddigon } from './druddigon';
import { Flaaffy } from './flaaffy';
import { Fletchinder } from './fletchinder';
import { Fletchling } from './fletchling';
import { Foongus } from './foongus';
import { Galvantula } from './galvantula';
import { GardevoirEx } from './gardevoir-ex';
import { Gastrodon } from './gastrodon';
import { Hawlucha } from './hawlucha';
import { Hoopa } from './hoopa';
import { Hoppip } from './hoppip';
import { Hydreigon } from './hydreigon';
import { HydreigonBreak } from './hydreigon-break';
import { Infernape } from './infernape';
import { Joltik } from './joltik';
import { Jumpluff } from './jumpluff';
import { Klang } from './klang';
import { Klefki } from './klefki';
import { Klink } from './klink';
import { Klinklang } from './klinklang';
import { Lampent } from './lampent';
import { Larvesta } from './larvesta';
import { Litleo } from './litleo';
import { Litwick } from './litwick';
import { MGardevoirEx } from './m-gardevoir-ex';
import { MSteelixEx } from './m-steelix-ex';
import { MagearnaEX } from './magearna-ex';
import { Mankey } from './mankey';
import { Mantine } from './mantine';
import { Mareep } from './mareep';
import { Marill } from './marill';
import { Meowth } from './meowth';
import { Monferno } from './monferno';
import { Nidoking } from './nidoking';
import { NidoranM } from './nidoran-m';
import { Nidorino } from './nidorino';
import { Nosepass } from './nosepass';
import { Nuzleaf } from './nuzleaf';
import { Oshawott } from './oshawott';
import { Pawniard } from './pawniard';
import { Persian } from './persian';
import { Ponyta } from './ponyta';
import { Primeape } from './primeape';
import { Probopass } from './probopass';
import { Pyroar } from './pyroar';
import { PyroarBreak } from './pyroar-break';
import { Rapidash } from './rapidash';
import { Rufflet } from './rufflet';
import { Samurott } from './samurott';
import { Seedot } from './seedot';
import { Shellos } from './shellos';
import { Shieldon } from './shieldon';
import { Shiftry } from './shiftry';
import { Skiploom } from './skiploom';
import { Sneasel } from './sneasel';
import { Spiritomb } from './spiritomb';
import { SteelixEx } from './steelix-ex';
import { Talonflame } from './talonflame';
import { TalonflameBreak } from './talonflame-break';
import { Tangela } from './tangela';
import { Tangrowth } from './tangrowth';
import { Toxicroak } from './toxicroak';
import { Volcanion } from './volcanion';
import { VolcanionEX } from './volcanion-ex';
import { Volcarona } from './volcarona';
import { Weavile } from './weavile';
import { Xerneas } from './xerneas';
import { XerneasBREAK } from './xerneas-break';
import { Yanma } from './yanma';
import { Yanmega } from './yanmega';
import { YanmegaBreak } from './yanmega-break';
import { Yveltal } from './yveltal';
import { YveltalBreak } from './yveltal-break';
import { Zweilous } from './zweilous';
import { ArmorFossilShieldon } from './armor-fossil-shieldon';
import { CaptivatingPokePuff } from './captivating-poke-puff';
import { ClawFossilAnorith } from './claw-fossil-anorith';
import { GreedyDice } from './greedy-dice';
import { NinjaBoy } from './ninja-boy';
import { PokemonRanger } from './pokemon-ranger';
import { SpecialCharge } from './special-charge';
import { SteelixSpiritLink } from './steelix-spirit-link';

import {
  MagearnaEXSTS,
  GardevoirSpiritLinkSTS,
  VolcanionEX2STS,
  GardevoirEx2STS,
  PokemonRanger2STS,
  ProfessorSycamoreSTS,
  VolcanionEX3STS,
  GardevoirEx3STS,
  SteelixEx2,
  MSteelixEx2,
} from './other-prints';
import { MGardevoirEx2 } from '../set-primal-clash/m-gardevoir-ex-2';

export const setSteamSiege: Card[] = [
  // Pokemon
  new Aipom(),
  new Ambipom(),
  new Amoonguss(),
  new Ampharos(),
  new Anorith(),
  new Armaldo(),
  new Avalugg(),
  new Azumarill(),
  new Bastiodon(),
  new Bergmite(),
  new Bisharp(),
  new Braviary(),
  new Chandelure(),
  new Chimchar(),
  new Clauncher(),
  new Clawitzer(),
  new ClawitzerBreak(),
  new Cobalion(),
  new Croagunk(),
  new Deino(),
  new Dewott(),
  new Drifblim(),
  new Drifloon(),
  new Druddigon(),
  new Flaaffy(),
  new Fletchinder(),
  new Fletchling(),
  new Foongus(),
  new Galvantula(),
  new GardevoirEx(),
  new Gastrodon(),
  new Hawlucha(),
  new Hoopa(),
  new Hoppip(),
  new Hydreigon(),
  new HydreigonBreak(),
  new Infernape(),
  new Joltik(),
  new Jumpluff(),
  new Klang(),
  new Klefki(),
  new Klink(),
  new Klinklang(),
  new Lampent(),
  new Larvesta(),
  new Litleo(),
  new Litwick(),
  new MGardevoirEx(),
  new MGardevoirEx2(),
  new MSteelixEx(),
  new MagearnaEX(),
  new Mankey(),
  new Mantine(),
  new Mareep(),
  new Marill(),
  new Meowth(),
  new Monferno(),
  new Nidoking(),
  new NidoranM(),
  new Nidorino(),
  new Nosepass(),
  new Nuzleaf(),
  new Oshawott(),
  new Pawniard(),
  new Persian(),
  new Ponyta(),
  new Primeape(),
  new Probopass(),
  new Pyroar(),
  new PyroarBreak(),
  new Rapidash(),
  new Rufflet(),
  new Samurott(),
  new Seedot(),
  new Shellos(),
  new Shieldon(),
  new Shiftry(),
  new Skiploom(),
  new Sneasel(),
  new Spiritomb(),
  new SteelixEx(),
  new Talonflame(),
  new TalonflameBreak(),
  new Tangela(),
  new Tangrowth(),
  new Toxicroak(),
  new Volcanion(),
  new VolcanionEX(),
  new Volcarona(),
  new Weavile(),
  new Xerneas(),
  new XerneasBREAK(),
  new Yanma(),
  new Yanmega(),
  new YanmegaBreak(),
  new Yveltal(),
  new YveltalBreak(),
  new Zweilous(),

  // Trainers
  new ArmorFossilShieldon(),
  new CaptivatingPokePuff(),
  new ClawFossilAnorith(),
  new GreedyDice(),
  new NinjaBoy(),
  new PokemonRanger(),
  new SpecialCharge(),
  new SteelixSpiritLink(),

  // Other Prints (Reprints & Alt Arts)
  new MagearnaEXSTS(),
  new GardevoirSpiritLinkSTS(),
  new VolcanionEX2STS(),
  new GardevoirEx2STS(),
  new PokemonRanger2STS(),
  new ProfessorSycamoreSTS(),
  new VolcanionEX3STS(),
  new GardevoirEx3STS(),
  new SteelixEx2(),
  new MSteelixEx2(),
];
