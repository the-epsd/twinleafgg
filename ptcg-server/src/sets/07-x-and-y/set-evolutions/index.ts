import { Card } from '../../../game/store/card/card';
import { Arcanine } from './arcanine';
import { Beedrill } from './beedrill';
import { Chansey } from './chansey';
import { Charizard } from './charizard';
import { Dewgong } from './dewgong';
import { Diglett } from './diglett';
import { Doduo } from './doduo';
import { DragoniteEX } from './dragonite-ex';
import { Drowzee } from './drowzee';
import { Dugtrio } from './dugtrio';
import { Electabuzz } from './electabuzz';
import { Electrode } from './electrode';
import { Exeggutor } from './exeggutor';
import { Farfetchd } from './farfetchd';
import { FlyingPikachu } from './flying-pikachu';
import { Gastly } from './gastly';
import { Growlithe } from './growlithe';
import { Gyarados } from './gyarados';
import { Haunter } from './haunter';
import { Hitmonchan } from './hitmonchan';
import { ImakuniSDoduo } from './imakunis-doduo';
import { Kakuna } from './kakuna';
import { Koffing } from './koffing';
import { MCharizardEx } from './m-charizard-ex';
import { MPidgeotEx } from './m-pidgeot-ex';
import { MSlowbroEx } from './m-slowbro-ex';
import { Machamp } from './machamp';
import { MachampBreak } from './machamp-break';
import { Machoke } from './machoke';
import { Machop } from './machop';
import { Magikarp } from './magikarp';
import { Magmar } from './magmar';
import { Magnemite } from './magnemite';
import { Magneton } from './magneton';
import { Mew } from './mew';
import { Mewtwo } from './mewtwo';
import { MewtwoEx } from './mewtwo-ex';
import { Nidoking } from './nidoking';
import { NidokingBreak } from './nidoking-break';
import { NidoranM } from './nidoran-m';
import { Nidorino } from './nidorino';
import { Ninetales } from './ninetales';
import { NinetalesBreak } from './ninetales-break';
import { Onix } from './onix';
import { PidgeotEx } from './pidgeot-ex';
import { Pikachu } from './pikachu';
import { Poliwag } from './poliwag';
import { Poliwhirl } from './poliwhirl';
import { Poliwrath } from './poliwrath';
import { Ponyta } from './ponyta';
import { Porygon } from './porygon';
import { Raichu } from './raichu';
import { Raticate } from './raticate';
import { Rattata } from './rattata';
import { Sandshrew } from './sandshrew';
import { Seel } from './seel';
import { SlowbroEx } from './slowbro-ex';
import { Starmie } from './starmie';
import { StarmieBreak } from './starmie-break';
import { Staryu } from './staryu';
import { SurfingPikachu } from './surfing-pikachu';
import { Tangela } from './tangela';
import { Voltorb } from './voltorb';
import { Vulpix } from './vulpix';
import { Zapdos } from './zapdos';
import { BlastoiseSpiritLink } from './blastoise-spirit-link';
import { BrocksGrit } from './brocks-grit';
import { CharizardSpiritLink } from './charizard-spirit-link';
import { DevolutionSpray } from './devolution-spray';
import { MistysDetermination } from './mistys-determination';
import { PidgeotSpiritLink } from './pidgeot-spirit-link';
import { ProfessorOaksHint } from './professor-oaks-hint';
import { SlowbroSpiritLink } from './slowbro-spirit-link';
import { VenusaurSpiritLink } from './venusaur-spirit-link';

import {
  PokedexEVO,
  DarknessEnergyEVO,
  CaterpieEVO,
  MetapodEVO,
  WeedleEVO,
  EnergyRetrievalEVO,
  FullHealEVO,
  MaintenanceEVO,
  PotionEVO,
  ReviveEVO,
  SuperPotionEVO,
  SwitchEVO,
  DoubleColorlessEnergyEVO,
  GrassEnergyEVO,
  FireEnergyEVO,
  WaterEnergyEVO,
  LightningEnergyEVO,
  PsychicEnergyEVO,
  FightingEnergyEVO,
  MetalEnergyEVO,
  FairyEnergyEVO,
  DragoniteEX2EVO,
  BrocksGrit2EVO,
  HereComesTeamRocketEVO,
  MVenusaurEXEVO,
  MVenusaurEXEVOSR,
  VenusaurExEVO,
  CharmanderEVO,
  CharmeleonEVO,
  CharizardExEVO,
  BlastoiseExEVO,
  MBlastoiseExEVO,
  ClefairyEVO,
  MCharizardEx2,
  MBlastoiseEx2,
  MewtwoEx2,
  PidgeotEx2,
  MPidgeotEx2,
  MistysDetermination2,
} from './other-prints';

export const setEvolutions: Card[] = [
  // Pokemon
  new Arcanine(),
  new Beedrill(),
  new Chansey(),
  new Charizard(),
  new Dewgong(),
  new Diglett(),
  new Doduo(),
  new DragoniteEX(),
  new Drowzee(),
  new Dugtrio(),
  new Electabuzz(),
  new Electrode(),
  new Exeggutor(),
  new Farfetchd(),
  new FlyingPikachu(),
  new Gastly(),
  new Growlithe(),
  new Gyarados(),
  new Haunter(),
  new Hitmonchan(),
  new ImakuniSDoduo(),
  new Kakuna(),
  new Koffing(),
  new MCharizardEx(),
  new MPidgeotEx(),
  new MSlowbroEx(),
  new Machamp(),
  new MachampBreak(),
  new Machoke(),
  new Machop(),
  new Magikarp(),
  new Magmar(),
  new Magnemite(),
  new Magneton(),
  new Mew(),
  new Mewtwo(),
  new MewtwoEx(),
  new Nidoking(),
  new NidokingBreak(),
  new NidoranM(),
  new Nidorino(),
  new Ninetales(),
  new NinetalesBreak(),
  new Onix(),
  new PidgeotEx(),
  new Pikachu(),
  new Poliwag(),
  new Poliwhirl(),
  new Poliwrath(),
  new Ponyta(),
  new Porygon(),
  new Raichu(),
  new Raticate(),
  new Rattata(),
  new Sandshrew(),
  new Seel(),
  new SlowbroEx(),
  new Starmie(),
  new StarmieBreak(),
  new Staryu(),
  new SurfingPikachu(),
  new Tangela(),
  new Voltorb(),
  new Vulpix(),
  new Zapdos(),

  // Trainers
  new BlastoiseSpiritLink(),
  new BrocksGrit(),
  new CharizardSpiritLink(),
  new DevolutionSpray(),
  new MistysDetermination(),
  new PidgeotSpiritLink(),
  new ProfessorOaksHint(),
  new SlowbroSpiritLink(),
  new VenusaurSpiritLink(),

  // Other Prints (Reprints & Alt Arts)
  new PokedexEVO(),
  new DarknessEnergyEVO(),
  new CaterpieEVO(),
  new MetapodEVO(),
  new WeedleEVO(),
  new EnergyRetrievalEVO(),
  new FullHealEVO(),
  new MaintenanceEVO(),
  new PotionEVO(),
  new ReviveEVO(),
  new SuperPotionEVO(),
  new SwitchEVO(),
  new DoubleColorlessEnergyEVO(),
  new GrassEnergyEVO(),
  new FireEnergyEVO(),
  new WaterEnergyEVO(),
  new LightningEnergyEVO(),
  new PsychicEnergyEVO(),
  new FightingEnergyEVO(),
  new MetalEnergyEVO(),
  new FairyEnergyEVO(),
  new DragoniteEX2EVO(),
  new BrocksGrit2EVO(),
  new HereComesTeamRocketEVO(),
  new MVenusaurEXEVO(),
  new MVenusaurEXEVOSR(),
  new VenusaurExEVO(),
  new CharmanderEVO(),
  new CharmeleonEVO(),
  new CharizardExEVO(),
  new BlastoiseExEVO(),
  new MBlastoiseExEVO(),
  new ClefairyEVO(),
  new MCharizardEx2(),
  new MBlastoiseEx2(),
  new MewtwoEx2(),
  new PidgeotEx2(),
  new MPidgeotEx2(),
  new MistysDetermination2(),
];
