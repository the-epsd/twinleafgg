import { Card } from '../../../game/store/card/card';
import { Accelgor } from './accelgor';
import { Aggron } from './aggron';
import { Ampharos } from './ampharos';
import { Arctovish } from './arctovish';
import { Arctozolt } from './arctozolt';
import { Ariados } from './ariados';
import { Aron } from './aron';
import { Beartic } from './beartic';
import { Blaziken } from './blaziken';
import { Bounsweet } from './bounsweet';
import { Bunnelby } from './bunnelby';
import { ButterfreeV } from './butterfree-v';
import { ButterfreeVmax } from './butterfree-vmax';
import { Carnivine } from './carnivine';
import { CentiskorchV } from './centiskorch-v';
import { CentiskorchVmax } from './centiskorch-vmax';
import { CharizardV } from './charizard-v';
import { CharizardVmax } from './charizard-vmax';
import { Combusken } from './combusken';
import { Copperajah } from './copperajah';
import { Corviknight } from './corviknight';
import { Corvisquire } from './corvisquire';
import { CrobatV } from './crobat-v';
import { Cubchoo } from './cubchoo';
import { Cufant } from './cufant';
import { Darkrai } from './darkrai';
import { Decidueye } from './decidueye';
import { Dedenne } from './dedenne';
import { Deino } from './deino';
import { Delcatty } from './delcatty';
import { Diggersby } from './diggersby';
import { Diglett } from './diglett';
import { Dracovish } from './dracovish';
import { Dracozolt } from './dracozolt';
import { Ducklett } from './ducklett';
import { Dugtrio } from './dugtrio';
import { Dunsparce } from './dunsparce';
import { Electrike } from './electrike';
import { Escavalier } from './escavalier';
import { EternatusV } from './eternatus-v';
import { EternatusVMAX } from './eternatus-vmax';
import { Feebas } from './feebas';
import { Flaaffy } from './flaaffy';
import { Fletchinder } from './fletchinder';
import { Fletchling } from './fletchling';
import { Flygon } from './flygon';
import { Furret } from './furret';
import { GalarianDarmanitan } from './galarian-darmanitan';
import { GalarianDarumaka } from './galarian-darumaka';
import { GalarianMrMime } from './galarian-mr-mime';
import { GalarianMrRime } from './galarian-mr-rime';
import { GalarianSlowbroV } from './galarian-slowbro-v';
import { GalarianStunfiskV } from './galarian-stunfisk-v';
import { Golett } from './golett';
import { Golisopod } from './golisopod';
import { Golurk } from './golurk';
import { Gothita } from './gothita';
import { Gothitelle } from './gothitelle';
import { Gothorita } from './gothorita';
import { Granbull } from './granbull';
import { Greedent } from './greedent';
import { Grimer } from './grimer';
import { GrimmsnarlV } from './grimmsnarl-v';
import { GrimmsnarlVmax } from './grimmsnarl-vmax';
import { Heatran } from './heatran';
import { Hippopotas } from './hippopotas';
import { Hippowdon } from './hippowdon';
import { HoundoomV } from './houndoom-v';
import { Hydreigon } from './hydreigon';
import { Jigglypuff } from './jigglypuff';
import { Kangaskhan } from './kangaskhan';
import { Karrablast } from './karrablast';
import { Klang } from './klang';
import { Klink } from './klink';
import { Klinklang } from './klinklang';
import { Lairon } from './lairon';
import { Larvesta } from './larvesta';
import { Larvitar } from './larvitar';
import { Liepard } from './liepard';
import { Lugia } from './lugia';
import { Lunatone } from './lunatone';
import { Manectric } from './manectric';
import { Mareanie } from './mareanie';
import { Mareep } from './mareep';
import { Melmetal } from './melmetal';
import { Meltan } from './meltan';
import { MewV } from './mew-v';
import { Milotic } from './milotic';
import { Mimikyu } from './mimikyu';
import { Morelull } from './morelull';
import { Muk } from './muk';
import { Nickit } from './nickit';
import { Panpour } from './panpour';
import { Pansage } from './pansage';
import { Pansear } from './pansear';
import { Paras } from './paras';
import { Parasect } from './parasect';
import { Passimian } from './passimian';
import { Pincurchin } from './pincurchin';
import { Polteageist } from './polteageist';
import { Pupitar } from './pupitar';
import { Purrloin } from './purrloin';
import { Relicanth } from './relicanth';
import { RhyperiorV } from './rhyperior-v';
import { Rookidee } from './rookidee';
import { Rowlet } from './rowlet';
import { SalamenceV } from './salamence-v';
import { SalamenceVmax } from './salamence-vmax';
import { ScizorV } from './scizor-v';
import { ScizorVmax } from './scizor-vmax';
import { Sentret } from './sentret';
import { Shelmet } from './shelmet';
import { Shiinotic } from './shiinotic';
import { Simipour } from './simipour';
import { Simisage } from './simisage';
import { Simisear } from './simisear';
import { Sinistea } from './sinistea';
import { Skarmory } from './skarmory';
import { Skitty } from './skitty';
import { Skwovet } from './skwovet';
import { Snubbull } from './snubbull';
import { Solrock } from './solrock';
import { Spinarak } from './spinarak';
import { Staraptor } from './staraptor';
import { Staravia } from './staravia';
import { Starly } from './starly';
import { Steenee } from './steenee';
import { Suicune } from './suicune';
import { Swanna } from './swanna';
import { Talonflame } from './talonflame';
import { TapuKoko } from './tapu-koko';
import { Tauros } from './tauros';
import { Teddiursa } from './teddiursa';
import { Thievul } from './thievul';
import { Torchic } from './torchic';
import { Toxapex } from './toxapex';
import { Toxel } from './toxel';
import { Toxtricity } from './toxtricity';
import { Trapinch } from './trapinch';
import { Tsareena } from './tsareena';
import { Tyranitar } from './tyranitar';
import { Ursaring } from './ursaring';
import { Vanillish } from './vanillish';
import { Vanillite } from './vanillite';
import { Vanilluxe } from './vanilluxe';
import { Vibrava } from './vibrava';
import { VikavoltV } from './vikavolt-v';
import { Volcarona } from './volcarona';
import { Wigglytuff } from './wigglytuff';
import { Wimpod } from './wimpod';
import { Wishiwashi } from './wishiwashi';
import { Zweilous } from './zweilous';
import { BigParasol } from './big-parasol';
import { BillowingSmoke } from './billowing-smoke';
import { BirdKeeper } from './bird-keeper';
import { CapeOfToughness } from './cape-of-toughness';
import { FamiliarBell } from './familiar-bell';
import { GlimwoodTangle } from './glimwood-tangle';
import { Kabu } from './kabu';
import { OldPc } from './old-pc';
import { PokemonBreedersNurturing } from './pokemon-breeders-nurturing';
import { RareFossil } from './rare-fossil';
import { Rose } from './rose';
import { RoseTower } from './rose-tower';
import { Spikemuth } from './spikemuth';
import { StruggleGloves } from './struggle-gloves';
import { TurboPatch } from './turbo-patch';
import { YellHorn } from './yell-horn';
import { HeatFireEnergy } from './heat-fire-energy';
import { HidingDarknessEnergy } from './hiding-darkness-energy';
import { PowerfulColorlessEnergy } from './powerful-colorless-energy';

import {
  CrobatVDAA,
  DartrixDAA,
  HoopaDAA,
  PiersDAA,
  VikavoltV2DAA,
  Piers2DAA,
  PokemonBreedersNurturing2DAA,
  EternatusVMAX2DAA,
  PokemonBreedersNurturing3DAA,
  RillaboomDAA,
  CoalossalDAA,
  BigParasol2DAA,
  CaptureEnergyDAA,
  GalarianDarmanitanDAA,
  GalarianSirfetchdDAA,
  ButterfreeV2,
  HoundoomV2,
  CentiskorchV2,
  RhyperiorV2,
  ScizorV2,
  GalarianStunfiskV2,
  SalamenceV2,
  Kabu2,
  Rose2,
  ButterfreeVmax2,
  CentiskorchVmax2,
  ScizorVmax2,
  SalamenceVmax2,
  Rose3,
  TurboPatch2,
} from './other-prints';

export const setDarknessAblaze: Card[] = [
  // Pokemon
  new Accelgor(),
  new Aggron(),
  new Ampharos(),
  new Arctovish(),
  new Arctozolt(),
  new Ariados(),
  new Aron(),
  new Beartic(),
  new Blaziken(),
  new Bounsweet(),
  new Bunnelby(),
  new ButterfreeV(),
  new ButterfreeVmax(),
  new Carnivine(),
  new CentiskorchV(),
  new CentiskorchVmax(),
  new CharizardV(),
  new CharizardVmax(),
  new Combusken(),
  new Copperajah(),
  new Corviknight(),
  new Corvisquire(),
  new CrobatV(),
  new Cubchoo(),
  new Cufant(),
  new Darkrai(),
  new Decidueye(),
  new Dedenne(),
  new Deino(),
  new Delcatty(),
  new Diggersby(),
  new Diglett(),
  new Dracovish(),
  new Dracozolt(),
  new Ducklett(),
  new Dugtrio(),
  new Dunsparce(),
  new Electrike(),
  new Escavalier(),
  new EternatusV(),
  new EternatusVMAX(),
  new Feebas(),
  new Flaaffy(),
  new Fletchinder(),
  new Fletchling(),
  new Flygon(),
  new Furret(),
  new GalarianDarmanitan(),
  new GalarianDarumaka(),
  new GalarianMrMime(),
  new GalarianMrRime(),
  new GalarianSlowbroV(),
  new GalarianStunfiskV(),
  new Golett(),
  new Golisopod(),
  new Golurk(),
  new Gothita(),
  new Gothitelle(),
  new Gothorita(),
  new Granbull(),
  new Greedent(),
  new Grimer(),
  new GrimmsnarlV(),
  new GrimmsnarlVmax(),
  new Heatran(),
  new Hippopotas(),
  new Hippowdon(),
  new HoundoomV(),
  new Hydreigon(),
  new Jigglypuff(),
  new Kangaskhan(),
  new Karrablast(),
  new Klang(),
  new Klink(),
  new Klinklang(),
  new Lairon(),
  new Larvesta(),
  new Larvitar(),
  new Liepard(),
  new Lugia(),
  new Lunatone(),
  new Manectric(),
  new Mareanie(),
  new Mareep(),
  new Melmetal(),
  new Meltan(),
  new MewV(),
  new Milotic(),
  new Mimikyu(),
  new Morelull(),
  new Muk(),
  new Nickit(),
  new Panpour(),
  new Pansage(),
  new Pansear(),
  new Paras(),
  new Parasect(),
  new Passimian(),
  new Pincurchin(),
  new Polteageist(),
  new Pupitar(),
  new Purrloin(),
  new Relicanth(),
  new RhyperiorV(),
  new Rookidee(),
  new Rowlet(),
  new SalamenceV(),
  new SalamenceVmax(),
  new ScizorV(),
  new ScizorVmax(),
  new Sentret(),
  new Shelmet(),
  new Shiinotic(),
  new Simipour(),
  new Simisage(),
  new Simisear(),
  new Sinistea(),
  new Skarmory(),
  new Skitty(),
  new Skwovet(),
  new Snubbull(),
  new Solrock(),
  new Spinarak(),
  new Staraptor(),
  new Staravia(),
  new Starly(),
  new Steenee(),
  new Suicune(),
  new Swanna(),
  new Talonflame(),
  new TapuKoko(),
  new Tauros(),
  new Teddiursa(),
  new Thievul(),
  new Torchic(),
  new Toxapex(),
  new Toxel(),
  new Toxtricity(),
  new Trapinch(),
  new Tsareena(),
  new Tyranitar(),
  new Ursaring(),
  new Vanillish(),
  new Vanillite(),
  new Vanilluxe(),
  new Vibrava(),
  new VikavoltV(),
  new Volcarona(),
  new Wigglytuff(),
  new Wimpod(),
  new Wishiwashi(),
  new Zweilous(),

  // Trainers
  new BigParasol(),
  new BillowingSmoke(),
  new BirdKeeper(),
  new CapeOfToughness(),
  new FamiliarBell(),
  new GlimwoodTangle(),
  new Kabu(),
  new OldPc(),
  new PokemonBreedersNurturing(),
  new RareFossil(),
  new Rose(),
  new RoseTower(),
  new Spikemuth(),
  new StruggleGloves(),
  new TurboPatch(),
  new YellHorn(),

  // Energy
  new HeatFireEnergy(),
  new HidingDarknessEnergy(),
  new PowerfulColorlessEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new CrobatVDAA(),
  new DartrixDAA(),
  new HoopaDAA(),
  new PiersDAA(),
  new VikavoltV2DAA(),
  new Piers2DAA(),
  new PokemonBreedersNurturing2DAA(),
  new EternatusVMAX2DAA(),
  new PokemonBreedersNurturing3DAA(),
  new RillaboomDAA(),
  new CoalossalDAA(),
  new BigParasol2DAA(),
  new CaptureEnergyDAA(),
  new GalarianDarmanitanDAA(),
  new GalarianSirfetchdDAA(),
  new ButterfreeV2(),
  new HoundoomV2(),
  new CentiskorchV2(),
  new RhyperiorV2(),
  new ScizorV2(),
  new GalarianStunfiskV2(),
  new SalamenceV2(),
  new Kabu2(),
  new Rose2(),
  new ButterfreeVmax2(),
  new CentiskorchVmax2(),
  new ScizorVmax2(),
  new SalamenceVmax2(),
  new Rose3(),
  new TurboPatch2(),
];
