import { Card } from '../../../game/store/card/card';
import { Aegislash } from './aegislash';
import { Aegislash2 } from './aegislash-2';
import { Baltoy } from './baltoy';
import { Bellsprout } from './bellsprout';
import { Bisharp } from './bisharp';
import { Blipbug } from './blipbug';
import { Boltund } from './boltund';
import { Bouffalant } from './bouffalant';
import { Bronzong } from './bronzong';
import { Bronzor } from './bronzor';
import { Bruxish } from './bruxish';
import { Cacnea } from './cacnea';
import { Cacturne } from './cacturne';
import { Carkol } from './carkol';
import { Carnivine } from './carnivine';
import { Centiskorch } from './centiskorch';
import { Cherrim } from './cherrim';
import { Cherubi } from './cherubi';
import { Chimecho } from './chimecho';
import { Claydol } from './claydol';
import { Coalossal } from './coalossal';
import { Conkeldurr } from './conkeldurr';
import { Corphish } from './corphish';
import { CorviknightV } from './corviknight-v';
import { CorviknightVmax } from './corviknight-vmax';
import { Crawdaunt } from './crawdaunt';
import { Crobat } from './crobat';
import { Cubone } from './cubone';
import { Dottler } from './dottler';
import { Doublade } from './doublade';
import { Drampa } from './drampa';
import { Durant } from './durant';
import { Electabuzz } from './electabuzz';
import { Electivire } from './electivire';
import { Emboar } from './emboar';
import { EmpoleonV } from './empoleon-v';
import { Entei } from './entei';
import { Espurr } from './espurr';
import { Falinks } from './falinks';
import { Fearow } from './fearow';
import { FlappleV } from './flapple-v';
import { FlappleVMAX } from './flapple-vmax';
import { Fomantis } from './fomantis';
import { Frillish } from './frillish';
import { GalarianMrMime } from './galarian-mr-mime';
import { GalarianMrRime } from './galarian-mr-rime';
import { GalarianSlowbro } from './galarian-slowbro';
import { GalarianSlowpoke } from './galarian-slowpoke';
import { Glameow } from './glameow';
import { Gligar } from './gligar';
import { Gliscor } from './gliscor';
import { Golbat } from './golbat';
import { Grumpig } from './grumpig';
import { Gurdurr } from './gurdurr';
import { Heatmor } from './heatmor';
import { Honchkrow } from './honchkrow';
import { Honedge } from './honedge';
import { Horsea } from './horsea';
import { Houndoom } from './houndoom';
import { Houndour } from './houndour';
import { Indeedee } from './indeedee';
import { Jellicent } from './jellicent';
import { Kingdra } from './kingdra';
import { KricketuneV } from './kricketune-v';
import { Lickilicky } from './lickilicky';
import { Lickitung } from './lickitung';
import { Lurantis } from './lurantis';
import { Luxio } from './luxio';
import { Luxray } from './luxray';
import { Mankey } from './mankey';
import { Marowak } from './marowak';
import { Mawile } from './mawile';
import { Meowstic } from './meowstic';
import { Mienfoo } from './mienfoo';
import { Mienshao } from './mienshao';
import { MimikyuV } from './mimikyu-v';
import { Morpeko } from './morpeko';
import { Murkrow } from './murkrow';
import { NecrozmaV } from './necrozma-v';
import { Octillery } from './octillery';
import { Onix } from './onix';
import { Orbeetle } from './orbeetle';
import { Pachirisu } from './pachirisu';
import { Pawniard } from './pawniard';
import { Pignite } from './pignite';
import { Primeape } from './primeape';
import { Purugly } from './purugly';
import { RapidStrikeUrshifuV } from './rapid-strike-urshifu-v';
import { RapidStrikeUrshifuVMAX } from './rapid-strike-urshifu-vmax';
import { Remoraid } from './remoraid';
import { Rolycoly } from './rolycoly';
import { Salandit } from './salandit';
import { Salazzle } from './salazzle';
import { Sandaconda } from './sandaconda';
import { Scatterbug } from './scatterbug';
import { Seadra } from './seadra';
import { Shinx } from './shinx';
import { Silicobra } from './silicobra';
import { SingleStrikeUrshifuV } from './single-strike-urshifu-v';
import { SingleStrikeUrshifuVMAX } from './single-strike-urshifu-vmax';
import { Sizzlipede } from './sizzlipede';
import { Spewpa } from './spewpa';
import { Spoink } from './spoink';
import { Steelix } from './steelix';
import { Stonjourner } from './stonjourner';
import { StoutlandV } from './stoutland-v';
import { TapuBulu } from './tapu-bulu';
import { TapuKokoV } from './tapu-koko-v';
import { TapuKokoVMAX } from './tapu-koko-vmax';
import { TyranitarV } from './tyranitar-v';
import { VictiniV } from './victini-v';
import { VictiniVMAX } from './victini-vmax';
import { Victreebel } from './victreebel';
import { Vivillon } from './vivillon';
import { Weepinbell } from './weepinbell';
import { Yamper } from './yamper';
import { Zubat } from './zubat';
import { Bruno } from './bruno';
import { CampingGear } from './camping-gear';
import { Cheryl } from './cheryl';
import { EnergyRecycler } from './energy-recycler';
import { EscapeRope } from './escape-rope';
import { FanOfWaves } from './fan-of-waves';
import { KorrinasFocus } from './korrinas-focus';
import { LevelBall } from './level-ball';
import { Phoebe } from './phoebe';
import { RapidStrikeScrollOfSwirls } from './rapid-strike-scroll-of-swirls';
import { RapidStrikeStyleMustard } from './rapid-strike-style-mustard';
import { SingleStrikeScrollOfScorn } from './single-strike-scroll-of-scorn';
import { SingleStrikeStyleMustard } from './single-strike-style-mustard';
import { SordwardAndShielbert } from './sordward-and-shielbert';
import { ToolJammer } from './tool-jammer';
import { TowerOfDarkness } from './tower-of-darkness';
import { TowerOfWaters } from './tower-of-waters';
import { UrnOfVitality } from './urn-of-vitality';
import { RapidStrikeEnergy } from './rapid-strike-energy';
import { SingleStrikeEnergy } from './single-strike-energy';

import {
  ExpShareBST,
  KricketuneV2BST,
  FlappleV2BST,
  VictiniV2BST,
  EmpoleonV2BST,
  EmpoleonV3BST,
  TapuKokoV2BST,
  MimikyuV2BST,
  NecrozmaV2BST,
  SingleStrikeUrshifuV2BST,
  SingleStrikeUrshifuV3BST,
  RapidStrikeUrshifuV2BST,
  RapidStrikeUrshifuV3BST,
  TyranitarV2BST,
  TyranitarV3BST,
  StoutlandV2BST,
  Bruno2BST,
  Cheryl2BST,
  KorrinasFocus2BST,
  SingleStrikeStyleMustard2BST,
  FlappleVMAX2BST,
  VictiniVMAX2BST,
  TapuKokoVMAX2BST,
  SingleStrikeUrshifuVMAX2BST,
  SingleStrikeUrshifuVMAX3BST,
  RapidStrikeUrshifuVMAX2BST,
  RapidStrikeUrshifuVMAX3BST,
  Bruno3BST,
  Cheryl3BST,
  KorrinasFocus3BST,
  SingleStrikeStyleMustard3BST,
  Octillery2BST,
  Houndoom2BST,
  ExpShare2BST,
  LevelBall2BST,
  RapidStrikeEnergy2BST,
  SingleStrikeEnergy2BST,
  TepigBST,
  TimburrBST,
  SpearowBST,
  CorviknightV2,
  Phoebe2,
  RapidStrikeStyleMustard2,
  CorviknightVmax2,
  Phoebe3,
  RapidStrikeStyleMustard3,
} from './other-prints';

export const setBattleStyles: Card[] = [
  // Pokemon
  new Aegislash(),
  new Aegislash2(),
  new Baltoy(),
  new Bellsprout(),
  new Bisharp(),
  new Blipbug(),
  new Boltund(),
  new Bouffalant(),
  new Bronzong(),
  new Bronzor(),
  new Bruxish(),
  new Cacnea(),
  new Cacturne(),
  new Carkol(),
  new Carnivine(),
  new Centiskorch(),
  new Cherrim(),
  new Cherubi(),
  new Chimecho(),
  new Claydol(),
  new Coalossal(),
  new Conkeldurr(),
  new Corphish(),
  new CorviknightV(),
  new CorviknightVmax(),
  new Crawdaunt(),
  new Crobat(),
  new Cubone(),
  new Dottler(),
  new Doublade(),
  new Drampa(),
  new Durant(),
  new Electabuzz(),
  new Electivire(),
  new Emboar(),
  new EmpoleonV(),
  new Entei(),
  new Espurr(),
  new Falinks(),
  new Fearow(),
  new FlappleV(),
  new FlappleVMAX(),
  new Fomantis(),
  new Frillish(),
  new GalarianMrMime(),
  new GalarianMrRime(),
  new GalarianSlowbro(),
  new GalarianSlowpoke(),
  new Glameow(),
  new Gligar(),
  new Gliscor(),
  new Golbat(),
  new Grumpig(),
  new Gurdurr(),
  new Heatmor(),
  new Honchkrow(),
  new Honedge(),
  new Horsea(),
  new Houndoom(),
  new Houndour(),
  new Indeedee(),
  new Jellicent(),
  new Kingdra(),
  new KricketuneV(),
  new Lickilicky(),
  new Lickitung(),
  new Lurantis(),
  new Luxio(),
  new Luxray(),
  new Mankey(),
  new Marowak(),
  new Mawile(),
  new Meowstic(),
  new Mienfoo(),
  new Mienshao(),
  new MimikyuV(),
  new Morpeko(),
  new Murkrow(),
  new NecrozmaV(),
  new Octillery(),
  new Onix(),
  new Orbeetle(),
  new Pachirisu(),
  new Pawniard(),
  new Pignite(),
  new Primeape(),
  new Purugly(),
  new RapidStrikeUrshifuV(),
  new RapidStrikeUrshifuVMAX(),
  new Remoraid(),
  new Rolycoly(),
  new Salandit(),
  new Salazzle(),
  new Sandaconda(),
  new Scatterbug(),
  new Seadra(),
  new Shinx(),
  new Silicobra(),
  new SingleStrikeUrshifuV(),
  new SingleStrikeUrshifuVMAX(),
  new Sizzlipede(),
  new Spewpa(),
  new Spoink(),
  new Steelix(),
  new Stonjourner(),
  new StoutlandV(),
  new TapuBulu(),
  new TapuKokoV(),
  new TapuKokoVMAX(),
  new TyranitarV(),
  new VictiniV(),
  new VictiniVMAX(),
  new Victreebel(),
  new Vivillon(),
  new Weepinbell(),
  new Yamper(),
  new Zubat(),

  // Trainers
  new Bruno(),
  new CampingGear(),
  new Cheryl(),
  new EnergyRecycler(),
  new EscapeRope(),
  new FanOfWaves(),
  new KorrinasFocus(),
  new LevelBall(),
  new Phoebe(),
  new RapidStrikeScrollOfSwirls(),
  new RapidStrikeStyleMustard(),
  new SingleStrikeScrollOfScorn(),
  new SingleStrikeStyleMustard(),
  new SordwardAndShielbert(),
  new ToolJammer(),
  new TowerOfDarkness(),
  new TowerOfWaters(),
  new UrnOfVitality(),

  // Energy
  new RapidStrikeEnergy(),
  new SingleStrikeEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new ExpShareBST(),
  new KricketuneV2BST(),
  new FlappleV2BST(),
  new VictiniV2BST(),
  new EmpoleonV2BST(),
  new EmpoleonV3BST(),
  new TapuKokoV2BST(),
  new MimikyuV2BST(),
  new NecrozmaV2BST(),
  new SingleStrikeUrshifuV2BST(),
  new SingleStrikeUrshifuV3BST(),
  new RapidStrikeUrshifuV2BST(),
  new RapidStrikeUrshifuV3BST(),
  new TyranitarV2BST(),
  new TyranitarV3BST(),
  new StoutlandV2BST(),
  new Bruno2BST(),
  new Cheryl2BST(),
  new KorrinasFocus2BST(),
  new SingleStrikeStyleMustard2BST(),
  new FlappleVMAX2BST(),
  new VictiniVMAX2BST(),
  new TapuKokoVMAX2BST(),
  new SingleStrikeUrshifuVMAX2BST(),
  new SingleStrikeUrshifuVMAX3BST(),
  new RapidStrikeUrshifuVMAX2BST(),
  new RapidStrikeUrshifuVMAX3BST(),
  new Bruno3BST(),
  new Cheryl3BST(),
  new KorrinasFocus3BST(),
  new SingleStrikeStyleMustard3BST(),
  new Octillery2BST(),
  new Houndoom2BST(),
  new ExpShare2BST(),
  new LevelBall2BST(),
  new RapidStrikeEnergy2BST(),
  new SingleStrikeEnergy2BST(),
  new TepigBST(),
  new TimburrBST(),
  new SpearowBST(),
  new CorviknightV2(),
  new Phoebe2(),
  new RapidStrikeStyleMustard2(),
  new CorviknightVmax2(),
  new Phoebe3(),
  new RapidStrikeStyleMustard3(),
];
