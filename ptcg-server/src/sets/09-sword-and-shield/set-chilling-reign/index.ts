import { Card } from '../../../game/store/card/card';
import { Abomasnow } from './abomasnow';
import { Aggron } from './aggron';
import { Ampharos } from './ampharos';
import { Aron } from './aron';
import { Banette } from './banette';
import { Beedrill } from './beedrill';
import { BlazikenV } from './blaziken-v';
import { BlazikenVMAX } from './blaziken-vmax';
import { BlisseyV } from './blissey-v';
import { Blitzle } from './blitzle';
import { Bounsweet } from './bounsweet';
import { Castform } from './castform';
import { CastformRainyForm } from './castform-rainy-form';
import { CastformSnowyForm } from './castform-snowy-form';
import { CastformSunnyForm } from './castform-sunny-form';
import { CelebiV } from './celebi-v';
import { CelebiVmax } from './celebi-vmax';
import { Cinderace } from './cinderace';
import { Clobbopus } from './clobbopus';
import { Cobalion } from './cobalion';
import { Crabominable } from './crabominable';
import { Crabrawler } from './crabrawler';
import { Cresselia } from './cresselia';
import { Deerling } from './deerling';
import { Delibird } from './delibird';
import { Diglett } from './diglett';
import { Drizzile } from './drizzile';
import { Dugtrio } from './dugtrio';
import { Electrode } from './electrode';
import { Flaaffy } from './flaaffy';
import { Froslass } from './froslass';
import { Furfrou } from './furfrou';
import { GalarianArticunoV } from './galarian-articuno-v';
import { GalarianFarfetchd } from './galarian-farfetchd';
import { GalarianMoltresV } from './galarian-moltres-v';
import { GalarianRapidashV } from './galarian-rapidash-v';
import { GalarianRunerigus } from './galarian-runerigus';
import { GalarianSirfetchd } from './galarian-sirfetchd';
import { GalarianSlowking } from './galarian-slowking';
import { GalarianSlowkingV } from './galarian-slowking-v';
import { GalarianSlowkingVmax } from './galarian-slowking-vmax';
import { GalarianSlowpoke } from './galarian-slowpoke';
import { GalarianWeezing } from './galarian-weezing';
import { GalarianYamask } from './galarian-yamask';
import { GalarianZapdosV } from './galarian-zapdos-v';
import { Gallade } from './gallade';
import { Gardevoir } from './gardevoir';
import { Gastly } from './gastly';
import { Gengar } from './gengar';
import { Golett } from './golett';
import { Golurk } from './golurk';
import { Grapploct } from './grapploct';
import { Greedent } from './greedent';
import { Grookey } from './grookey';
import { Hatenna } from './hatenna';
import { Hatterene } from './hatterene';
import { Hattrem } from './hattrem';
import { Haunter } from './haunter';
import { Heracross } from './heracross';
import { IceRiderCalyrexV } from './ice-rider-calyrex-v';
import { IceRiderCalyrexVMAX } from './ice-rider-calyrex-vmax';
import { Inkay } from './inkay';
import { Inteleon } from './inteleon';
import { Kakuna } from './kakuna';
import { Kecleon } from './kecleon';
import { Kirlia } from './kirlia';
import { Kubfu } from './kubfu';
import { Lairon } from './lairon';
import { Lapras } from './lapras';
import { Larvesta } from './larvesta';
import { Ledian } from './ledian';
import { Ledyba } from './ledyba';
import { LiepardV } from './liepard-v';
import { Lycanroc } from './lycanroc';
import { Malamar } from './malamar';
import { Mareep } from './mareep';
import { MetagrossV } from './metagross-v';
import { MetagrossVmax } from './metagross-vmax';
import { Passimian } from './passimian';
import { Porygon } from './porygon';
import { PorygonZ } from './porygon-z';
import { Porygon2 } from './porygon2';
import { Qwilfish } from './qwilfish';
import { Raboot } from './raboot';
import { Ralts } from './ralts';
import { RapidStrikeUrshifu } from './rapid-strike-urshifu';
import { Rillaboom } from './rillaboom';
import { Rockruff } from './rockruff';
import { SandacondaV } from './sandaconda-v';
import { SandacondaVmax } from './sandaconda-vmax';
import { Sawsbuck } from './sawsbuck';
import { Scolipede } from './scolipede';
import { Scorbunny } from './scorbunny';
import { Sealeo } from './sealeo';
import { Seviper } from './seviper';
import { ShadowRiderCalyrexV } from './shadow-rider-calyrex-v';
import { ShadowRiderCalyrexVMAX } from './shadow-rider-calyrex-vmax';
import { Shaymin } from './shaymin';
import { Shuppet } from './shuppet';
import { SingleStrikeUrshifu } from './single-strike-urshifu';
import { Skwovet } from './skwovet';
import { Slurpuff } from './slurpuff';
import { Sneasel } from './sneasel';
import { Snorunt } from './snorunt';
import { Snover } from './snover';
import { Sobble } from './sobble';
import { Spheal } from './spheal';
import { Spiritomb } from './spiritomb';
import { Steenee } from './steenee';
import { Swirlix } from './swirlix';
import { TapuFini } from './tapu-fini';
import { Tauros } from './tauros';
import { Thundurus } from './thundurus';
import { Thwackey } from './thwackey';
import { TornadusV } from './tornadus-v';
import { TornadusVmax } from './tornadus-vmax';
import { Tsareena } from './tsareena';
import { VolcanionV } from './volcanion-v';
import { Volcarona } from './volcarona';
import { Walrein } from './walrein';
import { Weavile } from './weavile';
import { WeedleCRE } from './weedle';
import { Weezing } from './weezing';
import { Whirlipede } from './whirlipede';
import { Zangoose } from './zangoose';
import { Zarude } from './zarude';
import { Zebstrika } from './zebstrika';
import { ZeraoraV } from './zeraora-v';
import { Agatha } from './agatha';
import { Avery } from './avery';
import { Brawly } from './brawly';
import { Caitlin } from './caitlin';
import { CrushingGloves } from './crushing-gloves';
import { Doctor } from './doctor';
import { DynaTreeHill } from './dyna-tree-hill';
import { EchoingHorn } from './echoing-horn';
import { ExpeditionUniform } from './expedition-uniform';
import { FireResistantGloves } from './fire-resistant-gloves';
import { Flannery } from './flannery';
import { FogCrystal } from './fog-crystal';
import { GalarianChestplate } from './galarian-chestplate';
import { Honey } from './honey';
import { JustifiedGloves } from './justified-gloves';
import { KarensConviction } from './karens-conviction';
import { Klara } from './klara';
import { Melony } from './melony';
import { OldCemetery } from './old-cemetery';
import { PathToThePeak } from './path-to-the-peak';
import { Peonia } from './peonia';
import { Peony } from './peony';
import { RapidStrikeScrollOfTheSkies } from './rapid-strike-scroll-of-the-skies';
import { RuggedHelmet } from './rugged-helmet';
import { Siebold } from './siebold';
import { SingleStrikeScrollOfPiercing } from './single-strike-scroll-of-piercing';
import { WeedingGloves } from './weeding-gloves';
import { WelcomingLantern } from './welcoming-lantern';
import { ImpactEnergy } from './impact-energy';
import { LuckyEnergy } from './lucky-energy';
import { SpiralEnergy } from './spiral-energy';

import {
  BronzongCRE,
  BlazikenV2CRE,
  IceRiderCalyrexV2CRE,
  IceRiderCalyrexV3CRE,
  GalarianArticunoV2CRE,
  GalarianArticunoV3CRE,
  ShadowRiderCalyrexV2CRE,
  ShadowRiderCalyrexV3CRE,
  GalarianZapdosV2CRE,
  GalarianZapdosV3CRE,
  GalarianMoltresV2CRE,
  GalarianMoltresV3CRE,
  BlisseyV2CRE,
  BlisseyV3CRE,
  Agatha2CRE,
  Avery2CRE,
  Brawly2CRE,
  Caitlin2CRE,
  Flannery2CRE,
  KarensConviction2CRE,
  Klara2CRE,
  Melony2CRE,
  Peonia2CRE,
  Peony2CRE,
  BlazikenVMAX2CRE,
  BlazikenVMAX3CRE,
  IceRiderCalyrexVMAX2CRE,
  IceRiderCalyrexVMAX3CRE,
  ShadowRiderCalyrexVMAX2CRE,
  ShadowRiderCalyrexVMAX3CRE,
  Agatha3CRE,
  Avery3CRE,
  Brawly3CRE,
  Caitlin3CRE,
  Flannery3CRE,
  KarensConviction3CRE,
  Klara3CRE,
  Melony3CRE,
  Peonia3CRE,
  Peony3CRE,
  SnorlaxCRE,
  EchoingHorn2CRE,
  FanOfWavesCRE,
  FogCrystal2CRE,
  UrnOfVitalityCRE,
  WaterEnergyCRE,
  PsychicEnergyCRE,
  FightingEnergyCRE,
  KoffingCRE,
  VenipedeCRE,
  CelebiV2,
  VolcanionV2,
  ZeraoraV2,
  ZeraoraV3,
  GalarianRapidashV2,
  SandacondaV2,
  GalarianSlowkingV2,
  GalarianSlowkingV3,
  LiepardV2,
  MetagrossV2,
  TornadusV2,
  TornadusV3,
  Doctor2,
  Honey2,
  Siebold2,
  CelebiVmax2,
  SandacondaVmax2,
  GalarianSlowkingVmax2,
  MetagrossVmax2,
  TornadusVmax2,
  Doctor3,
  Siebold3,
  RuggedHelmet2,
  WelcomingLantern2,
} from './other-prints';

export const setChillingReign: Card[] = [
  // Pokemon
  new Abomasnow(),
  new Aggron(),
  new Ampharos(),
  new Aron(),
  new Banette(),
  new Beedrill(),
  new BlazikenV(),
  new BlazikenVMAX(),
  new BlisseyV(),
  new Blitzle(),
  new Bounsweet(),
  new Castform(),
  new CastformRainyForm(),
  new CastformSnowyForm(),
  new CastformSunnyForm(),
  new CelebiV(),
  new CelebiVmax(),
  new Cinderace(),
  new Clobbopus(),
  new Cobalion(),
  new Crabominable(),
  new Crabrawler(),
  new Cresselia(),
  new Deerling(),
  new Delibird(),
  new Diglett(),
  new Drizzile(),
  new Dugtrio(),
  new Electrode(),
  new Flaaffy(),
  new Froslass(),
  new Furfrou(),
  new GalarianArticunoV(),
  new GalarianFarfetchd(),
  new GalarianMoltresV(),
  new GalarianRapidashV(),
  new GalarianRunerigus(),
  new GalarianSirfetchd(),
  new GalarianSlowking(),
  new GalarianSlowkingV(),
  new GalarianSlowkingVmax(),
  new GalarianSlowpoke(),
  new GalarianWeezing(),
  new GalarianYamask(),
  new GalarianZapdosV(),
  new Gallade(),
  new Gardevoir(),
  new Gastly(),
  new Gengar(),
  new Golett(),
  new Golurk(),
  new Grapploct(),
  new Greedent(),
  new Grookey(),
  new Hatenna(),
  new Hatterene(),
  new Hattrem(),
  new Haunter(),
  new Heracross(),
  new IceRiderCalyrexV(),
  new IceRiderCalyrexVMAX(),
  new Inkay(),
  new Inteleon(),
  new Kakuna(),
  new Kecleon(),
  new Kirlia(),
  new Kubfu(),
  new Lairon(),
  new Lapras(),
  new Larvesta(),
  new Ledian(),
  new Ledyba(),
  new LiepardV(),
  new Lycanroc(),
  new Malamar(),
  new Mareep(),
  new MetagrossV(),
  new MetagrossVmax(),
  new Passimian(),
  new Porygon(),
  new PorygonZ(),
  new Porygon2(),
  new Qwilfish(),
  new Raboot(),
  new Ralts(),
  new RapidStrikeUrshifu(),
  new Rillaboom(),
  new Rockruff(),
  new SandacondaV(),
  new SandacondaVmax(),
  new Sawsbuck(),
  new Scolipede(),
  new Scorbunny(),
  new Sealeo(),
  new Seviper(),
  new ShadowRiderCalyrexV(),
  new ShadowRiderCalyrexVMAX(),
  new Shaymin(),
  new Shuppet(),
  new SingleStrikeUrshifu(),
  new Skwovet(),
  new Slurpuff(),
  new Sneasel(),
  new Snorunt(),
  new Snover(),
  new Sobble(),
  new Spheal(),
  new Spiritomb(),
  new Steenee(),
  new Swirlix(),
  new TapuFini(),
  new Tauros(),
  new Thundurus(),
  new Thwackey(),
  new TornadusV(),
  new TornadusVmax(),
  new Tsareena(),
  new VolcanionV(),
  new Volcarona(),
  new Walrein(),
  new Weavile(),
  new WeedleCRE(),
  new Weezing(),
  new Whirlipede(),
  new Zangoose(),
  new Zarude(),
  new Zebstrika(),
  new ZeraoraV(),

  // Trainers
  new Agatha(),
  new Avery(),
  new Brawly(),
  new Caitlin(),
  new CrushingGloves(),
  new Doctor(),
  new DynaTreeHill(),
  new EchoingHorn(),
  new ExpeditionUniform(),
  new FireResistantGloves(),
  new Flannery(),
  new FogCrystal(),
  new GalarianChestplate(),
  new Honey(),
  new JustifiedGloves(),
  new KarensConviction(),
  new Klara(),
  new Melony(),
  new OldCemetery(),
  new PathToThePeak(),
  new Peonia(),
  new Peony(),
  new RapidStrikeScrollOfTheSkies(),
  new RuggedHelmet(),
  new Siebold(),
  new SingleStrikeScrollOfPiercing(),
  new WeedingGloves(),
  new WelcomingLantern(),

  // Energy
  new ImpactEnergy(),
  new LuckyEnergy(),
  new SpiralEnergy(),

  // Other Prints (Reprints & Alt Arts)
  new BronzongCRE(),
  new BlazikenV2CRE(),
  new IceRiderCalyrexV2CRE(),
  new IceRiderCalyrexV3CRE(),
  new GalarianArticunoV2CRE(),
  new GalarianArticunoV3CRE(),
  new ShadowRiderCalyrexV2CRE(),
  new ShadowRiderCalyrexV3CRE(),
  new GalarianZapdosV2CRE(),
  new GalarianZapdosV3CRE(),
  new GalarianMoltresV2CRE(),
  new GalarianMoltresV3CRE(),
  new BlisseyV2CRE(),
  new BlisseyV3CRE(),
  new Agatha2CRE(),
  new Avery2CRE(),
  new Brawly2CRE(),
  new Caitlin2CRE(),
  new Flannery2CRE(),
  new KarensConviction2CRE(),
  new Klara2CRE(),
  new Melony2CRE(),
  new Peonia2CRE(),
  new Peony2CRE(),
  new BlazikenVMAX2CRE(),
  new BlazikenVMAX3CRE(),
  new IceRiderCalyrexVMAX2CRE(),
  new IceRiderCalyrexVMAX3CRE(),
  new ShadowRiderCalyrexVMAX2CRE(),
  new ShadowRiderCalyrexVMAX3CRE(),
  new Agatha3CRE(),
  new Avery3CRE(),
  new Brawly3CRE(),
  new Caitlin3CRE(),
  new Flannery3CRE(),
  new KarensConviction3CRE(),
  new Klara3CRE(),
  new Melony3CRE(),
  new Peonia3CRE(),
  new Peony3CRE(),
  new SnorlaxCRE(),
  new EchoingHorn2CRE(),
  new FanOfWavesCRE(),
  new FogCrystal2CRE(),
  new UrnOfVitalityCRE(),
  new WaterEnergyCRE(),
  new PsychicEnergyCRE(),
  new FightingEnergyCRE(),
  new KoffingCRE(),
  new VenipedeCRE(),
  new CelebiV2(),
  new VolcanionV2(),
  new ZeraoraV2(),
  new ZeraoraV3(),
  new GalarianRapidashV2(),
  new SandacondaV2(),
  new GalarianSlowkingV2(),
  new GalarianSlowkingV3(),
  new LiepardV2(),
  new MetagrossV2(),
  new TornadusV2(),
  new TornadusV3(),
  new Doctor2(),
  new Honey2(),
  new Siebold2(),
  new CelebiVmax2(),
  new SandacondaVmax2(),
  new GalarianSlowkingVmax2(),
  new MetagrossVmax2(),
  new TornadusVmax2(),
  new Doctor3(),
  new Siebold3(),
  new RuggedHelmet2(),
  new WelcomingLantern2(),
];
