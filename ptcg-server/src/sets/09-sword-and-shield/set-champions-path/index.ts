import { Card } from '../../../game/store/card/card';
import { Absol } from './absol';
import { AlcremieV } from './alcremie-v';
import { AlcremieVmax } from './alcremie-vmax';
import { Altaria } from './altaria';
import { Arbok } from './arbok';
import { Beedrill } from './beedrill';
import { Carvanha } from './carvanha';
import { Centiskorch } from './centiskorch';
import { DrednawV } from './drednaw-v';
import { DrednawVmax } from './drednaw-vmax';
import { DuraludonV } from './duraludon-v';
import { Ekans } from './ekans';
import { EldegossV } from './eldegoss-v';
import { GalarianCursolaV } from './galarian-cursola-v';
import { GalarianLinoone } from './galarian-linoone';
import { GalarianObstagoon } from './galarian-obstagoon';
import { GalarianZigzagoon } from './galarian-zigzagoon';
import { GardevoirV } from './gardevoir-v';
import { GardevoirVmax } from './gardevoir-vmax';
import { GrapploctV } from './grapploct-v';
import { Hatenna } from './hatenna';
import { Hatterene } from './hatterene';
import { Hattrem } from './hattrem';
import { IncineroarV } from './incineroar-v';
import { Inkay } from './inkay';
import { Kakuna } from './kakuna';
import { Liepard } from './liepard';
import { LucarioV } from './lucario-v';
import { Lycanroc } from './lycanroc';
import { Machamp } from './machamp';
import { Machoke } from './machoke';
import { Machop } from './machop';
import { Malamar } from './malamar';
import { Nickit } from './nickit';
import { Rockruff } from './rockruff';
import { Rolycoly } from './rolycoly';
import { Scrafty } from './scrafty';
import { Scraggy } from './scraggy';
import { Sharpedo } from './sharpedo';
import { Swablu } from './swablu';
import { VenusaurV } from './venusaur-v';
import { Victini } from './victini';
import { Vulpix } from './vulpix';
import { WailordV } from './wailord-v';
import { Weedle } from './weedle';
import { Zygarde } from './zygarde';
import { HyperPotion } from './hyper-potion';
import { Piers } from './piers';
import { RotomPhone } from './rotom-phone';
import { Sonia } from './sonia';
import { SuspiciousFoodTin } from './suspicious-food-tin';
import { TurffieldStadium } from './turffield-stadium';

import {
  MarnieCPA,
  SizzlipedeCPA,
  PurrloinCPA,
  BedeCPA,
  FullHealCPA,
  GreatBallCPA,
  HopCPA,
  PokeBallCPA,
  PotionCPA,
  ProfessorsResearchCPA,
  Hop2CPA,
  Piers2CPA,
  TrubbishCPA,
  KabuCPA,
  MiloCPA,
  PokemonCenterLadyCPA,
  RotomBikeCPA,
  TeamYellGruntCPA,
  DrednawV2,
  GardevoirV2,
  GalarianCursolaV2,
  GrapploctV2,
  CharizardVmaxCPA,
  DrednawVmax2,
  GardevoirVmax2,
  Kabu2,
  CharizardVCPA,
  SuspiciousFoodTin2,
} from './other-prints';

export const setChampionsPath: Card[] = [
  // Pokemon
  new Absol(),
  new AlcremieV(),
  new AlcremieVmax(),
  new Altaria(),
  new Arbok(),
  new Beedrill(),
  new Carvanha(),
  new Centiskorch(),
  new DrednawV(),
  new DrednawVmax(),
  new DuraludonV(),
  new Ekans(),
  new EldegossV(),
  new GalarianCursolaV(),
  new GalarianLinoone(),
  new GalarianObstagoon(),
  new GalarianZigzagoon(),
  new GardevoirV(),
  new GardevoirVmax(),
  new GrapploctV(),
  new Hatenna(),
  new Hatterene(),
  new Hattrem(),
  new IncineroarV(),
  new Inkay(),
  new Kakuna(),
  new Liepard(),
  new LucarioV(),
  new Lycanroc(),
  new Machamp(),
  new Machoke(),
  new Machop(),
  new Malamar(),
  new Nickit(),
  new Rockruff(),
  new Rolycoly(),
  new Scrafty(),
  new Scraggy(),
  new Sharpedo(),
  new Swablu(),
  new VenusaurV(),
  new Victini(),
  new Vulpix(),
  new WailordV(),
  new Weedle(),
  new Zygarde(),

  // Trainers
  new HyperPotion(),
  new Piers(),
  new RotomPhone(),
  new Sonia(),
  new SuspiciousFoodTin(),
  new TurffieldStadium(),

  // Other Prints (Reprints & Alt Arts)
  new MarnieCPA(),
  new SizzlipedeCPA(),
  new PurrloinCPA(),
  new BedeCPA(),
  new FullHealCPA(),
  new GreatBallCPA(),
  new HopCPA(),
  new PokeBallCPA(),
  new PotionCPA(),
  new ProfessorsResearchCPA(),
  new Hop2CPA(),
  new Piers2CPA(),
  new TrubbishCPA(),
  new KabuCPA(),
  new MiloCPA(),
  new PokemonCenterLadyCPA(),
  new RotomBikeCPA(),
  new TeamYellGruntCPA(),
  new DrednawV2(),
  new GardevoirV2(),
  new GalarianCursolaV2(),
  new GrapploctV2(),
  new CharizardVmaxCPA(),
  new DrednawVmax2(),
  new GardevoirVmax2(),
  new Kabu2(),
  new CharizardVCPA(),
  new SuspiciousFoodTin2(),
];
