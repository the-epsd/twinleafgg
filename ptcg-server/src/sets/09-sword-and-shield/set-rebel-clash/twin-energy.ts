...
If this card is attached to a Pokémon V or a Pokémon-GX, it provides [C] Energy instead.`;
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof CheckProvidedEnergyEffect && effect.source.cards.includes(this)) {
      if (
        effect.source.getPokemonCard()!.hasTag(CardTag.POKEMON_GX) ||
        effect.source.getPokemonCard()!.hasTag(CardTag.POKEMON_V) ||
        effect.source.getPokemonCard()!.hasTag(CardTag.POKEMON_VSTAR) ||
        effect.source.getPokemonCard()!.hasTag(CardTag.POKEMON_VMAX)
      ) {
