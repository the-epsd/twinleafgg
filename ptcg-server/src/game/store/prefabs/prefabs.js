...
var AFTER_ATTACK = function (effect) {
...
function ABILITY_USED(player, card) {
    player.forEachPokemon(__1.PlayerType.BOTTOM_PLAYER, function (cardList) {
...
function DISCARD_A_STADIUM_CARD_IN_PLAY(state) {
    var stadiumCard = __1.StateUtils.getStadiumCard(state);
    if (stadiumCard !== undefined) {
        var cardList = __1.StateUtils.findCardList(state, stadiumCard);
        var player = __1.StateUtils.findOwner(state, cardList);
...
function SEARCH_YOUR_DECK_FOR_POKEMON_AND_PUT_ONTO_BENCH(store, state, player, filter, options) {
    ...
    var slots = GET_PLAYER_BENCH_SLOTS(player);
    BLOCK_IF_NO_SLOTS(slots);
    filter.superType = card_types_1.SuperType.POKEMON;
    return store.prompt(state, new __1.ChooseCardsPrompt(player, __1.GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, player.deck, filter, options), function (selected) {
        var cards = selected || [];
...
function SEARCH_YOUR_DECK_FOR_POKEMON_AND_PUT_INTO_HAND(store, state, player, filter, options) {
    ...
    if (options === void 0) { options = {}; }
    BLOCK_IF_DECK_EMPTY(player);
    var opponent = __1.StateUtils.getOpponent(state, player);
    filter.superType = card_types_1.SuperType.POKEMON;
    return store.prompt(state, new __1.ChooseCardsPrompt(player, __1.GameMessage.CHOOSE_CARD_TO_HAND, player.deck, filter, options), function (selected) {
        var cards = selected || [];
        SHOW_CARDS_TO_PLAYER(store, state, opponent, cards);
        cards.forEach(function (card) { return MOVE_CARD_TO(state, card, player.hand); });
        SHUFFLE_DECK(store, state, player);
    });
}
...
function GET_TOTAL_ENERGY_ATTACHED_TO_PLAYERS_POKEMON(player, store, state) {
    var totalEnergy = 0;
    player.forEachPokemon(__1.PlayerType.BOTTOM_PLAYER, function (cardList, card) {
        var checkProvidedEnergyEffect = new check_effects_1.CheckProvidedEnergyEffect(player, cardList);
        store.reduceEffect(state, checkProvidedEnergyEffect);
        checkProvidedEnergyEffect.energyMap.forEach(function (energy) {
            totalEnergy += 1;
        });
    });
    return totalEnergy;
...
function DEAL_MORE_DAMAGE_IF_OPPONENT_ACTIVE_HAS_CARD_TAG(effect, state, damage) {
    var cardTags = [];
    for (var _i = 3; _i < arguments.length; _i++) {
        cardTags[_i - 3] = arguments[_i];
    }
    var opponent = __1.StateUtils.getOpponent(state, effect.player);
    var opponentActive = opponent.active.getPokemonCard();
    var includesAnyTags = false;
    for (var _a = 0, cardTags_1 = cardTags; _a < cardTags_1.length; _a++) {
        var tag = cardTags_1[_a];
...
function HEAL_X_DAMAGE_FROM_THIS_POKEMON(effect, store, state, damage) {
    var player = effect.player;
    var healTargetEffect = new attack_effects_1.HealTargetEffect(effect, damage);
...
function TAKE_SPECIFIC_PRIZES(store, state, player, prizes, options) {
    if (options === void 0) { options = {}; }
    var _a = options.destination, destination = _a === void 0 ? player.hand : _a, _b = options.skipReduce, skipReduce = _b === void 0 ? false : _b;
    var preventDefault;
    if (!skipReduce) {
        var drawPrizesEffect = new game_effects_1.DrawPrizesEffect(player, prizes, destination);
        ...
        var prizesDestinationEffect = new check_effects_1.CheckPrizesDestinationEffect(player, drawPrizesEffect.destination);
...
function TAKE_X_PRIZES(store, state, player, count, options, callback) {
    if (options === void 0) { options = {}; }
    var _a = options.promptOptions, promptOptions = _a === void 0 ? {} : _a, takeOptions = __rest(options, ["promptOptions"]);
    state = store.prompt(state, new __1.ChoosePrizePrompt(player.id, __1.GameMessage.CHOOSE_PRIZE_CARD, __assign({ count: count, allowCancel: false }, promptOptions)), function (result) {
...
function PLAY_POKEMON_FROM_HAND_TO_BENCH(state, player, card) {
    var slot = GET_FIRST_PLAYER_BENCH_SLOT(player);
...
function THIS_ATTACK_DOES_X_DAMAGE_TO_X_OF_YOUR_OPPONENTS_BENCHED_POKEMON(damage, effect, store, state, min, max) {
    var player = effect.player;
    var opponent = __1.StateUtils.getOpponent(state, player);
    var targets = opponent.bench.filter(function (b) { return b.cards.length > 0; });
    if (targets.length === 0) {
        return state;
    }
    return store.prompt(state, new __1.ChoosePokemonPrompt(player.id, __1.GameMessage.CHOOSE_POKEMON_TO_DAMAGE, __1.PlayerType.TOP_PLAYER, [__1.SlotType.BENCH], { min: min, max: max, allowCancel: false }), function (selected) {
        var target = selected[0];
        var damageEffect = new attack_effects_1.PutDamageEffect(effect, damage);
...
function THIS_POKEMON_DOES_DAMAGE_TO_ITSELF(store, state, effect, amount) {
    var dealDamage = new attack_effects_1.DealDamageEffect(effect, amount);
...
function ATTACH_ENERGY_PROMPT(store, state, player, playerType, sourceSlot, destinationSlots, filter, options) {
    ...
    var source = player.getSlot(sourceSlot);
    return store.prompt(state, new __1.AttachEnergyPrompt(player.id, __1.GameMessage.ATTACH_ENERGY_CARDS, source, playerType, destinationSlots, filter, options), function (transfers) {
        transfers = transfers || [];
        for (var _i = 0, transfers_1 = transfers; _i < transfers_1.length; _i++) {
            var transfer = transfers_1[_i];
            var target = __1.StateUtils.getTarget(state, player, transfer.to);
            var energyCard = transfer.card;
            var attachEnergyEffect = new play_card_effects_1.AttachEnergyEffect(player, energyCard, target);
...
function DISCARD_X_ENERGY_FROM_YOUR_HAND(effect, store, state, minAmount, maxAmount) {
    var player = effect.player;
    var hasEnergyInHand = player.hand.cards.some(function (c) {
        return c instanceof __1.EnergyCard;
    });
    if (!hasEnergyInHand) {
        throw new __1.GameError(__1.GameMessage.CANNOT_USE_POWER);
    }
    return store.prompt(state, new __1.ChooseCardsPrompt(player, __1.GameMessage.CHOOSE_CARD_TO_DISCARD, player.hand, { superType: card_types_1.SuperType.ENERGY }, { allowCancel: false, min: minAmount, max: maxAmount }), function (cards) {
        cards = cards || [];
        if (cards.length === 0) {
            return;
        }
...
function DISCARD_ALL_ENERGY_FROM_POKEMON(store, state, effect, card) {
    var player = effect.player;
    var cardList = __1.StateUtils.findCardList(state, card);
    if (!(cardList instanceof __1.PokemonCardList))
        throw new __1.GameError(__1.GameMessage.INVALID_TARGET);
    var checkProvidedEnergy = new check_effects_1.CheckProvidedEnergyEffect(player);
    state = store.reduceEffect(state, checkProvidedEnergy);
    var cards = checkProvidedEnergy.energyMap.map(function (e) { return e.card; });
    var discardEnergy = new attack_effects_1.DiscardCardsEffect(effect, cards);
...
function GET_PLAYER_PRIZES(player) {
    return player.prizes.filter(function (p) { return p.cards.length > 0; });
}
...
function GET_PRIZES_AS_CARD_ARRAY(player) {
    var prizes = player.prizes.filter(function (p) { return p.cards.length > 0; });
    var allPrizeCards = [];
    prizes.forEach(function (p) { return allPrizeCards.push.apply(allPrizeCards, p.cards); });
...
function SHUFFLE_DECK(store, state, player) {
    return store.prompt(state, new __1.ShuffleDeckPrompt(player.id), function (order) { return player.deck.applyOrder(order); });
}
...
function SHUFFLE_PRIZES_INTO_DECK(store, state, player) {
    ...
    GET_PLAYER_PRIZES(player).forEach(function (p) { return p.cards = []; });
}
...
function DRAW_CARDS_AS_FACE_DOWN_PRIZES(player, count) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        if (card) {
            var prize = player.prizes.find(function (p) { return p.cards.length === 0; });
            if (prize) {
    ...
    player.prizes.forEach(function (p) { return p.isSecret = true; });
}
exports.DRAW_CARDS_AS_FACE_DOWN_PRIZES = DRAW_CARDS_AS_FACE_DOWN_PRIZES;
function SEARCH_DECK_FOR_CARDS_TO_HAND(store, state, player, min, max) {
    if (min === void 0) { min = 0; }
    if (max === void 0) { max = 1; }
    if (player.deck.cards.length === 0)
        return;
    var cards = [];
    store.prompt(state, new __1.ChooseCardsPrompt(player, __1.GameMessage.CHOOSE_CARD_TO_HAND, player.deck, {}, { min: 0, max: 3, allowCancel: false }), function (selected) {
        cards = selected || [];
...
function GET_CARDS_ON_BOTTOM_OF_DECK(player, amount) {
    if (amount === void 0) { amount = 1; }
    var start = player.deck.cards.length < amount ? 0 : player.deck.cards.length - amount;
    var end = player.deck.cards.length;
...
function IS_TOOL_BLOCKED(store, state, player, card) {
    ...
    try {
        var stub = new play_card_effects_1.ToolEffect(player, card);
...
function SWITCH_ACTIVE_WITH_BENCHED(store, state, player) {
    var hasBenched = player.bench.some(function (b) { return b.cards.length > 0; });
    if (!hasBenched)
        return state;
    return store.prompt(state, new __1.ChoosePokemonPrompt(player.id, __1.GameMessage.CHOOSE_NEW_ACTIVE_POKEMON, __1.PlayerType.BOTTOM_PLAYER, [__1.SlotType.BENCH], { allowCancel: false }), function (selected) {
...
function LOOK_AT_TOPDECK_AND_DISCARD_OR_RETURN(store, state, choosingPlayer, deckPlayer) {
    {
        ...
        var deckTop_1 = new __1.CardList();
        deckPlayer.deck.moveTo(deckTop_1, 1);
        SHOW_CARDS_TO_PLAYER(store, state, choosingPlayer, deckTop_1.cards);
        SELECT_PROMPT_WITH_OPTIONS(store, state, choosingPlayer, __1.GameMessage.CHOOSE_OPTION, [{
                ...
                action: function () { return deckTop_1.moveToTopOfDestination(deckPlayer.discard); }
            },
            {
                message: __1.GameMessage.RETURN_TO_TOP_OF_DECK,
                action: function () { return deckTop_1.moveToTopOfDestination(deckPlayer.deck); }
            }]);
    }
}
...
function SHOW_CARDS_TO_PLAYER(store, state, player, cards) {
    if (cards.length === 0)
        return state;
    return store.prompt(state, new __1.ShowCardsPrompt(player.id, __1.GameMessage.CARDS_SHOWED_BY_THE_OPPONENT, cards), function () { });
}
...
function SELECT_PROMPT_WITH_OPTIONS(store, state, player, message, options) {
    return store.prompt(state, new __1.SelectPrompt(player.id, message, options.map(function (opt) { return opt.message; }), { allowCancel: false }), function (choice) {
        var option = options[choice];
        option.action();
    });
}
exports.SELECT_PROMPT_WITH_OPTIONS = SELECT_PROMPT_WITH_OPTIONS;
function CONFIRMATION_PROMPT(store, state, player, callback, message) {
    if (message === void 0) { message = __1.GameMessage.WANT_TO_USE_ABILITY; }
...
function MULTIPLE_COIN_FLIPS_PROMPT(store, state, player, amount, callback) {
    var prompts = new Array(amount).fill(0).map(function (_) { return new __1.CoinFlipPrompt(player.id, __1.GameMessage.COIN_FLIP); });
...
function SIMULATE_COIN_FLIP(store, state, player) {
    ...
    var gameMessage = result ? __1.GameLog.LOG_PLAYER_FLIPS_HEADS : __1.GameLog.LOG_PLAYER_FLIPS_TAILS;
...
function GET_PLAYER_BENCH_SLOTS(player) {
    return player.bench.filter(function (b) { return b.cards.length === 0; });
}
...
function ADD_SPECIAL_CONDITIONS_TO_PLAYER_ACTIVE(store, state, player, source, specialConditions, poisonDamage, burnDamage, sleepFlips) {
...
function ADD_SLEEP_TO_PLAYER_ACTIVE(store, state, player, source, sleepFlips) {
    if (sleepFlips === void 0) { sleepFlips = 1; }
...
function ADD_POISON_TO_PLAYER_ACTIVE(store, state, player, source, poisonDamage) {
    if (poisonDamage === void 0) { poisonDamage = 10; }
...
function ADD_BURN_TO_PLAYER_ACTIVE(store, state, player, source, burnDamage) {
    if (burnDamage === void 0) { burnDamage = 20; }
...
function CLEAR_MARKER_AND_OPPONENTS_POKEMON_MARKER_AT_END_OF_TURN(state, effect, clearerMarker, pokemonMarker, source) {
    if (effect instanceof game_phase_effects_1.EndTurnEffect && HAS_MARKER(clearerMarker, effect.player, source)) {
        ...
        opponent.forEachPokemon(__1.PlayerType.TOP_PLAYER, function (cardList) { return REMOVE_MARKER(pokemonMarker, cardList, source); });
    }
}
