...
var pokemon_card_list_1 = require("./pokemon-card-list");
var Player = /** @class */ (function () {
    ...
    Player.prototype.getPrizeLeft = function () {
        return this.prizes.reduce(function (left, p) { return left + p.cards.length; }, 0);
    };
    Player.prototype.forEachPokemon = function (player, handler) {
        var pokemonCard = this.active.getPokemonCard();
        var target;
        if (pokemonCard !== undefined) {
            target = { player: player, slot: play_card_action_1.SlotType.ACTIVE, index: 0 };
            handler(this.active, pokemonCard, target);
        }
        for (var i = 0; i < this.bench.length; i++) {
            pokemonCard = this.bench[i].getPokemonCard();
            if (pokemonCard !== undefined) {
                target = { player: player, slot: play_card_action_1.SlotType.BENCH, index: i };
                handler(this.bench[i], pokemonCard, target);
            }
        }
    };
    Player.prototype.removePokemonEffects = function (target) {
    ...
    Player.prototype.getPokemonInPlay = function () {
        var list = [];
        this.forEachPokemon(play_card_action_1.PlayerType.BOTTOM_PLAYER, function (cardList, pokemonCard, target) {
            if (cardList.cards.length !== 0)
                list.push(cardList);
        });
        return list;
    };
    Player.prototype.vPokemon = function () {
        var result = false;
        this.forEachPokemon(play_card_action_1.PlayerType.BOTTOM_PLAYER, function (cardList, pokemonCard, target) {
    ...
    Player.prototype.singleStrike = function () {
        var result = false;
        this.forEachPokemon(play_card_action_1.PlayerType.BOTTOM_PLAYER, function (cardList, pokemonCard, target) {
            if (cardList.getPokemons().some(function (pokemon) { return pokemon.tags.includes(card_types_1.CardTag.SINGLE_STRIKE); })) {
                result = true;
            }
        });
        return result;
    };
    Player.prototype.fusionStrike = function () {
        var result = false;
        this.forEachPokemon(play_card_action_1.PlayerType.BOTTOM_PLAYER, function (cardList, pokemonCard, target) {
            if (cardList.getPokemons().some(function (pokemon) { return pokemon.tags.includes(card_types_1.CardTag.FUSION_STRIKE); })) {
                result = true;
            }
        });
        return result;
    };
    Player.prototype.rapidStrike = function () {
        var result = false;
        this.forEachPokemon(play_card_action_1.PlayerType.BOTTOM_PLAYER, function (cardList, pokemonCard, target) {
            if (cardList.getPokemons().some(function (pokemon) { return pokemon.tags.includes(card_types_1.CardTag.RAPID_STRIKE); })) {
                result = true;
            }
        });
        return result;
    };
    Player.prototype.getSlot = function (slotType) {
        switch (slotType) {
    ...
    Player.prototype.switchPokemon = function (target) {
        var benchIndex = this.bench.indexOf(target);
        if (benchIndex !== -1) {
