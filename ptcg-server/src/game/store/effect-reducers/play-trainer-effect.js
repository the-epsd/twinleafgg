...
function playTrainerReducer(store, state, effect) {
    ...
    if (effect instanceof play_card_effects_1.PlaySupporterEffect) {
        ...
        var playTrainer = new play_card_effects_1.TrainerEffect(effect.player, effect.trainerCard, effect.target);
    ...
    if (effect instanceof play_card_effects_1.TrainerEffect) {
        if (effect.player.hand.cards.includes(effect.trainerCard)) {
            ...
            var isSupporter = effect.trainerCard.trainerType === card_types_1.TrainerType.SUPPORTER;
            var target = isSupporter ? effect.player.supporter : effect.player.discard;
