...
function betweenTurns(store, state, onComplete) {
    ...
    if (store.hasPrompts()) {
        return store.waitPrompt(state, function () {
            check_effect_1.checkState(store, state, function () { return onComplete(); });
        });
    }
    return check_effect_1.checkState(store, state, function () { return onComplete(); });
}
...
function initNextTurn(store, state) {
    ...
    if (player.deck.cards.length === 0) {
        ...
        var winner = state.activePlayer ? state_1.GameWinner.PLAYER_1 : state_1.GameWinner.PLAYER_2;
        state = check_effect_1.endGame(store, state, winner);
        return state;
    }
    try {
        var beginTurn = new game_phase_effects_1.BeginTurnEffect(player);
    ...
    var drawnCard = player.hand.cards[player.hand.cards.length - 1];
    try {
        var drewTopdeck = new game_phase_effects_1.DrewTopdeckEffect(player, drawnCard);
...
function startNextTurn(store, state) {
    ...
    return betweenTurns(store, state, function () {
...
function handleSpecialConditions(store, state, effect) {
    ...
    for (var _i = 0, _a = player.active.specialConditions; _i < _a.length; _i++) {
        var sp = _a[_i];
        switch (sp) {
            ...
            case card_types_1.SpecialCondition.BURNED:
                ...
                store.prompt(state, new coin_flip_prompt_1.CoinFlipPrompt(player.id, game_message_1.GameMessage.FLIP_BURNED), function (result) {
                    if (result === true) {
            ...
            case card_types_1.SpecialCondition.ASLEEP:
                ...
                var flipsForSleep = [];
                for (var i = 0; i < effect.player.active.sleepFlips; i++) {
                ...
                if (flipsForSleep.length > 0) {
                    store.prompt(state, flipsForSleep, function (results) {
                        var wakesUp = Array.isArray(results) ? results.every(function (r) { return r; }) : results;
                        if (wakesUp) {
                            player.active.removeSpecialCondition(card_types_1.SpecialCondition.ASLEEP);
                        }
                    });
...
function gamePhaseReducer(store, state, effect) {
    if (effect instanceof game_phase_effects_1.EndTurnEffect) {
        ...
        state = check_effect_1.checkState(store, state, function () {
            if (state.phase === state_1.GamePhase.FINISHED) {
                return;
            }
