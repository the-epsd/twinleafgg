...
import { AttackAction, PassTurnAction, RetreatAction, RetreatStartAction, UseAbilityAction, UseEnergyAbilityAction, UseStadiumAction, UseTrainerAbilityAction } from '../store/actions/game-actions';
...
export function buildReplayActionPayload(action) {
...
export function buildActionFromReplayRecord(record, state) {
    const payload = record.payload || {};
    switch (record.type) {
        case 'ABORT_GAME':
            return new AbortGameAction(payload.culpritId, payload.reason);
        case 'ADD_PLAYER':
            return new AddPlayerAction(payload.clientId, payload.name, payload.deck, payload.artworksMap, payload.deckId);
        ...
        case 'SANDBOX_MODIFY_CARD':
            return new SandboxModifyCardAction(payload.clientId, payload.targetPlayerId, payload.action, payload.cardName, payload.fromZone, payload.toZone, payload.fromIndex, payload.toIndex, payload.prizeIndex);
        ...
        case 'SANDBOX_MODIFY_POKEMON':
            return new SandboxModifyPokemonAction(payload.clientId, payload.targetPlayerId, payload.location, payload.modifications, payload.benchIndex);
        default:
            throw new Error(`Unsupported replay action type: ${record.type}`);
    }
}
function buildResolvePromptAction(payload, state) {
    if (state !== undefined) {
    ...
    return new ResolvePromptAction(payload.id, payload.decodedResult !== undefined ? payload.decodedResult : payload.result, payload.log, payload.result);
}
