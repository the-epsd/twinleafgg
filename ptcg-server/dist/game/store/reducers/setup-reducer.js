...
function* setupPlayersConcurrently(players, chooseCardsOptions, state, store, next) {
    const prompts = players.map(player => createSetupChooseCardsPrompt(player, chooseCardsOptions));
    yield store.prompt(state, prompts, () => {
        next();
    });
}
function createSetupChooseCardsPrompt(player, chooseCardsOptions) {
    const prompt = new ChooseCardsPrompt(player, GameMessage.CHOOSE_STARTING_POKEMONS, player.hand, {}, Object.assign(Object.assign({}, chooseCardsOptions), { blocked: setupBlockedCardIndexes(player) }));
    let applied = false;
...
function setupBlockedCardIndexes(player) {
    const blocked = [];
    player.hand.cards.forEach((c, index) => {
        if (!(c.tags.includes((CardTag.PLAY_DURING_SETUP)) || (c instanceof PokemonCard && c.stage === Stage.BASIC))) {
            blocked.push(index);
