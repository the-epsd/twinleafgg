...
export function WAS_TRAINER_USED(effect, card) {
    return effect instanceof TrainerEffect && effect.trainerCard === card;
}
export function DISCARD_X_CARDS_FROM_YOUR_HAND(effect, store, state, minAmount, maxAmount) {
    ...
    if (cards.length > maxAmount) {
        state = store.prompt(state, new ChooseCardsPrompt(effect.player, GameMessage.CHOOSE_CARD_TO_DISCARD, player.hand, {}, { allowCancel: false, min: minAmount, max: maxAmount }), cards => {
            cards = cards || [];
            if (cards.length === 0) {
                return;
            }
...
export function TRAINER_SHOW_OPPONENT_CARDS(effect, store, state) {
    const player = effect.player;
    const opponent = StateUtils.getOpponent(state, player);
    const cards = [];
    if (cards.length > 0) {
        store.prompt(state, new ShowCardsPrompt(opponent.id, GameMessage.CARDS_SHOWED_BY_THE_OPPONENT, cards), () => { });
    }
}
export function SHUFFLE_DECK(effect, store, state) {
    const player = effect.player;
