...
import { AttachEnergyPrompt, CardList, ChooseCardsPrompt, ChoosePokemonPrompt, ChoosePrizePrompt, ChooseEnergyPrompt, ConfirmPrompt, EnergyCard, GameError, GameLog, GameMessage, MoveDamagePrompt, PlayerType, PokemonCardList, PowerType, SelectPrompt, ShowCardsPrompt, ShuffleDeckPrompt, SlotType, StateUtils, TrainerCard } from '../..';
 ...
 */
export function MOVED_TO_ACTIVE_THIS_TURN(player, pokemon) {
...
export function SEARCH_YOUR_DECK_FOR_POKEMON_AND_PUT_ONTO_BENCH(store, state, player, filter = {}, options = {}) {
    ...
    return store.prompt(state, new ChooseCardsPrompt(player, GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, player.deck, filter, options), selected => {
        ...
        cards.forEach((card, index) => {
            const playPokemonFromDeckEffect = new PlayPokemonFromDeckEffect(player, card, slots[index]);
 ...
 */
export function NEXT_TURN_ATTACK_BONUS(effect, options) {
 ...
 */
export function NEXT_TURN_ATTACK_BASE_DAMAGE(effect, options) {
...
function* copyBenchAttackGenerator(next, store, state, effect, options) {
    ...
    let targets = [];
    yield store.prompt(state, new ChoosePokemonPrompt(player.id, GameMessage.CHOOSE_POKEMON, PlayerType.BOTTOM_PLAYER, [SlotType.BENCH], { allowCancel }), results => {
        targets = results || [];
        next();
    });
    if (targets.length === 0) {
    ...
    let selected = null;
    yield store.prompt(state, new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_COPY, [benchedCard], { allowCancel }), result => {
        selected = result;
        next();
    });
 ...
 */
export function COPY_BENCH_ATTACK(store, state, effect, options = {}) {
...
function* copyOpponentActiveAttackGenerator(next, store, state, effect) {
    ...
    let selected;
    yield store.prompt(state, new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_COPY, [pokemonCard], { allowCancel: false }), result => {
        selected = result;
        next();
    });
...
export function COPY_OPPONENT_ACTIVE_ATTACK(store, state, effect) {
...
export function COPY_OPPONENTS_LAST_ATTACK(store, state, effect) {
 ...
 */
export function TOOL_ACTIVE_DAMAGE_BONUS(store, state, effect, tool, options) {
 ...
 */
export function TOOL_SET_HP_IF(store, state, effect, tool, options) {
 ...
 */
export function DEVOLVE_DEFENDING_AFTER_ATTACK(store, state, effect, index, user, destination = 'hand') {
...
export function THIS_ATTACK_DOES_X_DAMAGE_TO_X_OF_YOUR_OPPONENTS_POKEMON(damage, effect, store, state, min, max, applyWeaknessAndResistance = false, slots) {
    ...
    if (targets.length === 0 && !(slots === null || slots === void 0 ? void 0 : slots.includes(SlotType.ACTIVE))) {
        return state;
    }
...
export function THIS_ATTACK_DOES_X_DAMAGE_TO_EACH_OF_YOUR_OPPONENTS_POKEMON(damage, effect, store, state, benchOnly = false) {
    const player = effect.player;
    const opponent = StateUtils.getOpponent(state, player);
 ...
 */
export function DISCARD_SPECIFIC_ENERGY_FROM_THIS_POKEMON(store, state, effect, energyMap) {
    const player = effect.player;
    const checkProvidedEnergy = new CheckProvidedEnergyEffect(player);
    state = store.reduceEffect(state, checkProvidedEnergy);
    state = store.prompt(state, new ChooseEnergyPrompt(player.id, GameMessage.CHOOSE_ENERGIES_TO_DISCARD, checkProvidedEnergy.energyMap, energyMap, { allowCancel: false }), energy => {
...
export function PUT_SPECIFIC_ENERGY_FROM_THIS_POKEMON_INTO_HAND(store, state, effect, energyMap) {
    const player = effect.player;
    const checkProvidedEnergy = new CheckProvidedEnergyEffect(player);
    state = store.reduceEffect(state, checkProvidedEnergy);
    state = store.prompt(state, new ChooseEnergyPrompt(player.id, GameMessage.CHOOSE_CARD_TO_HAND, checkProvidedEnergy.energyMap, energyMap, { allowCancel: false }), energy => {
        const cards = (energy || []).map(e => e.card);
 ...
 */
export function AS_OFTEN_AS_YOU_LIKE_ATTACH_BASIC_TYPE_ENERGY_FROM_HAND(store, state, player, cardType, options = {}) {
    const { destinationSlots = [SlotType.BENCH, SlotType.ACTIVE], targetFilter, promptOptions = {} } = options;
    const basicEnergyName = getBasicEnergyNameByType(cardType);
    if (basicEnergyName === undefined) {
        throw new GameError(GameMessage.CANNOT_USE_POWER);
    }
    const hasMatchingEnergyInHand = player.hand.cards.some(card => card instanceof EnergyCard
    ...
    return store.prompt(state, new AttachEnergyPrompt(player.id, GameMessage.ATTACH_ENERGY_CARDS, player.hand, PlayerType.BOTTOM_PLAYER, destinationSlots, { superType: SuperType.ENERGY, energyType: EnergyType.BASIC, name: basicEnergyName }, Object.assign({ allowCancel: true, min: 1, max: 1, blockedTo }, promptOptions)), transfers => {
        transfers = transfers || [];
        for (const transfer of transfers) {
            const target = StateUtils.getTarget(state, player, transfer.to);
            const energyCard = transfer.card;
 ...
 */
export function ATTACH_X_TYPE_ENERGY_FROM_DISCARD_TO_1_OF_YOUR_POKEMON(store, state, player, amount, cardType, options = {}) {
    const { destinationSlots = [SlotType.BENCH, SlotType.ACTIVE], targetFilter, energyFilter = {}, min = 1, allowCancel = false, onAttached } = options;
    if (player.discard.cards.length === 0 || amount <= 0) {
        return state;
    }
    const blockedTo = getBlockedTargetsFromFilter(player, targetFilter);
    const promptEnergyFilter = Object.assign({ superType: SuperType.ENERGY }, energyFilter);
    const promptOptions = {
        allowCancel,
        min: Math.max(0, min),
        max: amount,
    ...
    return store.prompt(state, new AttachEnergyPrompt(player.id, GameMessage.ATTACH_ENERGY_CARDS, player.discard, PlayerType.BOTTOM_PLAYER, destinationSlots, promptEnergyFilter, promptOptions), transfers => {
        transfers = transfers || [];
        for (const transfer of transfers) {
            const target = StateUtils.getTarget(state, player, transfer.to);
            const energyCard = transfer.card;
 ...
 */
export function ATTACH_UP_TO_X_ENERGY_FROM_DECK_TO_Y_OF_YOUR_POKEMON(store, state, player, maxEnergyCards, maxPokemonTargets, options = {}) {
    const { destinationSlots = [SlotType.BENCH, SlotType.ACTIVE], targetFilter, energyFilter = {}, min = 0, allowCancel = false, differentTypes = false, differentTargets = false, sameTarget = false, validCardTypes, maxPerType, onAttached } = options;
    if (player.deck.cards.length === 0 || maxEnergyCards <= 0 || maxPokemonTargets <= 0) {
        return state;
    }
    const blockedTo = getBlockedTargetsFromFilter(player, targetFilter);
    return store.prompt(state, new AttachEnergyPrompt(player.id, GameMessage.ATTACH_ENERGY_CARDS, player.deck, PlayerType.BOTTOM_PLAYER, destinationSlots, Object.assign({ superType: SuperType.ENERGY }, energyFilter), {
        allowCancel,
        min: Math.max(0, min),
        max: maxEnergyCards,
        blockedTo,
 ...
 */
export function DISCARD_TOP_X_CARDS_FROM_YOUR_DECK(store, state, player, amount, card, sourceEffect) {
 ...
 */
export function COUNT_MATCHING_CARDS_IN_ZONE(player, zone, filter = {}, predicate = () => true) {
 ...
 */
export function THIS_POKEMON_HAS_ANY_ENERGY_ATTACHED(target) {
 ...
 */
export function BLOCK_IF_VSTAR_POWER_USED(player) {
 ...
 */
export function PLAYER_HAS_USED_VSTAR_POWER(player) {
    return player.usedVSTAR === true;
}
/**
 * Returns true if your opponent has already used their VSTAR Power this game.
 */
export function OPPONENT_HAS_USED_VSTAR_POWER(state, player) {
    const opponent = StateUtils.getOpponent(state, player);
 ...
 */
export function DISCARD_TOP_X_OF_OPPONENTS_DECK(store, state, player, amount, card, sourceEffect) {
    const opponent = StateUtils.getOpponent(state, player);
...
export function CLEAN_UP_SUPPORTER(store, effect, player) {
 ...
 */
export function SWITCH_IN_OPPONENT_BENCHED_POKEMON(store, state, player, options = {}) {
    ...
    return store.prompt(state, new ChoosePokemonPrompt(player.id, GameMessage.CHOOSE_POKEMON_TO_SWITCH, PlayerType.TOP_PLAYER, [SlotType.BENCH], { min: 1, max: 1, allowCancel, blocked }), selected => {
        if (!selected || selected.length === 0) {
            return;
        }
 ...
 */
export function SWITCH_OUT_OPPONENT_ACTIVE_POKEMON(store, state, player, options = {}) {
    const { allowCancel = false, blocked = [], onSwitched } = options;
    const opponent = StateUtils.getOpponent(state, player);
    const hasBenchedPokemon = opponent.bench.some(bench => bench.cards.length > 0);
    if (!hasBenchedPokemon) {
    ...
    return store.prompt(state, new ChoosePokemonPrompt(opponent.id, GameMessage.CHOOSE_POKEMON_TO_SWITCH, PlayerType.BOTTOM_PLAYER, [SlotType.BENCH], { min: 1, max: 1, allowCancel, blocked }), selected => {
        if (!selected || selected.length === 0) {
            return;
        }
        opponent.switchPokemon(selected[0], store, state);
 ...
 */
export function OPPONENT_SWITCHES_THEIR_ACTIVE_POKEMON(store, state, player, options = {}) {
 ...
 */
export function GUST_OPPONENT_BENCHED_POKEMON(store, state, player, options = {}) {
 ...
 */
export function MOVE_DAMAGE_COUNTERS(store, state, player, options = {}) {
    const moveEffect = new MoveDamageCountersEffect(player);
    state = store.reduceEffect(state, moveEffect);
    if (moveEffect.preventDefault) {
        return state;
    }
    const { playerType = PlayerType.BOTTOM_PLAYER, slots = [SlotType.ACTIVE, SlotType.BENCH], min = 1, max = undefined, allowCancel = false, blockedFrom = [], blockedTo = [], singleSourceTarget = false, singleDestinationTarget = false } = options;
    const opponent = StateUtils.getOpponent(state, player);
    ...
    return store.prompt(state, new MoveDamagePrompt(player.id, GameMessage.MOVE_DAMAGE, playerType, slots, maxAllowedDamage, {
        allowCancel,
        min,
        max,
 ...
 */
export function LOOK_AT_TOP_X_CARDS_AND_DO_WITH_MATCHING(store, state, player, options) {
    const { topCount, maxMatches, filter = {}, predicate = () => true, chooseMessage = GameMessage.CHOOSE_CARD_TO_HAND, allowCancel = false, remainderDestination = 'shuffle', onCardsChosen } = options;
    ...
    return store.prompt(state, new ChooseCardsPrompt(player, chooseMessage, topCards, {}, { min: 0, max: selectable, allowCancel, blocked }), selected => {
 ...
 */
export function LOOK_AT_TOP_X_CARDS_AND_PUT_UP_TO_Y_MATCHING_CARDS_INTO_HAND(store, state, player, topCount, maxToHand, options = {}) {
 ...
 */
export function LOOK_AT_TOP_X_CARDS_AND_ATTACH_UP_TO_Y_ENERGY(store, state, player, topCount, maxEnergyToAttach, options = {}) {
    const { destinationSlots = [SlotType.BENCH, SlotType.ACTIVE], targetFilter, energyFilter = {}, remainderDestination = 'shuffle', differentTypes = false, differentTargets = false, sameTarget = false, validCardTypes, maxPerType, maxPokemonTargets = maxEnergyToAttach } = options;
    if (player.deck.cards.length === 0 || topCount <= 0 || maxEnergyToAttach <= 0) {
        return state;
    }
    const topCards = new CardList();
    player.deck.moveTo(topCards, Math.min(topCount, player.deck.cards.length));
    const matchingEnergyCount = topCards.cards.filter(card => card instanceof EnergyCard && cardMatchesPartialFilter(card, energyFilter)).length;
    const maxAttach = Math.min(maxEnergyToAttach, matchingEnergyCount);
    const blockedTo = getBlockedTargetsFromFilter(player, targetFilter);
    return store.prompt(state, new AttachEnergyPrompt(player.id, GameMessage.ATTACH_ENERGY_CARDS, topCards, PlayerType.BOTTOM_PLAYER, destinationSlots, Object.assign({ superType: SuperType.ENERGY }, energyFilter), {
        allowCancel: false,
        min: 0,
        max: maxAttach,
        blockedTo,
 ...
 */
export function LOOK_AT_TOP_X_CARDS_AND_BENCH_UP_TO_Y_POKEMON(store, state, player, topCount, maxToBench, options = {}) {
    ...
    return store.prompt(state, new ChooseCardsPrompt(player, GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, topCards, {}, { min: 0, max: selectable, allowCancel: false, blocked }), selected => {
 ...
 */
export function FLIP_UNTIL_TAILS_AND_COUNT_HEADS(store, state, player, callback) {
    const sequenceEffect = new CoinFlipSequenceEffect(player, 'untilTails', (results) => {
 ...
 */
export function THIS_POKEMON_CANNOT_USE_THIS_ATTACK_NEXT_TURN(player, attack) {
 ...
 */
export function THIS_POKEMON_CANNOT_ATTACK_NEXT_TURN(player) {
...
export function ADD_SPECIAL_CONDITIONS_TO_PLAYER_ACTIVE(store, state, player, source, specialConditions, poisonDamage = 10, burnDamage = 20, sleepFlips = 1, confusionDamage = 30) {
    store.reduceEffect(state, new AddSpecialConditionsPowerEffect(player, source, player.active, specialConditions, poisonDamage, burnDamage, sleepFlips, confusionDamage));
}
...
export function ADD_CONFUSION_TO_PLAYER_ACTIVE(store, state, player, source, confusionDamage = 30) {
 ...
 */
export function PREVENT_AND_CLEAR_SPECIAL_CONDITIONS(state, effect, options) {
 ...
 */
export function USE_ABILITY_ONCE_PER_TURN(player, marker, source) {
 ...
 */
export function CAN_PLAY_SUPPORTER_CARD(store, state, player, trainerCard, bypassSupporterTurn = false) {
    try {
 ...
 * @returns true if the card can be played, false otherwise
 */
export function CAN_PLAY_TRAINER_CARD(store, state, player, trainerCard) {
    try {
 ...
 * @returns true if the card can be played, false otherwise
 */
export function CAN_PLAY_ENERGY_CARD(store, state, player, energyCard) {
    try {
        // Only check during player's turn
        if (state.phase !== GamePhase.PLAYER_TURN || state.players[state.activePlayer].id !== player.id) {
            return false;
 ...
 * @returns true if the card can be played, false otherwise
 */
export function CAN_PLAY_POKEMON_CARD(store, state, player, pokemonCard) {
    try {
        // Only check during player's turn
        if (state.phase !== GamePhase.PLAYER_TURN || state.players[state.activePlayer].id !== player.id) {
            return false;
 ...
 * @returns true if the card can be played, false otherwise
 */
export function CAN_PLAY_CARD(store, state, player, card) {
    try {
 ...
 */
export function ON_DAMAGED_BY_OPPONENT_ATTACK_EVEN_IF_KNOCKED_OUT(state, effect, options) {
 ...
 */
export function PREVENT_DAMAGE_TO_YOUR_BENCHED_POKEMON_FROM_OPPONENT_ATTACKS(store, state, effect, options) {
 ...
 */
export function PREVENT_EFFECTS_TO_YOUR_BENCHED_POKEMON_FROM_OPPONENT_ATTACKS(store, state, effect, options) {
 ...
 */
export function SURVIVE_ON_TEN_IF_FULL_HP(store, state, effect, options) {
