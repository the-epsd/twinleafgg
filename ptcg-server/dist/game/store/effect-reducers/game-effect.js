...
import { AttackEffect, EvolveEffect, HealEffect, KnockOutEffect, PlaceDamageCountersEffect, PowerEffect, PutDamageCountersEffect, TrainerPowerEffect, UseAttackEffect, UsePowerEffect, UseStadiumEffect, UseTrainerPowerEffect } from '../effects/game-effects';
            ...
            if (state.phase === GamePhase.ATTACK && isGlimwood && stadium && !HAS_MARKER(GLIMWOOD_REFLIP_USED, player, stadium)) {
                store.prompt(state, new ConfirmPrompt(player.id, GameMessage.WANT_TO_USE_ABILITY), wantToReflip => {
                    if (wantToReflip) {
                        ...
                        ADD_MARKER(GLIMWOOD_REFLIP_USED, player, stadium);
