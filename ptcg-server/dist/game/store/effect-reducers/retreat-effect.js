...
function promptBenchAndRetreat(store, state, player) {
    return store.prompt(state, new ChoosePokemonPrompt(player.id, GameMessage.CHOOSE_POKEMON_TO_SWITCH, PlayerType.BOTTOM_PLAYER, [SlotType.BENCH], { min: 1, max: 1, allowCancel: true, blocked: [] }), (benchResult) => {
        if (benchResult === null) {
            return;
        }
...
export function retreatReducer(store, state, effect) {
    ...
    if (effect instanceof RetreatStartEffect) {
        ...
        if (StateUtils.allEnergyProvidesIdentical(checkProvidedEnergy.energyMap)) {
            const selection = StateUtils.selectMinimalEnergyForCost(checkProvidedEnergy.energyMap, checkRetreatCost.cost);
