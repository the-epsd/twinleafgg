...
function* useStadium(next, store, state, effect) {
    ...
    if (player.deck.cards.length === 0) {
        ...
        let cards = [];
        return store.prompt(state, new game_1.ChooseCardsPrompt(player.id, game_message_1.GameMessage.CHOOSE_CARD_TO_PUT_ONTO_BENCH, player.deck, { superType: card_types_1.SuperType.TRAINER, trainerType: card_types_1.TrainerType.TOOL }, { min: 0, max: 1, allowCancel: true }), selectedCards => {
            cards = selectedCards || [];
