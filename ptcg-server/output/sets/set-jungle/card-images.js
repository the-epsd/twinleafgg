"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.WigglytuffArt = exports.WeepinbellArt = exports.VileplumeArt = exports.VictreebelArt = exports.VenonatArt = exports.VenomothArt = exports.VaporeonArt = exports.SpearowArt = exports.SeakingArt = exports.ScytherArt = exports.RhyhornArt = exports.RhydonArt = exports.RapidashArt = exports.PrimeapeArt = exports.PokeBallArt = exports.PinsirArt = exports.PikachuArt = exports.PidgeotArt = exports.PersianArt = exports.ParasectArt = exports.ParasArt = exports.OddishArt = exports.NidoranFemaleArt = exports.NidorinaArt = exports.NidoqueenArt = exports.MeowthArt = exports.MarowakArt = exports.LickitungArt = exports.KangaskhanArt = exports.JolteonArt = exports.JigglypuffArt = exports.GoldeenArt = exports.GloomArt = exports.FlareonArt = exports.FearowArt = exports.ExeggutorArt = exports.ExeggcuteArt = exports.ElectrodeArt = exports.EeveeArt = exports.DodrioArt = exports.CuboneArt = exports.ClefableArt = exports.ButterfreeArt = exports.BellsproutArt = void 0;
const bellsprout_1 = require("./bellsprout");
const butterfree_1 = require("./butterfree");
const clefable_1 = require("./clefable");
const cubone_1 = require("./cubone");
const dodrio_1 = require("./dodrio");
const eevee_1 = require("./eevee");
const electrode_1 = require("./electrode");
const exeggcute_1 = require("./exeggcute");
const exeggutor_1 = require("./exeggutor");
const fearow_1 = require("./fearow");
const flareon_1 = require("./flareon");
const gloom_1 = require("./gloom");
const goldeen_1 = require("./goldeen");
const jigglypuff_1 = require("./jigglypuff");
const jolteon_1 = require("./jolteon");
const kangaskhan_1 = require("./kangaskhan");
const lickitung_1 = require("./lickitung");
const marowak_1 = require("./marowak");
const meowth_1 = require("./meowth");
const nidoqueen_1 = require("./nidoqueen");
const nidoran_female_1 = require("./nidoran-female");
const nidorina_1 = require("./nidorina");
const oddish_1 = require("./oddish");
const paras_1 = require("./paras");
const parasect_1 = require("./parasect");
const persian_1 = require("./persian");
const pidgeot_1 = require("./pidgeot");
const pikachu_1 = require("./pikachu");
const pinsir_1 = require("./pinsir");
const pokeball_1 = require("./pokeball");
const primeape_1 = require("./primeape");
const rapidash_1 = require("./rapidash");
const rhydon_1 = require("./rhydon");
const rhyhorn_1 = require("./rhyhorn");
const scyther_1 = require("./scyther");
const seaking_1 = require("./seaking");
const spearow_1 = require("./spearow");
const vaporeon_1 = require("./vaporeon");
const venomoth_1 = require("./venomoth");
const venonat_1 = require("./venonat");
const victreebel_1 = require("./victreebel");
const vileplume_1 = require("./vileplume");
const weepinbell_1 = require("./weepinbell");
const wigglytuff_1 = require("./wigglytuff");
class BellsproutArt extends bellsprout_1.Bellsprout {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/bellsprout-jungle-ju-49.jpg';
    }
}
exports.BellsproutArt = BellsproutArt;
class ButterfreeArt extends butterfree_1.Butterfree {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/butterfree-jungle-ju-33.jpg';
    }
}
exports.ButterfreeArt = ButterfreeArt;
class ClefableArt extends clefable_1.Clefable {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://i0.wp.com/pkmncards.com/wp-content/uploads/clefable-jungle-ju-1.jpg?fit=600%2C825&ssl=1';
    }
}
exports.ClefableArt = ClefableArt;
class CuboneArt extends cubone_1.Cubone {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/cubone-jungle-ju-50.jpg';
    }
}
exports.CuboneArt = CuboneArt;
class DodrioArt extends dodrio_1.Dodrio {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://i0.wp.com/pkmncards.com/wp-content/uploads/dodrio-jungle-ju-34.jpg?fit=600%2C825&ssl=1';
    }
}
exports.DodrioArt = DodrioArt;
class EeveeArt extends eevee_1.Eevee {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/eevee-jungle-ju-51.jpg';
    }
}
exports.EeveeArt = EeveeArt;
class ElectrodeArt extends electrode_1.Electrode {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/electrode-jungle-ju-2.jpg';
    }
}
exports.ElectrodeArt = ElectrodeArt;
class ExeggcuteArt extends exeggcute_1.Exeggcute {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/exeggcute-jungle-ju-52.jpg';
    }
}
exports.ExeggcuteArt = ExeggcuteArt;
class ExeggutorArt extends exeggutor_1.Exeggutor {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/exeggutor-jungle-ju-35.jpg';
    }
}
exports.ExeggutorArt = ExeggutorArt;
class FearowArt extends fearow_1.Fearow {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/fearow-jungle-ju-36.jpg';
    }
}
exports.FearowArt = FearowArt;
class FlareonArt extends flareon_1.Flareon {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/flareon-jungle-ju-3.jpg';
    }
}
exports.FlareonArt = FlareonArt;
class GloomArt extends gloom_1.Gloom {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/gloom-jungle-ju-37.jpg';
    }
}
exports.GloomArt = GloomArt;
class GoldeenArt extends goldeen_1.Goldeen {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/goldeen-jungle-ju-53.jpg';
    }
}
exports.GoldeenArt = GoldeenArt;
class JigglypuffArt extends jigglypuff_1.Jigglypuff {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/jigglypuff-jungle-ju-54.jpg';
    }
}
exports.JigglypuffArt = JigglypuffArt;
class JolteonArt extends jolteon_1.Jolteon {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/jolteon-jungle-ju-4.jpg';
    }
}
exports.JolteonArt = JolteonArt;
class KangaskhanArt extends kangaskhan_1.Kangaskhan {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/kangaskhan-jungle-ju-5.jpg';
    }
}
exports.KangaskhanArt = KangaskhanArt;
class LickitungArt extends lickitung_1.Lickitung {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/lickitung-jungle-ju-38.jpg';
    }
}
exports.LickitungArt = LickitungArt;
class MarowakArt extends marowak_1.Marowak {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/marowak-jungle-ju-39.jpg';
    }
}
exports.MarowakArt = MarowakArt;
class MeowthArt extends meowth_1.Meowth {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/meowth-jungle-ju-56.jpg';
    }
}
exports.MeowthArt = MeowthArt;
class NidoqueenArt extends nidoqueen_1.Nidoqueen {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/nidoqueen-jungle-ju-7.jpg';
    }
}
exports.NidoqueenArt = NidoqueenArt;
class NidorinaArt extends nidorina_1.Nidorina {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/nidorina-jungle-ju-40.jpg';
    }
}
exports.NidorinaArt = NidorinaArt;
class NidoranFemaleArt extends nidoran_female_1.NidoranFemale {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/nidoran-female-jungle-ju-57.jpg';
    }
}
exports.NidoranFemaleArt = NidoranFemaleArt;
class OddishArt extends oddish_1.Oddish {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/oddish-jungle-ju-58.jpg';
    }
}
exports.OddishArt = OddishArt;
class ParasArt extends paras_1.Paras {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/paras-jungle-ju-59.jpg';
    }
}
exports.ParasArt = ParasArt;
class ParasectArt extends parasect_1.Parasect {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/parasect-jungle-ju-41.jpg';
    }
}
exports.ParasectArt = ParasectArt;
class PersianArt extends persian_1.Persian {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/persian-jungle-ju-42.jpg';
    }
}
exports.PersianArt = PersianArt;
class PidgeotArt extends pidgeot_1.Pidgeot {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/pidgeot-jungle-ju-8.jpg';
    }
}
exports.PidgeotArt = PidgeotArt;
class PikachuArt extends pikachu_1.Pikachu {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/pikachu-jungle-ju-60.jpg';
    }
}
exports.PikachuArt = PikachuArt;
class PinsirArt extends pinsir_1.Pinsir {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/pinsir-jungle-ju-9.jpg';
    }
}
exports.PinsirArt = PinsirArt;
class PokeBallArt extends pokeball_1.PokeBall {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://i0.wp.com/pkmncards.com/wp-content/uploads/poke-ball-jungle-ju-64.jpg?fit=600%2C825&ssl=1';
    }
}
exports.PokeBallArt = PokeBallArt;
class PrimeapeArt extends primeape_1.Primeape {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/primeape-jungle-ju-43.jpg';
    }
}
exports.PrimeapeArt = PrimeapeArt;
class RapidashArt extends rapidash_1.Rapidash {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/rapidash-jungle-ju-44.jpg';
    }
}
exports.RapidashArt = RapidashArt;
class RhydonArt extends rhydon_1.Rhydon {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/rhydon-jungle-ju-45.jpg';
    }
}
exports.RhydonArt = RhydonArt;
class RhyhornArt extends rhyhorn_1.Rhyhorn {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/rhyhorn-jungle-ju-61.jpg';
    }
}
exports.RhyhornArt = RhyhornArt;
class ScytherArt extends scyther_1.Scyther {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://i0.wp.com/pkmncards.com/wp-content/uploads/scyther-jungle-ju-10.jpg?fit=600%2C825&ssl=1';
    }
}
exports.ScytherArt = ScytherArt;
class SeakingArt extends seaking_1.Seaking {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/seaking-jungle-ju-46.jpg';
    }
}
exports.SeakingArt = SeakingArt;
class SpearowArt extends spearow_1.Spearow {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/spearow-jungle-ju-62.jpg';
    }
}
exports.SpearowArt = SpearowArt;
class VaporeonArt extends vaporeon_1.Vaporeon {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/vaporeon-jungle-ju-12.jpg';
    }
}
exports.VaporeonArt = VaporeonArt;
class VenomothArt extends venomoth_1.Venomoth {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/venomoth-jungle-ju-13.jpg';
    }
}
exports.VenomothArt = VenomothArt;
class VenonatArt extends venonat_1.Venonat {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/venonat-jungle-ju-63.jpg';
    }
}
exports.VenonatArt = VenonatArt;
class VictreebelArt extends victreebel_1.Victreebel {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/victreebel-jungle-ju-14.jpg';
    }
}
exports.VictreebelArt = VictreebelArt;
class VileplumeArt extends vileplume_1.Vileplume {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/vileplume-jungle-ju-15.jpg';
    }
}
exports.VileplumeArt = VileplumeArt;
class WeepinbellArt extends weepinbell_1.Weepinbell {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/weepinbell-jungle-ju-48.jpg';
    }
}
exports.WeepinbellArt = WeepinbellArt;
class WigglytuffArt extends wigglytuff_1.Wigglytuff {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://pkmncards.com/wp-content/uploads/wigglytuff-jungle-ju-16.jpg';
    }
}
exports.WigglytuffArt = WigglytuffArt;
