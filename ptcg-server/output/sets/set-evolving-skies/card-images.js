"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ZoruaArt = exports.ZoroarkArt = exports.ZinniasResolveArt = exports.WishiwashiArt = exports.VigorothArt = exports.UmbreonVMAXArt = exports.UmbreonVArt = exports.TropiusArt = exports.SylveonVMAXArt = exports.SylveonVArt = exports.SwitchingCupsArt = exports.SwabluArt = exports.SuicuneVArt = exports.StormyMountainsArt = exports.SkiploomArt = exports.RescueCarrierArt = exports.RegielekiArt = exports.RegidragoArt = exports.RayquazaVMAXArt = exports.RayquazaVArt = exports.RaihanArt = exports.PumpkabooArt = exports.MareepArt = exports.LeafeonVMAXArt = exports.LeafeonVArt = exports.JumpluffArt = exports.HoppipArt = exports.GyaradosVMAXArt = exports.GyaradosVArt = exports.GalarianZapdosArt = exports.GalarianMoltresArt = exports.GalarianArticunoArt = exports.FullFaceGuardArt = exports.FlappleArt = exports.FlaaffyArt = exports.EspeonVMAXArt = exports.EspeonVArt = exports.DuraludonVMAXArt = exports.DuraludonVArt = exports.DrampaArt = exports.DragoniteVArt = exports.CrystalCaveArt = exports.CopycatArt = void 0;
const copycat_1 = require("./copycat");
const crystal_cave_1 = require("./crystal-cave");
const dragonite_v_1 = require("./dragonite-v");
const drampa_1 = require("./drampa");
const duraludon_v_1 = require("./duraludon-v");
const duraludon_vmax_1 = require("./duraludon-vmax");
const espeon_v_1 = require("./espeon-v");
const espeon_vmax_1 = require("./espeon-vmax");
const EVS_130_Vigoroth_1 = require("./EVS_130_Vigoroth");
const EVS_2_Hoppip_1 = require("./EVS_2_Hoppip");
const EVS_3_Skiploom_1 = require("./EVS_3_Skiploom");
const EVS_4_Jumpluff_1 = require("./EVS_4_Jumpluff");
const EVS_82_Galarian_Zapdos_1 = require("./EVS_82_Galarian_Zapdos");
const flaaffy_1 = require("./flaaffy");
const flapple_1 = require("./flapple");
const full_face_guard_1 = require("./full-face-guard");
const galarian_articuno_1 = require("./galarian-articuno");
const galarian_moltres_1 = require("./galarian-moltres");
const gyarados_v_1 = require("./gyarados-v");
const gyarados_vmax_1 = require("./gyarados-vmax");
const leafeon_v_1 = require("./leafeon-v");
const leafeon_vmax_1 = require("./leafeon-vmax");
const mareep_1 = require("./mareep");
const pumpkaboo_1 = require("./pumpkaboo");
const raihan_1 = require("./raihan");
const rayquaza_v_1 = require("./rayquaza-v");
const rayquaza_vmax_1 = require("./rayquaza-vmax");
const regidrago_1 = require("./regidrago");
const regieleki_1 = require("./regieleki");
const rescue_carrier_1 = require("./rescue-carrier");
const stormy_mountains_1 = require("./stormy-mountains");
const suicune_v_1 = require("./suicune-v");
const swablu_1 = require("./swablu");
const switching_cups_1 = require("./switching-cups");
const sylveon_v_1 = require("./sylveon-v");
const sylveon_vmax_1 = require("./sylveon-vmax");
const tropius_1 = require("./tropius");
const umbreon_v_1 = require("./umbreon-v");
const umbreon_vmax_1 = require("./umbreon-vmax");
const wishiwashi_1 = require("./wishiwashi");
const zinnias_resolve_1 = require("./zinnias-resolve");
const zoroark_1 = require("./zoroark");
const zorua_1 = require("./zorua");
class CopycatArt extends copycat_1.Copycat {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_143_R_EN_LG.png';
    }
}
exports.CopycatArt = CopycatArt;
class CrystalCaveArt extends crystal_cave_1.CrystalCave {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_144_R_EN_LG.png';
    }
}
exports.CrystalCaveArt = CrystalCaveArt;
class DragoniteVArt extends dragonite_v_1.DragoniteV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_191_R_EN_LG.png';
    }
}
exports.DragoniteVArt = DragoniteVArt;
class DrampaArt extends drampa_1.Drampa {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_119_R_EN_LG.png';
    }
}
exports.DrampaArt = DrampaArt;
class DuraludonVArt extends duraludon_v_1.DuraludonV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_122_R_EN_LG.png';
    }
}
exports.DuraludonVArt = DuraludonVArt;
class DuraludonVMAXArt extends duraludon_vmax_1.DuraludonVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_123_R_EN_LG.png';
    }
}
exports.DuraludonVMAXArt = DuraludonVMAXArt;
class EspeonVArt extends espeon_v_1.EspeonV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_064_R_EN_LG.png';
    }
}
exports.EspeonVArt = EspeonVArt;
class EspeonVMAXArt extends espeon_vmax_1.EspeonVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_065_R_EN_LG.png';
    }
}
exports.EspeonVMAXArt = EspeonVMAXArt;
class FlaaffyArt extends flaaffy_1.Flaaffy {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_055_R_EN_LG.png';
    }
}
exports.FlaaffyArt = FlaaffyArt;
class FlappleArt extends flapple_1.Flapple {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_120_R_EN_LG.png';
    }
}
exports.FlappleArt = FlappleArt;
class FullFaceGuardArt extends full_face_guard_1.FullFaceGuard {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_148_R_EN_LG.png';
    }
}
exports.FullFaceGuardArt = FullFaceGuardArt;
class GalarianArticunoArt extends galarian_articuno_1.GalarianArticuno {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_063_R_EN_LG.png';
    }
}
exports.GalarianArticunoArt = GalarianArticunoArt;
class GalarianMoltresArt extends galarian_moltres_1.GalarianMoltres {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_093_R_EN_LG.png';
    }
}
exports.GalarianMoltresArt = GalarianMoltresArt;
class GalarianZapdosArt extends EVS_82_Galarian_Zapdos_1.GalarianZapdos {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_082_R_EN_LG.png';
    }
}
exports.GalarianZapdosArt = GalarianZapdosArt;
class GyaradosVArt extends gyarados_v_1.GyaradosV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_028_R_EN.png';
    }
}
exports.GyaradosVArt = GyaradosVArt;
class GyaradosVMAXArt extends gyarados_vmax_1.GyaradosVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_029_R_EN.png';
    }
}
exports.GyaradosVMAXArt = GyaradosVMAXArt;
class HoppipArt extends EVS_2_Hoppip_1.Hoppip {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_002_R_EN_LG.png';
    }
}
exports.HoppipArt = HoppipArt;
class JumpluffArt extends EVS_4_Jumpluff_1.Jumpluff {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_004_R_EN_LG.png';
    }
}
exports.JumpluffArt = JumpluffArt;
class LeafeonVArt extends leafeon_v_1.LeafeonV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_007_R_EN_LG.png';
    }
}
exports.LeafeonVArt = LeafeonVArt;
class LeafeonVMAXArt extends leafeon_vmax_1.LeafeonVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_008_R_EN_LG.png';
    }
}
exports.LeafeonVMAXArt = LeafeonVMAXArt;
class MareepArt extends mareep_1.Mareep {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_054_R_EN_LG.png';
    }
}
exports.MareepArt = MareepArt;
class PumpkabooArt extends pumpkaboo_1.Pumpkaboo {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_076_R_EN_LG.png';
    }
}
exports.PumpkabooArt = PumpkabooArt;
class RaihanArt extends raihan_1.Raihan {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_152_R_EN_LG.png';
    }
}
exports.RaihanArt = RaihanArt;
class RayquazaVArt extends rayquaza_v_1.RayquazaV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_110_R_EN_LG.png';
    }
}
exports.RayquazaVArt = RayquazaVArt;
class RayquazaVMAXArt extends rayquaza_vmax_1.RayquazaVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_111_R_EN_LG.png';
    }
}
exports.RayquazaVMAXArt = RayquazaVMAXArt;
class RegidragoArt extends regidrago_1.Regidrago {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_124_R_EN_LG.png';
    }
}
exports.RegidragoArt = RegidragoArt;
class RegielekiArt extends regieleki_1.Regieleki {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_060_R_EN_LG.png';
    }
}
exports.RegielekiArt = RegielekiArt;
class RescueCarrierArt extends rescue_carrier_1.RescueCarrier {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_154_R_EN_LG.png';
    }
}
exports.RescueCarrierArt = RescueCarrierArt;
class SkiploomArt extends EVS_3_Skiploom_1.Skiploom {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_003_R_EN_LG.png';
    }
}
exports.SkiploomArt = SkiploomArt;
class StormyMountainsArt extends stormy_mountains_1.StormyMountains {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_161_R_EN_LG.png';
    }
}
exports.StormyMountainsArt = StormyMountainsArt;
class SuicuneVArt extends suicune_v_1.SuicuneV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_031_R_EN_LG.png';
    }
}
exports.SuicuneVArt = SuicuneVArt;
class SwabluArt extends swablu_1.Swablu {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_132_R_EN_LG.png';
    }
}
exports.SwabluArt = SwabluArt;
class SwitchingCupsArt extends switching_cups_1.SwitchingCups {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_162_R_EN_LG.png';
    }
}
exports.SwitchingCupsArt = SwitchingCupsArt;
class SylveonVArt extends sylveon_v_1.SylveonV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_074_R_EN.png';
    }
}
exports.SylveonVArt = SylveonVArt;
class SylveonVMAXArt extends sylveon_vmax_1.SylveonVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_075_R_EN.png';
    }
}
exports.SylveonVMAXArt = SylveonVMAXArt;
class TropiusArt extends tropius_1.Tropius {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_006_R_EN_LG.png';
    }
}
exports.TropiusArt = TropiusArt;
class UmbreonVArt extends umbreon_v_1.UmbreonV {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_094_R_EN_LG.png';
    }
}
exports.UmbreonVArt = UmbreonVArt;
class UmbreonVMAXArt extends umbreon_vmax_1.UmbreonVMAX {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_095_R_EN_LG.png';
    }
}
exports.UmbreonVMAXArt = UmbreonVMAXArt;
class VigorothArt extends EVS_130_Vigoroth_1.Vigoroth {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_130_R_EN_LG.png';
    }
}
exports.VigorothArt = VigorothArt;
class WishiwashiArt extends wishiwashi_1.Wishiwashi {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_046_R_EN_LG.png';
    }
}
exports.WishiwashiArt = WishiwashiArt;
class ZinniasResolveArt extends zinnias_resolve_1.ZinniasResolve {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_164_R_EN_LG.png';
    }
}
exports.ZinniasResolveArt = ZinniasResolveArt;
class ZoroarkArt extends zoroark_1.Zoroark {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_103_R_EN_LG.png';
    }
}
exports.ZoroarkArt = ZoroarkArt;
class ZoruaArt extends zorua_1.Zorua {
    constructor() {
        super(...arguments);
        this.cardImage = 'https://limitlesstcg.nyc3.digitaloceanspaces.com/tpci/EVS/EVS_102_R_EN_LG.png';
    }
}
exports.ZoruaArt = ZoruaArt;
